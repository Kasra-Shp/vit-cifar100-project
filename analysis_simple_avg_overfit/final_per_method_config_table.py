...
values and the same resolution arithmetic add_method() uses, producing the
VERIFICATION table for this session's two tasks:
  Task A (analysis_rankext_bughunt/): no bug found -> no code change ->
    rank_extension's calibration/rank-schedule config is BYTE-IDENTICAL to the
    prior (fix2) session's own verification table.
  Task B (analysis_simple_avg_overfit/): no rank/dropout change -> LORA_R/
    LORA_DROPOUT unchanged; new flags GROWING_OVERFITTING_DIAGNOSTICS_ENABLED /
    GROWING_OVERFITTING_VAL_CE_RISE_THRESHOLD surfaced as extra columns.
...
NUM_STEPS = 5
EPOCHS = 9                          # confirmed unchanged this session (Task A and B both -- not touched)
LORA_R = 80                         # confirmed unchanged this session (Task B: rank reduction evaluated, NOT adopted)
LORA_DROPOUT = 0.05                 # confirmed unchanged this session (Task B: dropout increase evaluated, NOT adopted)
LORA_ALPHA = 160.0
LAMBDA_ORTH = 50.0
KD_WEIGHT = 1.0
KD_TEMPERATURES = [2.0]
...
RANKEXT_RANK_SCHEDULE = [16, 32, 48, 64, 80]           # unchanged this session (Task A: no bug -> no fix -> no change)
...
RANKEXT_FAMILY_AWARE_CALIBRATION_ENABLED = True                 # unchanged this session
RANKEXT_CONFIDENCE_WEIGHTED_CALIBRATION_ENABLED = True          # unchanged this session (Task A found no bug in this mechanism)

# Task B (this session): new diagnostic-only flags. Purely additive -- do not
# affect training, rank, dropout, or any existing accuracy number.
...
def add_method(method_name, family, uses_kd=False, kd_temperature=0.0,
    ...
    rows.append({
        ...
        "rankext_bug_found_task_a": False,
