...
#
# ==============================================================================
# FINAL 8-ARM FAMILY-SPECIFIC EXPERIMENT (5x20) -- derived by copying
# experiments_prepared/simpleavg_rankext_singlejob_improvement_pack_5x20.py
# (job 4969059's "current corrected pipeline", itself derived from the
# VERIFIED R7/Exp1 harness, supervisor_exp1_cifar100_5x20_fixed_rankext.py)
# and applying a small, precisely-documented set of changes. Both source
# scripts are left completely UNTOUCHED at their own paths -- this is a new,
# separate, standalone file.
#
# PURPOSE: restore the clean 4-arm factorial structure (Plain / KD / Orth /
# Orth+KD) for EACH family, using FAMILY-SPECIFIC KD weights and
# orthogonality formulations, per the design audit at
# R7/lambda_and_kd_weight_design_audit.md:
#   - SimpleAvg keeps its CURRENT DenseOrth (lambda=20) and gets a REDUCED,
#     no-warmup KD weight of 0.5 (down from the current pipeline's weight=1),
#     because the audit's loss-scale evidence showed KD weight=1 is
#     comparable-to-larger than CE for SimpleAvg and 0.5 best matches the
#     effective dose of the already-successful warmup arm.
#   - RankExt SWITCHES from the current pipeline's DenseOrth back to the
#     HISTORICAL FactorOrth formulation (lambda=50, Exp1/R7's own
#     `orth_mode="factor_orth"` code path -- see compute_delta_orth_components()
#     and DeltaOrthRankExtensionTrainer, both UNCHANGED below) and gets an
#     INCREASED KD weight of 1.5 (up from the current pipeline's weight=1),
#     because the audit found RankExt's KD/CE loss-pressure headroom is
#     several times smaller than SimpleAvg's at the same nominal weight.
#
# THIS IS NOT A RETURN TO THE FULL HISTORICAL EXP1 PROTOCOL. Only the
# FactorOrth orthogonality *formulation* is restored (its loss geometry,
# lambda=50, and its own pre-existing RANKEXT_ORTH_LAMBDA_WARMUP_* warmup --
# all UNCHANGED, reused verbatim from the current pipeline, which never
# deleted this code path, only stopped calling it). Everything else stays at
# the CURRENT pipeline's values: 6 epochs (not 9), old-seen-only KD (not
# historical full-100-way KD), the FIXED classifier protected-row restoration
# (true `tensor[idx] = value` in-place assignment -- see
# restore_protected_classifier_rows()'s own docstring below for the bug this
# fixes; the historical advanced-indexing `.copy_()` no-op is NEVER
# reinstated), the current evaluation/calibration/optimizer/scheduler
# pipeline, and arithmetic-only merging for SimpleAvg (DO-Merge is REMOVED
# entirely from this script -- out of scope per the task brief). The historical
# "protect_weight" (OLD-CLASS SEMANTIC-SUBSPACE PROJECTED FEATURE
# CONSOLIDATION) mechanism that was historically bundled with the
# `rank_extension_orth_factor_lam_50_kd_T2` identifier is DELIBERATELY NOT
# reintroduced here either -- it is a separate retention mechanism, not part
# of FactorOrth's own geometry, and the new RankExt method names below are
# chosen so they never match RANKEXT_PROJECTED_PROTECT_METHODS (verified in
# the static audit). See R7/final_8arm_family_specific_implementation_audit.md
# Section 4 ("FactorOrth porting audit") for the full A vs. B behavior split.
#
# EVERYTHING not explicitly listed above as a deliberate change is preserved
# BYTE-IDENTICAL in MECHANISM to the current pipeline: dataset, class
# split/ordering, seed=42, optimizer, LR, weight decay, scheduler, LR warmup,
# gradient clipping, mixed precision, validation split, best-epoch selection
# (pure validation-CE argmin), classifier training/stitching/calibration,
# open/restricted evaluation, metrics, per-method seed reset, RankExt's
# persistent-growing-model architecture (GrowingRankLoRALinear, frozen prior
# blocks, new-block warmup, hard classifier-row restoration).
#
# TRAINING ARMS -- 4 SimpleAvg (family="simple_avg", rank=80/alpha=160/
# scaling=2, q_proj/v_proj) + 4 RankExt (family="rank_extension", schedule
# [16,32,48,64,80]/scaling=2, q_proj/v_proj) = 8 total, all seed 42, all 6
# epochs, arithmetic merge only (SimpleAvg) / persistent (RankExt):
#   SA-1: simple_avg                                                          -- plain, unchanged mechanism
#   SA-2: simple_avg_kd_oldseen_T2_weight0p5                                  -- old-seen KD, T=2, weight=0.5, NO warmup
#   SA-3: simple_avg_dense_orth_lam20                                        -- current DenseOrth, lambda=20 (unchanged)
#   SA-4: simple_avg_dense_orth_lam20_kd_oldseen_T2_weight0p5                -- SA-2 + SA-3 combined
#   RE-1: rank_extension                                                      -- plain, unchanged mechanism
#   RE-2: rank_extension_kd_oldseen_T2_weight1p5_warmup                       -- old-seen KD, T=2, weight=1.5, 1-epoch warmup
#   RE-3: rank_extension_factor_orth_lam50                                    -- HISTORICAL FactorOrth, lambda=50 (restored)
#   RE-4: rank_extension_factor_orth_lam50_kd_oldseen_T2_weight1p5_warmup    -- RE-2 + RE-3 combined
#
# MERGE OPERATORS: SimpleAvg arithmetic-only (simple_average_deltas(),
# unchanged) -- DO-Merge is not evaluated anywhere in this script. RankExt
# uses merge_method="persistent" (its own final accumulated model state), as
# in every prior run. TOTAL: 4 + 4 = 8 evaluated configurations.
#
# GRANULAR LOGGING / CL DIAGNOSTICS: this script additionally (a) writes the
# FULL per-batch training-loss log for all 8 arms (job 4969059 only ever
# wrote this for the 2 plain baseline arms -- the accumulators were already
# being populated for every arm upstream; only the final CSV write was
# scoped down), (b) adds `kd_warmup_multiplier`/`kd_class_scope`/
# `dense_orth_mean`/`dense_orth_warmup_multiplier` columns to RankExt's
# per-batch log (pure additive logging fields, computed from already-existing
# local variables -- zero effect on the `loss` tensor actually optimized),
# (c) records a per-CL-step continual-learning retention/plasticity
# trajectory table for RankExt (open + restricted accuracy on every
# previously-seen step's classes, evaluated right after each step's model is
# finalized), and (d) records classifier protected-row weight/bias max-diff
# separately (previously computed but only printed, never saved) for every
# RankExt arm and step. See R7/final_8arm_family_specific_implementation_audit.md
# for the full list.
#
# PERSISTENCE: every arm's trained state is checkpointed and resumed exactly
# as in the current pipeline (`save_arm_checkpoint_atomic` /
# `load_arm_checkpoint_if_present` / `ensure_arm_trained` for SimpleAvg;
# their RankExt analogues below) -- UNCHANGED.
# ==============================================================================
#
...
# long).
RUN_NAME_BASE = f"cifar100_5x20_final_8arm_family_specific_seed{SEED}"
...
FT_EPOCHS = 9
# JOINT SIMPLEAVG+RANKEXT PACK: intentional epoch reduction (user directive,
# 2026-09-16) -- 9 -> 6 for BOTH SimpleAvg (LORA_EPOCHS) and RankExt
# (RANKEXT_EPOCHS) training, applied identically to all 9 training arms. This
# is NOT numerically identical to Exp1 (9 epochs); primary comparisons in
# this run are same-run 6-epoch baselines, historical Exp1 9-epoch numbers
# are context-only references. See thesis_agent/reports/
# simpleavg_rankext_singlejob_improvement_pack_final_readiness.md for the
# historical best-epoch-selection audit (how many methods/steps would have
# been affected by this cap under the OLD 9-epoch protocol) that motivated
# this being an acceptable, deliberate trade against scarce cluster time.
# JOINT PACK ADDITION (GPU preflight): reduced to 1 epoch ONLY when
# FAST_RUN_DEBUG=1 (Phase 1 of the single-job sbatch) -- Phase 2 (the real
# run, FAST_RUN False by default) always gets exactly 6, asserted below.
LORA_EPOCHS = 1 if FAST_RUN else 6
JOINT_EPOCHS = 9
ORTH_EPOCHS = 9
RANKEXT_EPOCHS = 1 if FAST_RUN else 6
...
METHODS_TO_RUN = {
    ...
    # FINAL 8-ARM SCRIPT: job 4969059's weight=1 old-seen-KD SimpleAvg arms
    # are REPLACED by new, distinct base_method keys at weight=0.5 (added
    # further below, near their own add_method() calls) -- these two flags
    # are flipped False so the OLD (weight=1) arms are never retrained here.
    "simple_avg_kd_oldseen": False,
    "simple_avg_kd_oldseen_warmup": False,
    "simple_avg_dense_orth": True,
    # FINAL 8-ARM SCRIPT: same reasoning as simple_avg_kd_oldseen* above --
    # the weight=1 combined arm is replaced by a new weight=0.5 base_method
    # key below.
    "simple_avg_dense_orth_kd_oldseen_warmup": False,
    # FINAL 8-ARM SCRIPT -- SA-2/SA-4's new weight=0.5 base_method keys (see
    # their own add_method() calls below, which set kd_weight_scale=0.5 and
    # kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM1 i.e. no warmup).
    "simple_avg_kd_oldseen_weight0p5": True,
    "simple_avg_dense_orth_kd_oldseen_weight0p5": True,
    ...
    # FINAL 8-ARM SCRIPT: job 4969059's weight=1 old-seen-KD RankExt arm is
    # REPLACED by a new, distinct base_method key at weight=1.5 (added
    # further below) -- flipped False so the OLD (weight=1) arm is never
    # retrained here.
    "rank_extension_kd_oldseen_warmup": False,
    "rank_extension_dense_orth": False,
    "rank_extension_dense_orth_kd_oldseen_warmup": False,
    # FINAL 8-ARM SCRIPT -- RE-2/RE-3/RE-4's new base_method keys (see their
    # own add_method() calls below).
    "rank_extension_kd_oldseen_weight1p5_warmup": True,
    "rank_extension_factor_orth_lam50_new": True,
    "rank_extension_factor_orth_kd_oldseen_weight1p5_warmup": True,
...
def build_active_method_configs():
    ...
    # ==========================================================================
    # FINAL 8-ARM SCRIPT (SA-2): old-seen KD, T=2, weight=0.5 (kd_weight_scale
    # multiplies the module-global KD_WEIGHT=1.0 constant -- see add_method()'s
    # own "kd_weight" field formula above), kd_warmup_epochs=ARM1's value
    # (0.0) -- i.e. NO KD-specific warmup, per the design audit's Part E.6
    # recommendation (flat weight matched to warmup's observed average
    # effective dose, tested WITHOUT warmup to isolate the dose vs. shape
    # question). masked_kd_loss()/kd_class_scope="old_seen" mechanism itself
    # is completely unchanged from job 4969059.
    add_method(
        "simple_avg_kd_oldseen_T2_weight0p5", "simple_avg", "simple_avg_kd_oldseen_weight0p5",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
        kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM1, kd_weight_scale=0.5,
    )
    # FINAL 8-ARM SCRIPT (SA-3): current DenseOrth, lambda=20, UNCHANGED --
    # same method_name/base_method/config as job 4969059's arm (its own
    # 1-epoch dense_orth_warmup_epochs ramp, see DENSE_ORTH_WARMUP_EPOCHS and
    # IndependentLoraOrthTrainer, is preserved automatically since nothing
    # about this call or that trainer class changed).
    add_method(
        "simple_avg_dense_orth_lam20", "simple_avg", "simple_avg_dense_orth",
        uses_dense_orth=True,
    )
    # FINAL 8-ARM SCRIPT (SA-4): current DenseOrth lambda=20 + old-seen KD
    # weight=0.5, no warmup -- same reasoning as SA-2 above.
    add_method(
        "simple_avg_dense_orth_lam20_kd_oldseen_T2_weight0p5", "simple_avg", "simple_avg_dense_orth_kd_oldseen_weight0p5",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
        kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM1, kd_weight_scale=0.5, uses_dense_orth=True,
    )
    # ==========================================================================
    ...
    # ==========================================================================
    # FINAL 8-ARM SCRIPT (RE-2): old-seen KD, T=2, weight=1.5 (kd_weight_scale
    # multiplies KD_WEIGHT=1.0), 1-epoch warmup (KD_OLDSEEN_WARMUP_EPOCHS_
    # ARM2_ARM4, unchanged convention) -- per the design audit's Part F.8
    # recommendation (RankExt's measured KD/CE ratio-of-means at weight=1 is
    # ~0.22, well below SimpleAvg's, so there is quantified headroom to raise
    # the weight). masked_kd_loss()/kd_class_scope="old_seen" mechanism and
    # teacher construction (frozen previous-step RankExt model) are completely
    # unchanged from job 4969059.
    add_method(
        "rank_extension_kd_oldseen_T2_weight1p5_warmup", "rank_extension", "rank_extension_kd_oldseen_weight1p5_warmup",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
        kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM2_ARM4, kd_weight_scale=1.5,
    )
    # FINAL 8-ARM SCRIPT (RE-3): HISTORICAL FactorOrth restored via
    # uses_factor_orth=True -- resolves to orth_mode="factor_orth" in the
    # execution loop below (unchanged dispatch logic) and
    # lambda_orth=LAMBDA_ORTH(50.0)*lambda_orth_scale(1.0, default)=50.0 in
    # add_method()'s own formula above -- i.e. EXACTLY Exp1/R7's historical
    # RankExt FactorOrth arm (see the pre-existing, currently-disabled
    # "rank_extension_orth_factor_lam_50" add_method() call further below in
    # this same function for the byte-identical historical call signature
    # this mirrors). Given a NEW method_name/base_method (not the legacy
    # "rank_extension_orth_factor_lam_50" identifier) so this run's arm never
    # matches RANKEXT_PROJECTED_PROTECT_METHODS or
    # RANKEXT_NEW_BLOCK_WARMUP_DISABLED_METHODS -- protect_weight resolves to
    # 0.0 and new-block-warmup stays at its default 1-epoch value, exactly
    # like every other non-legacy RankExt arm in this script. NO KD here.
    add_method(
        "rank_extension_factor_orth_lam50", "rank_extension", "rank_extension_factor_orth_lam50_new",
        uses_factor_orth=True,
    )
    # FINAL 8-ARM SCRIPT (RE-4): FactorOrth lambda=50 + old-seen KD weight=1.5
    # + 1-epoch warmup -- RE-2 + RE-3 combined. Deliberately NOT passing
    # lambda_orth_scale/kd_weight_scale tied to COMBINED_LOSS_SCALE_ENABLED
    # (that historical rescaling mechanism, documented above
    # COMBINED_LAMBDA_ORTH_SCALE, exists to avoid a full-strength
    # kd=1.0/lambda=50 training collapse seen historically for the analogous
    # SimpleAvg/RankExt combined arms) -- the user's brief specifies EXACT
    # values (lambda=50, weight=1.5) for this arm, so no automatic
    # down-scaling is applied here; the historical collapse precedent is
    # instead flagged as an explicit risk in the implementation audit report
    # (Section 13, "unresolved risks") rather than silently mitigated.
    add_method(
        "rank_extension_factor_orth_lam50_kd_oldseen_T2_weight1p5_warmup", "rank_extension",
        "rank_extension_factor_orth_kd_oldseen_weight1p5_warmup",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
        kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM2_ARM4, kd_weight_scale=1.5,
        uses_factor_orth=True,
    )
...
EXPECTED_ENABLED_METHOD_FAMILIES = {
    "simple_avg",
    "simple_avg_kd_oldseen_weight0p5",
    "simple_avg_dense_orth",
    "simple_avg_dense_orth_kd_oldseen_weight0p5",
    "rank_extension",
    "rank_extension_kd_oldseen_weight1p5_warmup",
...
_FINAL8_SA_KD_ARMS = ("simple_avg_kd_oldseen_T2_weight0p5", "simple_avg_dense_orth_lam20_kd_oldseen_T2_weight0p5")
_FINAL8_RE_KD_ARMS = (
    "rank_extension_kd_oldseen_T2_weight1p5_warmup",
    "rank_extension_factor_orth_lam50_kd_oldseen_T2_weight1p5_warmup",
)
for _kd_arm in _FINAL8_SA_KD_ARMS + _FINAL8_RE_KD_ARMS:
    assert ACTIVE_METHOD_MAP[_kd_arm]["uses_kd"] is True, f"{_kd_arm} must have uses_kd=True"
    assert float(ACTIVE_METHOD_MAP[_kd_arm]["kd_temperature"]) == 2.0, f"{_kd_arm} must use T=2.0"
    assert ACTIVE_METHOD_MAP[_kd_arm]["kd_class_scope"] == "old_seen", f"{_kd_arm} must use old-seen-only KD"
# --- design-audit-specific weight/warmup checks (Part 17 static audit) -----
for _sa_kd_arm in _FINAL8_SA_KD_ARMS:
    assert abs(float(ACTIVE_METHOD_MAP[_sa_kd_arm]["kd_weight"]) - 0.5) < 1e-9, (
        f"{_sa_kd_arm} must resolve to kd_weight=0.5, got {ACTIVE_METHOD_MAP[_sa_kd_arm]['kd_weight']}"
    )
    assert float(ACTIVE_METHOD_MAP[_sa_kd_arm]["kd_warmup_epochs"]) == 0.0, (
        f"{_sa_kd_arm} must have NO KD-weight warmup (SimpleAvg is flat-weight by design, per the lambda/KD audit)"
    )
for _re_kd_arm in _FINAL8_RE_KD_ARMS:
    assert abs(float(ACTIVE_METHOD_MAP[_re_kd_arm]["kd_weight"]) - 1.5) < 1e-9, (
        f"{_re_kd_arm} must resolve to kd_weight=1.5, got {ACTIVE_METHOD_MAP[_re_kd_arm]['kd_weight']}"
    )
    assert float(ACTIVE_METHOD_MAP[_re_kd_arm]["kd_warmup_epochs"]) == 1.0, (
        f"{_re_kd_arm} must have a 1-epoch KD-weight warmup"
    )
# "T-squared enabled" is a structural property of IndependentLoraOrthTrainer/
# DeltaOrthRankExtensionTrainer's compute_loss (`kd_loss = F.kl_div(...) *
# (self.kd_temperature ** 2)`, unconditional) -- nothing to override here
# beyond confirming this file never modifies those trainer classes' KD-loss
# formula itself (it does not -- only their logged-row dict gained new,
# purely additive fields; see the trainer class definitions below).

# --- FactorOrth (RankExt, restored) + DenseOrth (SimpleAvg, unchanged) -----
...
_FINAL8_SA_DENSE_ORTH_ARMS = ("simple_avg_dense_orth_lam20", "simple_avg_dense_orth_lam20_kd_oldseen_T2_weight0p5")
_FINAL8_RE_FACTOR_ORTH_ARMS = (
    "rank_extension_factor_orth_lam50", "rank_extension_factor_orth_lam50_kd_oldseen_T2_weight1p5_warmup",
)
for _dense_orth_arm in _FINAL8_SA_DENSE_ORTH_ARMS:
...
for _factor_orth_arm in _FINAL8_RE_FACTOR_ORTH_ARMS:
    ...
    # FactorOrth-porting audit requirement: the new RankExt FactorOrth arms
    # must NEVER match the legacy protect_weight/new-block-warmup-disabled
    # method-name sets -- only FactorOrth's own geometry is restored, not the
    # historical "OLD-CLASS SEMANTIC-SUBSPACE PROJECTED FEATURE
    # CONSOLIDATION" (protect_weight) mechanism it happened to be bundled
    # with historically.
    assert _factor_orth_arm not in RANKEXT_PROJECTED_PROTECT_METHODS, (
        f"{_factor_orth_arm} must not collide with the legacy protect_weight method set "
        f"(RANKEXT_PROJECTED_PROTECT_METHODS) -- this would silently reintroduce unrelated "
        f"historical Exp1 behavior"
    )
...
_EXPECTED_SINGLEJOB_METHOD_NAMES = {
    "simple_avg",
    "simple_avg_kd_oldseen_T2_weight0p5",
    "simple_avg_dense_orth_lam20",
    "simple_avg_dense_orth_lam20_kd_oldseen_T2_weight0p5",
    "rank_extension",
    "rank_extension_kd_oldseen_T2_weight1p5_warmup",
    "rank_extension_factor_orth_lam50",
    "rank_extension_factor_orth_lam50_kd_oldseen_T2_weight1p5_warmup",
}
assert set(ACTIVE_METHOD_NAMES) == _EXPECTED_SINGLEJOB_METHOD_NAMES, (
    "This experiment must run exactly these 8 arms (4 SimpleAvg + 4 RankExt) -- mismatch: "
    f"{set(ACTIVE_METHOD_NAMES)} != {_EXPECTED_SINGLEJOB_METHOD_NAMES}"
)
...
# JOINT PACK: NUM_EPOCHS==6 for every active arm, both families -- the one
# intentional protocol change in this script (see LORA_EPOCHS/RANKEXT_EPOCHS'
# own comment above).
assert FAST_RUN or (LORA_EPOCHS == 6 and RANKEXT_EPOCHS == 6), (
    f"Both families must use 6 epochs in a REAL (non-FAST_RUN) run, got LORA_EPOCHS={LORA_EPOCHS}, "
    f"RANKEXT_EPOCHS={RANKEXT_EPOCHS}"
)
...
# FINAL 8-ARM SCRIPT: SimpleAvg is arithmetic-merge-ONLY here (DO-Merge
# removed entirely, per the task brief) -- 1 merge per SA arm, not 2.
_expected_final_configs = len(_sa_arms) * 1 + len(_re_arms) * 1
print("=" * 80)
print("FINAL 8-ARM FAMILY-SPECIFIC EXPERIMENT -- READINESS VERIFICATION")
print("=" * 80)
print(f"SCRIPT UNDER TEST: experiments_prepared/final_8arm_family_specific_5x20.py")
...
# ==============================================================================
# FINAL 8-ARM SCRIPT -- NEW REPORTING SECTION (replaces the inherited
# SUPERVISOR_SELECTED / ~30-plot downstream tail from the current pipeline,
# which was hardcoded to job 4969059's 9-arm-name set and would not run
# correctly against this script's 8 different arm names). Everything below
# reads ONLY from in-memory accumulators already populated, UNCHANGED, by the
# training/evaluation loops above (training_merge_summary_rows,
...
print("=" * 80)
print("FINAL 8-ARM SCRIPT -- REPORTING SECTION START")
print("=" * 80)

EXPECTED_8_METHODS = sorted(_EXPECTED_SINGLEJOB_METHOD_NAMES)
print(f"Expected 8 methods: {EXPECTED_8_METHODS}")

# ------------------------------------------------------------------------
# 1) Granular per-batch loss log for ALL 8 arms (Part 9 fix).
# ------------------------------------------------------------------------
final8_batch_log_df = pd.DataFrame(orth_kd_train_rows)
final8_batch_log_path = os.path.join(LOGS_DIR, "final_8arm_training_loss_history_by_batch.csv")
...
if len(final8_batch_log_df) > 0 and {"method", "step", "epoch"}.issubset(final8_batch_log_df.columns):
    ...
    final8_epoch_agg_path = os.path.join(TABLES_DIR, "final_8arm_loss_by_method_step_epoch.csv")
...
final8_epoch_loss_path = os.path.join(TABLES_DIR, "final_8arm_training_loss_history_by_epoch.csv")
...
final8_best_epoch_path = os.path.join(TABLES_DIR, "final_8arm_best_epoch_selection_by_method_step.csv")
...
final8_new_block_warmup_path = os.path.join(TABLES_DIR, "final_8arm_rankext_new_block_warmup_diagnostics.csv")
...
# ------------------------------------------------------------------------
# 3) Per-step accuracy (open + restricted), all 8 arms (Part 12).
# ------------------------------------------------------------------------
final8_per_step_open_df = pd.DataFrame(per_step_accuracy_rows)
final8_per_step_open_path = os.path.join(TABLES_DIR, "final_8arm_per_step_accuracy_open.csv")
...
final8_per_step_restricted_path = os.path.join(TABLES_DIR, "final_8arm_per_step_accuracy_open_vs_restricted.csv")
...
# ------------------------------------------------------------------------
# 4) Classifier protected-row restoration diagnostics (Part 11).
# ------------------------------------------------------------------------
final8_classifier_restoration_df = pd.DataFrame(classifier_restoration_rows)
final8_classifier_restoration_path = os.path.join(TABLES_DIR, "final_8arm_classifier_restoration_diagnostics.csv")
...
if len(final8_classifier_restoration_df) > 0:
    ...
    assert _max_w < 1e-6 and _max_b < 1e-6, (
        "STOP CONDITION (Part 21): classifier protected-row restoration failed for at least one "
        f"RankExt arm/step -- max weight diff={_max_w}, max bias diff={_max_b} (expected ~0.0). "
        "This experiment must not be declared ready to run if this fails."
    )

# ------------------------------------------------------------------------
# 5) CL retention/plasticity trajectory (Part 10) -- RankExt only; SimpleAvg
#    has no persistent inter-step model to re-evaluate (5 independently
#    trained specialists, merged exactly once at the very end), so a
#    step-by-step trajectory is structurally undefined for it -- NOT
#    fabricated here, per the task brief's explicit instruction.
# ------------------------------------------------------------------------
final8_cl_trajectory_df = pd.DataFrame(cl_trajectory_rows)
final8_cl_trajectory_path = os.path.join(TABLES_DIR, "final_8arm_rankext_cl_retention_plasticity_trajectory.csv")
...
print(
    "[reporting] NOTE: no equivalent trajectory table is produced for SimpleAvg -- its 4 arms "
    ...
    "final_8arm_training_loss_history_by_epoch.csv (each specialist's own train/val CE at "
...
# ------------------------------------------------------------------------
# 6) FINAL SUMMARY TABLE -- all 8 arms (Part 15).
# ------------------------------------------------------------------------
...
assert set(final8_training_merge_df["training_method"].unique()) == set(EXPECTED_8_METHODS), (
    "STOP CONDITION (Part 21): the result table is missing at least one of the 8 expected arms -- "
...
# for best-epoch selection -- accuracy is only ever computed on the separate
# eval/test-style datasets via evaluate_model()/evaluate_per_step_accuracy(),
# never on the held-out validation split itself) -- left as NaN rather than
# fabricated, and called out explicitly in the implementation audit report.
...
for _method in EXPECTED_8_METHODS:
    ...
    _summary_rows.append({
        ...
        "all_seen": float(_row["all_seen"]),
        "restricted_mean": float(_row["restricted_mean"]) if not pd.isna(_row["restricted_mean"]) else np.nan,
        "first_step": float(_row["first_step"]),
        "later_steps_mean": float(_row["later_steps"]),
        "old_new_gap": float(_row["old_new_gap"]),
        ...
        "validation_accuracy": np.nan,  # not tracked by this pipeline -- see note above
    })
final8_summary_df = pd.DataFrame(_summary_rows)
final8_summary_path = os.path.join(TABLES_DIR, "final_8arm_summary_table.csv")
final8_summary_df.to_csv(final8_summary_path, index=False)
print(f"[reporting] wrote final 8-arm summary table -> {final8_summary_path}")
print(final8_summary_df.to_string(index=False))

# ------------------------------------------------------------------------
# 7) AUTOMATIC WITHIN-RUN COMPARISONS (Part 16).
# ------------------------------------------------------------------------
...
_sa_kd = "simple_avg_kd_oldseen_T2_weight0p5"
_sa_orth = "simple_avg_dense_orth_lam20"
_sa_combined = "simple_avg_dense_orth_lam20_kd_oldseen_T2_weight0p5"
_re_plain = "rank_extension"
_re_kd = "rank_extension_kd_oldseen_T2_weight1p5_warmup"
_re_orth = "rank_extension_factor_orth_lam50"
_re_combined = "rank_extension_factor_orth_lam50_kd_oldseen_T2_weight1p5_warmup"

_within_run_deltas = [
    {"comparison": "SA KD0.5 - SA", "delta_all_seen_pp": _all_seen(_sa_kd) - _all_seen(_sa_plain)},
    {"comparison": "SA DenseOrth20 - SA", "delta_all_seen_pp": _all_seen(_sa_orth) - _all_seen(_sa_plain)},
    {"comparison": "SA DenseOrth20+KD0.5 - SA", "delta_all_seen_pp": _all_seen(_sa_combined) - _all_seen(_sa_plain)},
    {"comparison": "SA DenseOrth20+KD0.5 - SA KD0.5", "delta_all_seen_pp": _all_seen(_sa_combined) - _all_seen(_sa_kd)},
    {"comparison": "RE KD1.5 - RE", "delta_all_seen_pp": _all_seen(_re_kd) - _all_seen(_re_plain)},
    {"comparison": "RE FactorOrth50 - RE", "delta_all_seen_pp": _all_seen(_re_orth) - _all_seen(_re_plain)},
    {"comparison": "RE FactorOrth50+KD1.5 - RE", "delta_all_seen_pp": _all_seen(_re_combined) - _all_seen(_re_plain)},
    {"comparison": "RE FactorOrth50+KD1.5 - RE KD1.5", "delta_all_seen_pp": _all_seen(_re_combined) - _all_seen(_re_kd)},
]
final8_within_run_df = pd.DataFrame(_within_run_deltas)
final8_within_run_path = os.path.join(TABLES_DIR, "final_8arm_automatic_within_run_comparisons.csv")
...
# ------------------------------------------------------------------------
# 8) CROSS-RUN COMPARISON vs. job 4969059 (current pipeline) and historical
#    R7/Exp1 -- CLEARLY LABELED AS CONFOUNDED, never treated as causal.
# ------------------------------------------------------------------------
_cross_run_rows = [
    {
        "method": _sa_plain, "this_run_all_seen": _all_seen(_sa_plain),
        "job4969059_all_seen": 75.98, "r7_exp1_all_seen": 75.16,
        "confound_note": "epochs identical (6ep vs 6ep for job4969059); R7 comparison confounded by 9-vs-6 epochs",
    },
    {
        "method": f"{_sa_kd} (nearest: job4969059 SA KD weight1 warmup / R7 full-KD)",
        "this_run_all_seen": _all_seen(_sa_kd),
        "job4969059_all_seen": 76.66, "r7_exp1_all_seen": 62.98,
        "confound_note": "KD weight differs by design (0.5 vs job4969059 1.0, no warmup vs warmup) -- not an isolated ablation vs either reference; R7 additionally used full-100-way KD",
    },
    {
        "method": _sa_orth, "this_run_all_seen": _all_seen(_sa_orth),
        "job4969059_all_seen": 75.11, "r7_exp1_all_seen": 74.29,
        "confound_note": "DenseOrth unchanged vs job4969059 (should replicate closely); R7 used FactorOrth, a different formulation",
    },
    {
        "method": f"{_sa_combined} (nearest: job4969059 combined / R7 FactorOrth+full-KD)",
        "this_run_all_seen": _all_seen(_sa_combined),
        "job4969059_all_seen": 76.19, "r7_exp1_all_seen": 69.40,
        "confound_note": "KD weight differs by design vs job4969059; R7 comparison additionally confounded by FactorOrth-vs-DenseOrth and full-vs-old-seen KD",
    },
    {
        "method": _re_plain, "this_run_all_seen": _all_seen(_re_plain),
        "job4969059_all_seen": 30.71, "r7_exp1_all_seen": 35.31,
        "confound_note": "epochs identical vs job4969059; R7 comparison confounded by 9-vs-6 epochs AND the old (non-restoring) classifier-row bug",
    },
    {
        "method": f"{_re_kd} (nearest: job4969059 RE KD weight1 warmup / R7 full-KD)",
        "this_run_all_seen": _all_seen(_re_kd),
        "job4969059_all_seen": 47.24, "r7_exp1_all_seen": 64.44,
        "confound_note": "KD weight differs by design (1.5 vs job4969059 1.0); R7 comparison additionally confounded by 9-vs-6 epochs, old restoration bug, and full-vs-old-seen KD",
    },
    {
        "method": f"{_re_orth} (nearest: job4969059 RE DenseOrth20 / R7 FactorOrth50)",
        "this_run_all_seen": _all_seen(_re_orth),
        "job4969059_all_seen": 30.78, "r7_exp1_all_seen": 42.21,
        "confound_note": "orthogonality formulation differs by design vs job4969059 (FactorOrth vs DenseOrth, not a strength comparison); R7 comparison is FactorOrth-vs-FactorOrth but confounded by 9-vs-6 epochs and the old restoration bug",
    },
    {
        "method": f"{_re_combined} (nearest: job4969059 combined / R7 FactorOrth+full-KD)",
        "this_run_all_seen": _all_seen(_re_combined),
        "job4969059_all_seen": 46.75, "r7_exp1_all_seen": 70.72,
        "confound_note": "orth formulation AND KD weight both differ by design vs job4969059; R7 comparison additionally confounded by 9-vs-6 epochs, old restoration bug, and full-vs-old-seen KD",
    },
]
final8_cross_run_df = pd.DataFrame(_cross_run_rows)
final8_cross_run_df["delta_vs_job4969059_pp"] = final8_cross_run_df["this_run_all_seen"] - final8_cross_run_df["job4969059_all_seen"]
final8_cross_run_df["delta_vs_r7_exp1_pp"] = final8_cross_run_df["this_run_all_seen"] - final8_cross_run_df["r7_exp1_all_seen"]
final8_cross_run_path = os.path.join(TABLES_DIR, "final_8arm_cross_run_comparison_CONFOUNDED.csv")
...
# ------------------------------------------------------------------------
# 9) Output checklist (Part 21 stop-condition summary).
# ------------------------------------------------------------------------
_overall_pass = (
    set(final8_training_merge_df["training_method"].unique()) == set(EXPECTED_8_METHODS)
    and len(_methods_in_batch_log) >= 6  # at least the 6 non-plain arms must have granular logging
...
final8_checklist_path = os.path.join(REPORTS_DIR, "final_8arm_output_checklist.txt")
...
print("=" * 80)
print("FINAL 8-ARM SCRIPT -- REPORTING SECTION COMPLETE")
print("=" * 80)
