...
#
# ==============================================================================
# FINAL 8-METHOD CIFAR-100 5x20 REFINED EXPERIMENT.
#
# Derived by copying experiments_prepared/final_9method_5x20_performance_recovery.py
# (job 4971615's script) UNCHANGED at its own path, and applying exactly three
# further, precisely-documented refinements on top of it, all in this new,
# separate, standalone file:
#
#   (a) REMOVED the simple_avg_kd_oldseen_T4_warmup arm (the T=2-vs-T=4
#       old-seen-KD temperature ablation is closed; only T=2 is retained).
#       This experiment therefore trains exactly 8 methods, not 9.
#   (b) UNIFIED the head-classifier LR multiplier to a single shared
#       HEAD_LR_MULTIPLIER = 1.0 for BOTH families (was 10.0 for simple_avg,
#       1.0 for rank_extension) -- see R7/head_lr_final_decision.md for the
#       evidence audit behind this choice; there is no more
#       HEAD_LR_MULTIPLIER_BY_FAMILY asymmetry.
#   (c) ADDED a validation-fitted, per-task-group logit-scale calibration
#       stage for RankExt ONLY, applied after the existing
#       confidence_weighted_regime_grouped classifier-row-norm calibration
#       (which is kept, unchanged) -- see PerTaskScaleCalibration /
#       fit_rankext_task_scale_calibration() below. This is an internal
#       refinement of RankExt's own evaluation pipeline, not a new method: it
#       does not add a 9th column/row anywhere in the main results tables,
#       and RankExt method names are unchanged. SimpleAvg is not touched by
#       this stage.
#
# What follows below (until explicitly noted) is the ORIGINAL job 4971615
# script's own header, describing how THAT script was itself derived from
# final_8arm_family_specific_5x20.py
...
#
# FINAL 8 METHODS (this refined experiment; see EXPECTED_METHODS below for
# the enforced assertion) -- SA-3 (T=4) from the original 9-method script is
# REMOVED:
...
# All 4 RankExt arms additionally receive the new post-hoc logit-scale
# calibration stage (refinement (c) above) as part of their own final
# evaluation -- no new method/column is created for this.
#
# MERGE OPERATORS: SimpleAvg arithmetic-only. RankExt uses
# merge_method="persistent". TOTAL: 4 + 4 = 8 evaluated configurations.
#
...
# long).
RUN_NAME_BASE = f"cifar100_5x20_final_8method_refined_seed{SEED}"
...
# PROTOCOL LOCK -- hard assertions (task brief Section 1). CIFAR-100 5x20
# ONLY; this script must never silently run any other protocol depth/width.
assert NUM_STEPS == 5, f"This script is 5x20 ONLY; got NUM_STEPS={NUM_STEPS}"
assert CLASSES_PER_STEP == 20, f"This script is 5x20 ONLY; got CLASSES_PER_STEP={CLASSES_PER_STEP}"
...
# FINAL-8-METHOD REFINEMENT (see R7/head_lr_final_decision.md for the full
# evidence audit): classifier-head LR = LR_LORA/LR_RANKEXT times this
# multiplier, via HeadLRTrainerMixin.create_optimizer() below. Set to 1.0 to
# fully disable (falls back to the untouched stock Trainer.create_optimizer()).
#
# This experiment uses ONE SHARED value for both families -- the previous
# family-conditional asymmetry (simple_avg x10 / rank_extension x1, see the
# original 9-method script's own history) is removed. Evidence audit summary:
# rank_extension x10 was previously documented as a plausible amplifier of a
# transient step-boundary CE-spike instability, specifically worsened for
# FactorOrth configurations (analysis_rankext_firststep/report.txt); x1.0 is
# the setting that actually produced rank_extension's own historical best.
# simple_avg's x10 was never implicated in any instability, but no controlled
# x10-vs-x1 ablation for simple_avg exists anywhere in this repository -- its
# use of x10 is "not observed to be harmful," not "shown to be necessary."
# Given a documented failure mode on one side and no controlled evidence of
# benefit on the other, the safer, common value is chosen: x1.0 for BOTH
# families. This is a single explicit constant, not a per-family dict --
# family_head_lr_multiplier() below always returns this same value regardless
# of family, so there is no hidden family-specific override anywhere.
HEAD_LR_MULTIPLIER = 1.0


def family_head_lr_multiplier(family):
    """Classifier-head LR multiplier -- ONE shared value for every family in
    this experiment (see HEAD_LR_MULTIPLIER's own comment above and
    R7/head_lr_final_decision.md). The `family` argument is accepted only for
    call-site compatibility with the rest of the codebase's per-family
    dispatch pattern; it does not change the returned value."""
    return float(HEAD_LR_MULTIPLIER)
...
METHODS_TO_RUN = {
    ...
    # SA-3 (T=4 old-seen KD ablation) REMOVED for this 8-method refined
    # experiment -- no base_method flag registered for it.
...
def build_active_method_configs():
    ...
    # SA-3 (T=4 old-seen KD ablation) REMOVED for this 8-method refined
    # experiment -- FINAL-8-METHOD REFINEMENT: the T2-vs-T4 ablation is
    # closed; only the T=2 old-seen KD arm (SA-2 above) is retained.
...
# --- method set: exact 9-name match, not just the family-flag set ----------
# (task brief Section 6 example, reproduced verbatim as EXPECTED_METHODS)
EXPECTED_METHODS = {
    "simple_avg",
    "simple_avg_kd_oldseen_T2_warmup",
...
print("=" * 80)
print("FINAL 8-METHOD 5x20 REFINED EXPERIMENT -- READINESS VERIFICATION")
print("=" * 80)
print(f"SCRIPT UNDER TEST: experiments_prepared/final_8method_5x20_refined.py")
...
# FINAL 8-METHOD REFINED EXPERIMENT: this check verifies every KD
# base_method sweeps the SAME global KD_TEMPERATURES=[2.0] list (a leftover
# multi-temperature-sweep convention from Exp1). The T=4 one-off ablation
# (SA-3) that used to need a special-case exclusion here has been removed
# entirely from this script, so no exclusion set is needed any more -- every
# remaining KD arm (both families) is T=2.0 by construction.
...
assert "simple_avg_kd_oldseen_T4_warmup" not in ACTIVE_METHOD_MAP, (
    "T=4 ablation arm must not be present in this 8-method refined experiment"
)
...
print({
    ...
    "HEAD_LR_MULTIPLIER (shared, both families)": HEAD_LR_MULTIPLIER,
...
# PROTOCOL LOCK -- hard assertion (task brief Section 1): exact class splits.
assert class_splits == [
    list(range(0, 20)),
    list(range(20, 40)),
    list(range(40, 60)),
    list(range(60, 80)),
    list(range(80, 100)),
], f"class_splits must be the exact 5x20 CIFAR-100 partition, got {class_splits}"
...
print("Head LR multiplier (single shared value, both families, HEAD_LR_MULTIPLIER):", HEAD_LR_MULTIPLIER)
...
# =============================================================================
# FINAL-8-METHOD REFINEMENT: validation-fitted inter-task logit-scale
# calibration for RankExt (RankExt only -- never applied to simple_avg).
#
# This is a SEPARATE, ADDITIONAL post-hoc stage applied AFTER the existing
# calibrate_classifier_row_norms_confidence_weighted() row-norm calibration
# above (which is kept, unchanged, for every RankExt arm exactly as before).
# It does not touch that function or its call sites.
#
# Mechanism: z'[:, group_t] = s_t * z[:, group_t], s_t = exp(a_t) > 0, one
# positive scalar per fixed 20-class task group (classes 0-19, 20-39, ...,
# 80-99); the most-recent group (task 5) is fixed at s_5 = 1.0 as the
# identifiability reference, leaving exactly 4 free scalar parameters
# (a_1..a_4). This operates ONLY on the already-computed logit vector, at
# inference time -- it never touches the backbone, LoRA parameters,
# classifier.weight, or classifier.bias, and it never performs task-ID
# inference on individual samples: the class-group boundaries are the same
# fixed [0:20],[20:40],...,[80:100] ranges used everywhere else in this
# script, applied identically to every sample regardless of its own label.
#
# Fitting uses VALIDATION data ONLY (never test labels/accuracy for fitting,
# selection, or stopping), matches the already-validated design in
# R7/posthoc_calibration/evaluate_posthoc_task_calibration.py exactly (same
# equation, same optimizer recipe), but fits FRESH scales for THIS
# checkpoint's own validation logits every time this function is called --
# it must never reuse another job's previously-diagnosed scale values.
#
# Toggle: set RANKEXT_TASK_SCALE_CALIBRATION_ENABLED = False to fully disable
# this stage (finalize_rank_extension_arm() below falls back to exactly the
# pre-refinement behavior: only the existing row-norm calibration applies).
RANKEXT_TASK_SCALE_CALIBRATION_ENABLED = True
RANKEXT_TASK_SCALE_CALIBRATION_NUM_GROUPS = NUM_STEPS
RANKEXT_TASK_SCALE_CALIBRATION_FIXED_GROUP = NUM_STEPS - 1  # most-recent task = reference, scale fixed at 1.0

# Module-global diagnostics accumulator (auditability only -- NEVER read by
# the main summary-table code path, never becomes an extra column there).
rankext_task_scale_calibration_diagnostic_rows = []


class PerTaskScaleCalibration(nn.Module):
    """z_cal[:, group] = exp(a_group) * z[:, group] for every group except
    `fixed_group` (scale pinned at 1.0 -- softmax is scale-invariant up to a
    common factor, so exactly one group's scale must be fixed for
    identifiability). classes_per_group=20, num_groups=NUM_STEPS matches this
    script's fixed 5x20 class-group structure everywhere else. Only `self.a`
    (num_groups-1 scalars) are ever trainable; identity at init (a=0 =>
    s=1) is exact and verified by the smoke test in this script's static
    validation. Operates purely on logits -- holds no reference to, and never
    touches, any model parameter."""

    def __init__(self, num_groups=RANKEXT_TASK_SCALE_CALIBRATION_NUM_GROUPS,
                 classes_per_group=CLASSES_PER_STEP, fixed_group=RANKEXT_TASK_SCALE_CALIBRATION_FIXED_GROUP):
        super().__init__()
        self.num_groups = int(num_groups)
        self.classes_per_group = int(classes_per_group)
        self.fixed_group = int(fixed_group)
        self.free_groups = [g for g in range(self.num_groups) if g != self.fixed_group]
        self.a = nn.Parameter(torch.zeros(len(self.free_groups)))

    def scales(self):
        s = torch.ones(self.num_groups, dtype=self.a.dtype, device=self.a.device)
        for j, g in enumerate(self.free_groups):
            s[g] = torch.exp(self.a[j])
        return s

    def forward(self, logits):
        s = self.scales()
        cpg = self.classes_per_group
        out = logits.clone()
        for g in range(self.num_groups):
            out[:, g * cpg:(g + 1) * cpg] = logits[:, g * cpg:(g + 1) * cpg] * s[g]
        return out


def collect_rankext_logits_labels(model, hf_dataset, batch_size=BATCH_LORA):
    """Forward-only (no_grad) logit/label collection for calibration fitting
    and diagnostics. Never computes or applies gradients to `model` -- used
    both for the validation set (fitting) and, separately, for a frozen-
    scales diagnostic pass over the test set (measurement only, never fed
    back into the fit)."""
    loader = torch.utils.data.DataLoader(hf_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)
    was_training = model.training
    model.eval()
...
def fit_rankext_task_scale_calibration(val_logits, val_labels, lr=0.02, max_steps=500, patience=20, min_improve=1e-5):
    """Fits PerTaskScaleCalibration's 4 free parameters against validation
    cross-entropy ONLY -- val_logits/val_labels must come from a validation
    split (never the test set). Fixed, pre-registered optimizer recipe (no
    grid, no test-set-informed tuning): Adam, lr=0.02, <=500 steps, early
    stop after `patience` steps without a validation-CE improvement of at
    least `min_improve`. Only `module.a` receives gradients -- the logits
    passed in are plain detached tensors, not connected to any model graph,
    so no gradient can reach the backbone/LoRA/classifier even in principle.
    Returns (fitted module with requires_grad frozen off, history, initial_ce, final_ce)."""
    module = PerTaskScaleCalibration()
    ...
    initial_ce = float(F.cross_entropy(logits_t, labels_t).item())
    for step in range(max_steps):
        opt.zero_grad()
        cal_logits = module(logits_t)
        ce = F.cross_entropy(cal_logits, labels_t)
        ce.backward()
        opt.step()
        ce_val = float(ce.item())
        history.append({"step": step, "val_ce": ce_val})
        if ce_val < best_ce - min_improve:
            best_ce = ce_val
            steps_since_improve = 0
        else:
        ...
        if steps_since_improve >= patience:
            break
    final_ce = history[-1]["val_ce"] if history else initial_ce
    for p in module.parameters():
        p.requires_grad = False
    module.eval()
    return module, history, initial_ce, final_ce


class RankExtCalibratedModel(nn.Module):
    """Thin inference-only wrapper: forward() calls the already-fully-trained,
    already-row-norm-calibrated `base_model` unchanged, then applies the
    frozen `calibration` module to its logits before returning. Recomputes
    `.loss` (plain cross-entropy) from the CALIBRATED logits so this wrapper
    is a drop-in replacement anywhere a RankExt model is passed to
    evaluate_model()/evaluate_per_step_accuracy() -- the SAME method name's
    reported all_seen/first_step/later_steps/restricted numbers become the
    POST-calibration numbers, with no new method or table column created.
    base_model's parameters are frozen (requires_grad=False, enforced by the
    caller before wrapping) and this class adds no trainable parameters of
    its own (`calibration` is passed in already-fitted-and-frozen)."""

    def __init__(self, base_model, calibration_module):
        super().__init__()
        self.base_model = base_model
        self.calibration = calibration_module
        self.classifier = getattr(base_model, "classifier", None)  # passthrough for any diagnostic that reads it
        self.vision_model = getattr(base_model, "vision_model", None)
        self.config = getattr(base_model, "config", None)  # HF Trainer sometimes inspects model.config

    def forward(self, pixel_values=None, labels=None, **kwargs):
        out = self.base_model(pixel_values=pixel_values, **kwargs)
        calibrated_logits = self.calibration(out.logits)
        loss = None
        if labels is not None:
            loss = F.cross_entropy(calibrated_logits, labels)
        return ImageClassifierOutput(loss=loss, logits=calibrated_logits)


def apply_rankext_task_scale_calibration(model, method_name, eval_all_seen_dataset):
    """Orchestrates the full refinement stage for one RankExt arm's already-
    row-norm-calibrated `model`:
      1. freezes every model parameter (belt-and-braces -- the model is
         already eval()/no-grad in normal use, this makes the "no gradient
         may reach the backbone" rule explicit and checkable),
      2. fits fresh PerTaskScaleCalibration scales on THIS checkpoint's own
         validation logits (make_val_dataset(all_classes) -- never the test
         set, never another job's hard-coded scales),
      3. measures (diagnostic only, frozen scales, no further fitting) open/
         restricted accuracy before vs. after on `eval_all_seen_dataset` (the
         SAME test set evaluate_model() itself uses for the "all_seen"
         metric) purely to populate the auditability record and run the
         restricted-accuracy invariance check,
      4. returns a RankExtCalibratedModel wrapping `model` for the caller to
         use in place of the raw model for all further (official) evaluation.
    Never mutates `model`'s own parameters. Appends one diagnostic row to the
    module-global rankext_task_scale_calibration_diagnostic_rows list --
    never writes to the main summary-table accumulators.
    """
    for p in model.parameters():
        p.requires_grad = False

    val_ds = make_val_dataset(all_classes)
    val_logits, val_labels = collect_rankext_logits_labels(model, val_ds)
    calibration_module, history, val_ce_before, val_ce_after = fit_rankext_task_scale_calibration(val_logits, val_labels)
    fitted_scales = calibration_module.scales().detach().numpy().tolist()

    test_logits, test_labels = collect_rankext_logits_labels(model, eval_all_seen_dataset)
    test_groups = (test_labels // CLASSES_PER_STEP).astype(int)
    with torch.no_grad():
        test_logits_calibrated = calibration_module(torch.tensor(test_logits, dtype=torch.float32)).numpy()

    open_before = float((np.argmax(test_logits, axis=1) == test_labels).mean()) * 100.0
    open_after = float((np.argmax(test_logits_calibrated, axis=1) == test_labels).mean()) * 100.0

    restricted_before_per_task, restricted_after_per_task = {}, {}
    for g in range(NUM_STEPS):
        mask = test_groups == g
        if mask.sum() == 0:
            continue
        allowed = classes_for_step(g)
        restricted_before_per_task[g] = restricted_argmax_accuracy(test_logits[mask], test_labels[mask], allowed) * 100.0
        restricted_after_per_task[g] = restricted_argmax_accuracy(test_logits_calibrated[mask], test_labels[mask], allowed) * 100.0
    restricted_before_mean = float(np.mean(list(restricted_before_per_task.values()))) if restricted_before_per_task else float("nan")
    restricted_after_mean = float(np.mean(list(restricted_after_per_task.values()))) if restricted_after_per_task else float("nan")
    max_restricted_diff = max(
        (abs(restricted_before_per_task[g] - restricted_after_per_task[g]) for g in restricted_before_per_task),
        default=float("nan"),
    )
    invariance_pass = max_restricted_diff < 1e-6
    if not invariance_pass:
        print(
            f"[rankext_task_scale_calibration] WARNING -- restricted-accuracy invariance "
            f"VIOLATED for {method_name}: max |diff| = {max_restricted_diff:.6e} "
            f"(expected < 1e-6). Investigate before trusting this arm's calibrated result."
        )
    else:
        print(
            f"[rankext_task_scale_calibration] {method_name}: restricted-accuracy invariance "
            f"PASS (max |diff| = {max_restricted_diff:.3e})."
        )
    print(
        f"[rankext_task_scale_calibration] {method_name}: fitted scales (s1..s{NUM_STEPS}, "
        f"s{NUM_STEPS} fixed=1.0) = {[round(s, 4) for s in fitted_scales]} | "
        f"val CE {val_ce_before:.4f} -> {val_ce_after:.4f} | "
        f"test all_seen open {open_before:.3f}% -> {open_after:.3f}% | "
        f"restricted mean {restricted_before_mean:.3f}% -> {restricted_after_mean:.3f}%"
    )

    rankext_task_scale_calibration_diagnostic_rows.append({
        "method": method_name,
        "n_val": int(len(val_labels)),
        "n_test": int(len(test_labels)),
        "fitted_scales_s1_to_sN": json.dumps([round(s, 6) for s in fitted_scales]),
        "val_ce_before": val_ce_before,
        "val_ce_after": val_ce_after,
        "open_accuracy_before": open_before,
        "open_accuracy_after": open_after,
        "restricted_accuracy_before_mean": restricted_before_mean,
        "restricted_accuracy_after_mean": restricted_after_mean,
        "restricted_invariance_max_abs_diff": max_restricted_diff,
        "restricted_invariance_pass": bool(invariance_pass),
    })

    return RankExtCalibratedModel(model, calibration_module)
...
def finalize_rank_extension_arm(
    ...
    # FINAL-8-METHOD REFINEMENT: validation-fitted inter-task logit-scale
    # calibration, applied AFTER the existing row-norm calibration above (kept
    # unchanged) and AFTER the drift diagnostic (which reads model.classifier/
    # .vision_model directly and must see the pre-refinement model). This is
    # an internal refinement of RankExt's own evaluation pipeline -- the
    # method_name passed to evaluate_model() below is unchanged, so this
    # arm's reported all_seen/first_step/later_steps/restricted numbers
    # become the POST-calibration numbers under the SAME method name; no new
    # method or table column is created anywhere. Never applied to simple_avg
    # (that family's own calibration call sites are untouched by this
    # refinement). Disable via RANKEXT_TASK_SCALE_CALIBRATION_ENABLED.
    if RANKEXT_TASK_SCALE_CALIBRATION_ENABLED:
        final_rank_model = apply_rankext_task_scale_calibration(final_rank_model, method_name, eval_all_seen)
...
print("=" * 80)
print("FINAL 8-METHOD REFINED EXPERIMENT -- REPORTING SECTION START")
print("=" * 80)

EXPECTED_9_METHODS = sorted(EXPECTED_METHODS)  # name kept for minimal diff; holds the 8 methods of THIS script
print(f"Expected 8 methods: {EXPECTED_9_METHODS}")

# ------------------------------------------------------------------------
# 1) Granular per-batch loss log for ALL 9 arms.
# ------------------------------------------------------------------------
final9_batch_log_df = pd.DataFrame(orth_kd_train_rows)
final9_batch_log_path = os.path.join(LOGS_DIR, "final_8method_training_loss_history_by_batch.csv")
...
if len(final9_batch_log_df) > 0 and {"method", "step", "epoch"}.issubset(final9_batch_log_df.columns):
    ...
    final9_epoch_agg_path = os.path.join(TABLES_DIR, "final_8method_loss_by_method_step_epoch.csv")
...
final9_epoch_loss_path = os.path.join(TABLES_DIR, "final_8method_training_loss_history_by_epoch.csv")
...
final9_best_epoch_path = os.path.join(TABLES_DIR, "final_8method_best_epoch_selection_by_method_step.csv")
...
final9_new_block_warmup_path = os.path.join(TABLES_DIR, "final_8method_rankext_new_block_warmup_diagnostics.csv")
...
final9_per_step_open_path = os.path.join(TABLES_DIR, "final_8method_per_step_accuracy_open.csv")
...
final9_per_step_restricted_path = os.path.join(TABLES_DIR, "final_8method_per_step_accuracy_open_vs_restricted.csv")
...
final9_classifier_restoration_path = os.path.join(TABLES_DIR, "final_8method_classifier_restoration_diagnostics.csv")
...
final9_cl_trajectory_path = os.path.join(TABLES_DIR, "final_8method_rankext_cl_retention_plasticity_trajectory.csv")
...
final9_summary_path = os.path.join(TABLES_DIR, "final_8method_summary_table.csv")
...
# FINAL-8-METHOD REFINEMENT: automatic end-of-run results table/PNG
# generation (R7/final_8method_results_table.csv/.png, plus an all-seen bar
# chart). Reads values programmatically from final9_summary_df above -- never
# hand-typed. Deliberately FAILS LOUDLY (raises, aborting the reporting
# section) rather than silently emitting a partial/misleading table if any of
# the 8 expected arms is missing, duplicated, or if an unexpected extra row
# (e.g. a stray calibration-method row) is present -- see
# render_final_8method_results.validate_summary_before_render(). This does
# NOT add a calibration method/row/column anywhere: RankExt's all_seen value
# here is already the normal post-calibration final result computed above.
print("\n[reporting] generating final 8-method results table (CSV + PNG)...")
import sys as _sys
_this_script_dir = os.path.dirname(os.path.abspath(__file__))
if _this_script_dir not in _sys.path:
    _sys.path.insert(0, _this_script_dir)
from render_final_8method_results import render_final_8method_results as _render_final_8method_results

_final_results_csv, _final_results_png, _final_allseen_bar_png = _render_final_8method_results(
    final9_summary_df, output_dir="R7",
)
print(f"[reporting] wrote final results CSV -> {_final_results_csv}")
print(f"[reporting] wrote final results table PNG -> {_final_results_png}")
if _final_allseen_bar_png:
    print(f"[reporting] wrote final all-seen accuracy bar chart PNG -> {_final_allseen_bar_png}")
...
final9_within_run_path = os.path.join(TABLES_DIR, "final_8method_automatic_within_run_comparisons.csv")
...
final9_reference_path = os.path.join(TABLES_DIR, "final_8method_reference_values_not_retrained.csv")
...
final9_cross_run_path = os.path.join(TABLES_DIR, "final_8method_cross_run_comparison_CONFOUNDED.csv")
...
_checklist_lines = [
    ...
    f"8 expected methods present in results table: {set(final9_training_merge_df['training_method'].unique()) == set(EXPECTED_9_METHODS)}",
...
final9_checklist_path = os.path.join(REPORTS_DIR, "final_8method_output_checklist.txt")
...
print("=" * 80)
print("FINAL 8-METHOD REFINED EXPERIMENT -- REPORTING SECTION COMPLETE")
print("=" * 80)
