...
#
# ==============================================================================
# REFINED 8-METHOD CUB-200-2011 PRODUCTION EXPERIMENT (2026-09-23).
#
# This is a new production artifact derived from the completed
# job4971615-compatible ep5 CUB run. Intentional production changes are:
# (1) nine epochs for every method, (2) RankExt classifier/head LR multiplier
# x2 while SimpleAvg remains x10, and (3) aspect-ratio-preserving CLIP
# preprocessing with deterministic center-crop evaluation. The canonical ep5
# file remains unchanged.
#
# This file is a DEDICATED COPY of
# experiments_prepared/final_9method_imagenet100_5x20_performance_recovery.py
# (the existing 9-method ImageNet-100 script, UNCHANGED and left byte-for-byte
# as-is by this task -- not edited in place, per the task's explicit
# instruction). Exactly ONE functional change was made relative to that file:
# the SA-3 T4 ablation arm (`simple_avg_kd_oldseen_T4_warmup`) is disabled, so
# this script trains the SAME 8-method set as the canonical CIFAR-100
# production benchmark (job4971615, `experiments_prepared/
# final_9method_5x20_performance_recovery.py` with that same T4 flag left
# False) -- see "WHAT CHANGED" below for the exhaustive, line-level list.
# Every method definition, hyperparameter, training loop, loss, calibration,
# evaluation, and metric computation is REUSED UNCHANGED, transitively, from
# the canonical CIFAR-100 script -- this file is diff-able against both its
# immediate ImageNet parent and the CIFAR-100 script at any time to prove no
# algorithmic behavior drifted, either because the dataset changed or because
# the T4 arm was disabled.
#
# See R7/cub200_job4971615_compatibility_readiness.md for the full
# structural compatibility audit this file was produced for, including the
# per-mechanism hyperparameter cross-check (KD weight, DenseOrth/FactorOrth
# lambda, protect weight, head-LR multipliers, calibration mode) against
# live-read values from the canonical CIFAR-100 script, and the exhaustive
# DATASET-REQUIRED / IMPLEMENTATION-NECESSARY / SCIENTIFIC-DIFFERENCE /
# REPORTING-ONLY classification of every line that differs from job4971615's
# own source.
#
# WHAT CHANGED vs. final_9method_imagenet100_5x20_performance_recovery.py
# (exhaustive; everything else in this file, including the ImageNet dataset
# adaptation described below, is inherited unchanged from that file):
#   A. `METHODS_TO_RUN["simple_avg_kd_oldseen_T4_warmup"]`: True -> False.
#      This is the ONLY METHODS_TO_RUN flag touched -- the other 8 True flags
#      and every False flag are untouched.
#   B. `EXPECTED_METHODS` and `EXPECTED_ENABLED_METHOD_FAMILIES`: the T4
#      method name removed from both sets, matching change A.
#   C. Method-count assertions updated 9->8 total / 5->4 SimpleAvg arms (4
#      RankExt arms is unchanged) throughout the file, matching change A --
#      no assertion's PASS/FAIL logic changed in kind, only the literal
#      expected count.
#   D. `RUN_NAME_BASE`: now encodes "final_8method_job4971615_compatible"
#      instead of "final_9method_performance_recovery", so this run's
#      results/checkpoints directory can never collide with, or be mistaken
#      for, either the 9-method ImageNet sibling's output or any CIFAR-100
#      run's own output.
#   E. Purely-cosmetic "9-method"/"9 methods" diagnostic strings updated to
#      say "8-method"/"8 methods" so console/log output is accurate; this did
#      not touch any "9 epochs" (`LORA_EPOCHS`/`RANKEXT_EPOCHS`) text, which
#      is unrelated and unchanged (both families still train 9 epochs/step).
#
# ==============================================================================
# ORIGINAL IMAGENET-100 ADAPTATION HEADER BELOW (unchanged, preserved verbatim
# for provenance -- describes the 9-method ImageNet script's own lineage, not
# this file's -- the "9-method"/"9 methods" language below is historical and
# refers to that parent file, not to this one):
# ==============================================================================
#
# ==============================================================================
# IMAGENET-100 ADAPTATION of the FINAL 9-METHOD 5x20 PERFORMANCE-RECOVERY
# EXPERIMENT (2026-09-18).
#
# This file is a DEDICATED COPY of
# experiments_prepared/final_9method_5x20_performance_recovery.py (the
# CIFAR-100 script, UNCHANGED and left byte-for-byte as-is by this task --
# do not edit it to add ImageNet support). Only the dataset-loading /
# class-remapping cell and a small number of dataset-identity assertions and
# diagnostic strings were changed; every method definition, hyperparameter,
# training loop, loss, calibration, evaluation, and metric computation is
# REUSED UNCHANGED from the CIFAR-100 script -- this file is diff-able
# against it at any time to prove no algorithmic behavior drifted merely
# because the dataset changed.
#
# WHAT CHANGED (exhaustive list; everything else below this banner is
# identical to the CIFAR-100 script until its own dataset-loading cell):
#   1. RUN_NAME_BASE now encodes "imagenet100" instead of "cifar100", so
#      this run's results/checkpoints directories can never collide with,
#      or be mistaken for, any CIFAR-100 run's own output (Section 11 of
#      the task brief this file was written for).
#   2. New constants IMAGENET100_DATASET_ID / IMAGENET100_CACHE_ROOT /
#      IMAGENET100_CLASS_ORDER_ARTIFACT_PATH / FINAL_EVAL_SPLIT_NAME, added
#      immediately before the dataset-loading cell.
#   3. The dataset-loading cell itself: `load_dataset("cifar100")` ->
#      `load_dataset(IMAGENET100_DATASET_ID, cache_dir=IMAGENET100_CACHE_ROOT)`,
#      plus loading (or, on first run only, generating) the persistent
#      class-order/task-split artifact and remapping the dataset's own
#      label column into that fixed 0..99 benchmark-local ID space, plus
#      ImageNet-100-appropriate dataset-identity/sanity assertions in place
#      of the CIFAR-specific fine_label/coarse_label/50000/10000 checks.
#      `class_splits` itself (contiguous chunking of NUM_STEPS x
#      CLASSES_PER_STEP over whatever 0..99 label space it is given) is
#      NOT modified -- it now simply operates on the remapped ImageNet-100
#      label space instead of CIFAR-100's native one, exactly the same
#      mechanism, unchanged.
#   4. `make_eval_dataset()`'s `dataset["test"]` -> `dataset[FINAL_EVAL_SPLIT_NAME]`
#      (`"validation"` for this dataset, since the ImageNet-100 mirror used
#      here has no split literally named "test" -- see the readiness report).
#   5. Two purely-cosmetic diagnostic print strings (startup banner, dataset
#      label) updated so console/log output correctly says "imagenet100",
#      not "cifar100".
# See R7/imagenet100_5x20_static_readiness.md for the full readiness audit,
# including exactly what has and has NOT been verified in this pass, and
# thesis_agent/reports/imagenet100_generalization_preparation.md for the
# earlier (2026-09-08) dataset-selection investigation this file's dataset
# choice is grounded in.
# ==============================================================================

#
# ==============================================================================
# ORIGINAL CIFAR-100 SCRIPT HEADER BELOW (unchanged, preserved verbatim for
# provenance -- describes the CIFAR-100 script's own lineage, not this file's):
# ==============================================================================
...
#   7. Exactly 8 methods are active (4 SimpleAvg + 4 RankExt, Section 6 of
...
DEBUG_MODE = FAST_RUN
# Lightweight readiness mode: load the real dataset/processor/model and run
# one forward/backward for each family, but never enter an epoch or training
# loop. The normal cluster run leaves this disabled.
CUB200_PREFLIGHT_ONLY = os.environ.get("CUB200_PREFLIGHT_ONLY", "0") == "1"
...
# long).
RUN_NAME_BASE = f"cub200_5x40_final_8method_improved_rankext_seed{SEED}_ep9"
RUN_NAME = f"{RUN_NAME_BASE}_{'PREFLIGHT_ONLY' if CUB200_PREFLIGHT_ONLY else 'EPOCH9_MAIN'}"
...
NUM_CLASSES = 200
assert NUM_CLASSES == 200, f"This script is CUB-200 ONLY; got NUM_CLASSES={NUM_CLASSES}"
...
# PROTOCOL LOCK -- hard assertions. CUB-200 uses exactly 5 tasks of 40
# classes; all downstream loops and reporting consume these generic constants.
assert NUM_STEPS == 5, f"This script is CUB-200 5x40 ONLY; got NUM_STEPS={NUM_STEPS}"
assert CLASSES_PER_STEP == 40, f"This script is CUB-200 5x40 ONLY; got CLASSES_PER_STEP={CLASSES_PER_STEP}"
...
# PROTOCOL LOCK -- hard assertions (task brief Section 5). These are
# unconditional and intentionally have no FAST_RUN escape hatch.
assert LORA_EPOCHS == 9, f"LORA_EPOCHS must be 9 for this script, got {LORA_EPOCHS}"
assert RANKEXT_EPOCHS == 9, f"RANKEXT_EPOCHS must be 9 for this script, got {RANKEXT_EPOCHS}"
...
# TARGET_MODULES_BY_FAMILY. For this refined production run, SimpleAvg keeps
# x10 and all four RankExt variants intentionally use the validated x2 head-LR
# multiplier. This is the sole RankExt optimizer change relative to the ep5
# production baseline; the LoRA/base learning rates remain unchanged.
HEAD_LR_MULTIPLIER = 10.0

# RESTORED (2026-08-25, explicit user correction): simple_avg's head-LR
# multiplier remains x10, matching the canonical SimpleAvg configuration.
# RankExt is intentionally overridden independently below to x2 for this
# refined run; no SimpleAvg setting is changed by that override.
HEAD_LR_MULTIPLIER_BY_FAMILY = {
    "simple_avg": float(HEAD_LR_MULTIPLIER),
    "rank_extension": 2.0,
}
assert HEAD_LR_MULTIPLIER_BY_FAMILY["simple_avg"] == 10.0, "SimpleAvg head LR multiplier must remain x10"
assert HEAD_LR_MULTIPLIER_BY_FAMILY["rank_extension"] == 2.0, "RankExt head LR multiplier must be x2"
...
assert VALIDATION_PER_CLASS == 5, "The full CUB-200 5x40 run uses exactly 5 validation images per class"
LOSS_NA_FILL = 0.0
...
METHODS_TO_RUN = {
    ...
    "simple_avg_kd_oldseen_warmup": True,
    # SA-3 (task brief Section 10): NEW -- old-seen KD, T=4, otherwise
    # identical to SA-2. Never active in any prior run.
    # JOB4971615-COMPATIBLE 8-METHOD SCRIPT: flipped False here -- T4 is an
    # ablation-only arm, not one of job4971615's 8 canonical methods. This is
    # the ONE functional change this file makes relative to its 9-method
    # ImageNet parent (see this file's own top-of-file header).
    "simple_avg_kd_oldseen_T4_warmup": False,
    "simple_avg_dense_orth": True,
    # SA-5 (task brief Section 12): DenseOrth lambda=20 + old-seen KD, T=2,
    # weight=1.0, 1-epoch warmup. Re-enabled from its job-4970580-era False.
    "simple_avg_dense_orth_kd_oldseen_warmup": True,
    # FINAL 9-METHOD SCRIPT: job 4970580's weight=0.5 variants are not part
    # of the parent script's 9-method set -- disabled here.
    "simple_avg_kd_oldseen_weight0p5": False,
    "simple_avg_dense_orth_kd_oldseen_weight0p5": False,
    # JOINT PACK: RE-0 (canonical plain RankExt, same-run baseline).
    "rank_extension": True,
    ...
    # weight=1.5 RankExt arms are NOT part of this refined script's 8-method set --
    ...
    "rank_extension_factor_orth_normalized_lam50": True,
    "rank_extension_factor_orth_kd_oldseen_weight1p5_warmup": False,
    # RE-2 (task brief Section 15): HISTORICAL full-100-way KD, T=2,
    # weight=1.0, NO KD-weight warmup, protect_weight=30.0, new-block output
    # warmup DISABLED. NEW method, never active in any prior run.
    "rank_extension_fullkd_T2_protect30": True,
    # RE-4 (task brief Section 17): RE-2 + RE-3 combined -- the main final
    # RankExt candidate. NEW method, never active in any prior run.
    "rank_extension_factor_orth_normalized_lam50_fullkd_T2_protect30": True,
...
def build_active_method_configs():
    ...
    add_method(
        "rank_extension_factor_orth_normalized_lam50", "rank_extension", "rank_extension_factor_orth_normalized_lam50",
    ...
    add_method(
        "rank_extension_factor_orth_normalized_lam50_fullkd_T2_protect30", "rank_extension",
        "rank_extension_factor_orth_normalized_lam50_fullkd_T2_protect30",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="full",
        kd_warmup_epochs=0.0, kd_weight_scale=1.0,
        uses_factor_orth=True,
...
EXPECTED_ENABLED_METHOD_FAMILIES = {
    "simple_avg",
    "simple_avg_kd_oldseen_warmup",
    # JOB4971615-COMPATIBLE 8-METHOD SCRIPT: "simple_avg_kd_oldseen_T4_warmup"
    # removed -- matches METHODS_TO_RUN's own flag flipped to False above.
    "simple_avg_dense_orth",
    "simple_avg_dense_orth_kd_oldseen_warmup",
    "rank_extension",
    "rank_extension_fullkd_T2_protect30",
...
# exactly.
# CUB protocol adaptation: SimpleAvg retains its canonical fixed rank=80 and
# alpha=160, while RankExt grows by +16 per task to final rank=80. These are
# intentionally distinct family capacities; do not couple them by assertion.
assert LORA_R == 80
assert LORA_ALPHA == 160
...
# --- dataset / output isolation from the CIFAR-100 script ------------------
# (`dataset`/`LABEL_COL` do not exist yet at this point in module execution
# -- the dataset-load cell runs later, further down this file; the
# ImageNet-100-specific identity checks are asserted there instead, right
# after `LABEL_COL` is set, still well before any training starts -- see
# "DATASET IDENTITY CHECK" below.)
#
# IMAGENET-100 ADAPTATION: this script is the deliberate ImageNet-100
# counterpart of the CIFAR-100 script's own "no ImageNet configuration must
# be active" guard (which exists there to prevent an earlier, since-reverted
# ImageNet integration attempt from silently reactivating on the CIFAR
# script). Here the check runs in the OPPOSITE direction: RUN_NAME_BASE
# MUST mention imagenet100 (so this run's output can never collide with, or
# be mistaken for, a CIFAR-100 run's own results/checkpoints directory) and
# must NOT mention cifar100 (guards against an accidental copy-paste
# regression back toward the CIFAR script's own naming).
assert NUM_CLASSES == 200, f"This experiment requires 200 total classes, got {NUM_CLASSES}"
assert "cub200" in RUN_NAME_BASE.lower(), f"RUN_NAME_BASE must mention cub200: {RUN_NAME_BASE!r}"
assert "cifar100" not in RUN_NAME_BASE.lower(), f"RUN_NAME_BASE must not mention cifar100: {RUN_NAME_BASE!r}"

# --- protocol / seed ---------------------------------------------------------
assert SEED == 42, f"Experiment 1 requires seed=42, got {SEED}"
assert NUM_STEPS == 5 and CLASSES_PER_STEP == 40, (
    f"Experiment 1 is the 5x40 protocol; got NUM_STEPS={NUM_STEPS}, "
    f"CLASSES_PER_STEP={CLASSES_PER_STEP}"
)
assert NUM_STEPS * CLASSES_PER_STEP == NUM_CLASSES == 200
...
assert active_rankext_rank_schedule() == [16, 32, 48, 64, 80], (
    f"Experiment 1 requires the CUB 5-task RankExt cumulative schedule "
    f"[16,32,48,64,80], got {active_rankext_rank_schedule()}"
)
...
assert float(ACTIVE_METHOD_MAP["rank_extension_factor_orth_normalized_lam50_fullkd_T2_protect30"]["kd_temperature"]) == 2.0
...
_RE_FACTOR_ORTH_ARMS = ("rank_extension_factor_orth_normalized_lam50", "rank_extension_factor_orth_normalized_lam50_fullkd_T2_protect30")
...
assert "rank_extension_factor_orth_normalized_lam50" not in RANKEXT_PROJECTED_PROTECT_METHODS, (
    "rank_extension_factor_orth_normalized_lam50 (non-KD) must not activate protect_weight"
)
assert "rank_extension_factor_orth_normalized_lam50" not in RANKEXT_NEW_BLOCK_WARMUP_DISABLED_METHODS, (
    "rank_extension_factor_orth_normalized_lam50 (non-KD) must keep new-block warmup enabled"
)
...
assert set(active_methods) == EXPECTED_METHODS, (
    "This experiment must run exactly these 8 arms (4 SimpleAvg + 4 RankExt) -- mismatch: "
    f"{set(active_methods)} != {EXPECTED_METHODS}"
)
assert len(active_methods) == 8, f"Must be exactly 8 active methods, got {len(active_methods)}"
assert sum(1 for m in ACTIVE_METHOD_NAMES if ACTIVE_METHOD_MAP[m]["family"] == "simple_avg") == 4, (
    "Must be exactly 4 SimpleAvg training arms"
)
assert sum(1 for m in ACTIVE_METHOD_NAMES if ACTIVE_METHOD_MAP[m]["family"] == "rank_extension") == 4, (
    "Must be exactly 4 RankExt training arms"
)
...
# PROTOCOL LOCK -- both families use 9 epochs, unconditionally (task brief
# Section 5; also asserted immediately after each constant's own definition
# above -- repeated here as a final, post-method-registration confirmation).
assert LORA_EPOCHS == 9 and RANKEXT_EPOCHS == 9, (
    f"Both families must use 9 epochs, got LORA_EPOCHS={LORA_EPOCHS}, RANKEXT_EPOCHS={RANKEXT_EPOCHS}"
)
...
assert active_rankext_rank_schedule() == [16, 32, 48, 64, 80], (
    f"RankExt must use the CUB 5-task [16,32,48,64,80] schedule, "
    f"got {active_rankext_rank_schedule()}"
)
assert len(ACTIVE_METHOD_NAMES) == 8, f"Expected 8 total training arms, got {len(ACTIVE_METHOD_NAMES)}"
assert set(ENABLED_METHOD_FAMILIES) == EXPECTED_ENABLED_METHOD_FAMILIES

print("=" * 80)
print("EXPERIMENT 1 STARTUP DIAGNOSTICS -- CUB-200 5x40 evaluation under the current fair/")
print("canonical pipeline, corrected RankExt implementation, historical RankExt schedule")
print("=" * 80)
print(f"  dataset               = CUB-200-2011 (Donghyun99/CUB-200-2011) (label_col diagnostic deferred -- 'dataset'/'LABEL_COL'")
...
print("=" * 80)
print("FINAL 8-METHOD JOB4971615-COMPATIBLE CUB-200 5x40 EXPERIMENT -- READINESS VERIFICATION")
print("=" * 80)
print(f"SCRIPT UNDER TEST: experiments_prepared/final_8method_cub200_5x40_job4971615_ep5.py")
...
print("=" * 80)
assert LORA_EPOCHS == 9 and RANKEXT_EPOCHS == 9
assert len(_sa_arms) == 4 and len(_re_arms) == 4 and len(ACTIVE_METHOD_NAMES) == 8
assert _expected_final_configs == 8, f"Expected 8 final configurations, computed {_expected_final_configs}"
...
# JOB4971615-COMPATIBLE 8-METHOD SCRIPT: the T4 temperature re-check is
# removed here -- "simple_avg_kd_oldseen_T4_warmup" is disabled (METHODS_TO_RUN)
# and therefore absent from ACTIVE_METHOD_MAP; asserting its T=4.0 config
# would KeyError. _T4_ABLATION_BASE_METHODS above still harmlessly excludes
# it by name from the sweep-consistency loop (a no-op now, since no active
# config's base_method is ever a member of that set).
assert "simple_avg_kd_oldseen_T4_warmup" not in ACTIVE_METHOD_MAP, (
    "T4 arm must be fully disabled in this 8-method script"
)
...
# ============================================================================
# CUB-200-2011 DATASET CONFIGURATION (dataset/protocol adaptation only).
# ============================================================================
...
# train=126689 / validation=5000 examples; features = an "image" column plus
# a 200-way ClassLabel "label" column) -- see
...
# built from. This confirms the identifier is real and load_dataset()-able
...
CUB_DATASET_ID = "Donghyun99/CUB-200-2011"
CUB_DATASET_REVISION = "main"

# Configurable Hugging Face `datasets` download-cache root. Overridable via
# the CUB200_CACHE_ROOT environment variable; defaults to a
# cluster-appropriate path, never a Windows path. If the dataset is already
# cached there from a prior run, `load_dataset` reuses it automatically (no
# re-download).
CUB200_CACHE_ROOT = os.environ.get(
    "CUB200_CACHE_ROOT",
    "/nfsd/lttm4/tesisti/shahrampour/datasets/huggingface",
)

# Persistent, checked-in class-order/task-split artifact (task brief Section
# 5): generated ONCE (already present in the repository -- see the
# readiness report) and only ever LOADED, never regenerated, by this script
...
CUB200_CLASS_ORDER_ARTIFACT_PATH = os.path.join(
    "experiments_prepared", "splits", "cub200_class_order_seed42_5x40.json"
)

# CUB supplies an official test split. Validation is carved from train only.
FINAL_EVAL_SPLIT_NAME = "test"

dataset = load_dataset(CUB_DATASET_ID, revision=CUB_DATASET_REVISION, cache_dir=CUB200_CACHE_ROOT)
...
IMAGE_COL = "image" if "image" in dataset["train"].column_names else "img"

# Stable split-local IDs are retained only for the exported offline-calibration
# artifacts. They do not enter any model input or loss and therefore cannot
# alter the refined training/evaluation semantics.
dataset["train"] = dataset["train"].add_column(
    "__sample_id__", list(range(dataset["train"].num_rows))
)
dataset[FINAL_EVAL_SPLIT_NAME] = dataset[FINAL_EVAL_SPLIT_NAME].add_column(
    "__sample_id__", list(range(dataset[FINAL_EVAL_SPLIT_NAME].num_rows))
)

# DATASET IDENTITY CHECK (ImageNet-100 equivalent of the CIFAR-100 script's
# own fine_label/coarse_label/50000/10000 check -- adapted, not removed).
# Still runs well before any training starts.
assert "train" in dataset and FINAL_EVAL_SPLIT_NAME in dataset, (
    f"Expected 'train' and {FINAL_EVAL_SPLIT_NAME!r} splits from {CUB_DATASET_ID!r}, "
    f"got {list(dataset.keys())}"
)
assert LABEL_COL in dataset["train"].column_names and IMAGE_COL in dataset["train"].column_names, (
    f"Loaded dataset does not expose the expected label/image columns -- "
    f"got columns: {dataset['train'].column_names}"
)
assert dataset["train"].num_rows == 5994, f"Official CUB train split must have 5994 rows, got {dataset['train'].num_rows}"
assert dataset[FINAL_EVAL_SPLIT_NAME].num_rows == 5794, f"Official CUB test split must have 5794 rows, got {dataset[FINAL_EVAL_SPLIT_NAME].num_rows}"
print(f"CUB-200 dataset identity check PASSED: {CUB_DATASET_ID}, "
      f"train={dataset['train'].num_rows}, {FINAL_EVAL_SPLIT_NAME}={dataset[FINAL_EVAL_SPLIT_NAME].num_rows}, "
      f"label_col={LABEL_COL!r}, image_col={IMAGE_COL!r}")

# --- Fixed class order / 5x40 task split --------------------------------
# Load the persistent class-order artifact. It must already exist -- this
# script never generates or overwrites it; see
# CUB200_CLASS_ORDER_ARTIFACT_PATH above and the readiness report for
# how it was produced.
assert os.path.isfile(CUB200_CLASS_ORDER_ARTIFACT_PATH), (
    f"Missing persistent class-order artifact: {CUB200_CLASS_ORDER_ARTIFACT_PATH!r}. "
    "It must be committed and is never regenerated automatically at training time."
)
with open(CUB200_CLASS_ORDER_ARTIFACT_PATH, "r", encoding="utf-8") as _f:
    CUB200_CLASS_ORDER = json.load(_f)

assert CUB200_CLASS_ORDER["dataset_identifier"] == CUB_DATASET_ID, (
    f"Class-order artifact was generated for {CUB200_CLASS_ORDER['dataset_identifier']!r}, "
    f"not the dataset this script loads ({CUB_DATASET_ID!r})"
)
assert CUB200_CLASS_ORDER.get("dataset_revision_or_config", "main").startswith("main"), (
    "Class-order artifact must record the CUB dataset main/default revision/config"
)
assert CUB200_CLASS_ORDER["seed"] == SEED, (
    f"Class-order artifact was generated for seed {CUB200_CLASS_ORDER['seed']}, got SEED={SEED}"
)
assert CUB200_CLASS_ORDER["num_classes"] == NUM_CLASSES == 200
assert CUB200_CLASS_ORDER["num_tasks"] == NUM_STEPS == 5
assert CUB200_CLASS_ORDER["classes_per_task"] == CLASSES_PER_STEP == 40
...
# persistent 0..199 ID. This new ID -- NOT the original CUB label
# and NOT necessarily meaningful outside this benchmark -- is what the
# classifier and every downstream method actually train and evaluate on.
_ORIGINAL_ID_TO_NEW_ID = {int(k): int(v) for k, v in CUB200_CLASS_ORDER["original_id_to_new_id"].items()}
assert set(_ORIGINAL_ID_TO_NEW_ID.keys()) == set(range(NUM_CLASSES)), "class-order artifact must cover all 200 original IDs"
assert set(_ORIGINAL_ID_TO_NEW_ID.values()) == set(range(NUM_CLASSES)), "class-order artifact's new IDs must be exactly 0..199"

# Verify the LOADED dataset's own label set matches what the artifact was
# built from (200 distinct original IDs, 0..199) before remapping -- fails
# loudly, not silently, on a mismatch (e.g. a different dataset revision).
_observed_original_ids = set(int(v) for v in dataset["train"].unique(LABEL_COL)) | set(int(v) for v in dataset[FINAL_EVAL_SPLIT_NAME].unique(LABEL_COL))
assert _observed_original_ids == set(range(NUM_CLASSES)), (
    f"Loaded dataset's {LABEL_COL!r} column does not contain exactly the 200 original IDs "
    f"[0..199] the class-order artifact expects -- got {sorted(_observed_original_ids)}"
)
try:
    _feature_names = dataset["train"].features[LABEL_COL].names
    _artifact_names = CUB200_CLASS_ORDER["original_class_names_by_original_id"]
    _name_mismatches = [i for i in range(NUM_CLASSES) if _feature_names[i] != _artifact_names[str(i)]]
    if _name_mismatches:
        print(f"WARNING: {len(_name_mismatches)} class NAME(s) differ between the live dataset build "
              f"and the class-order artifact (IDs unaffected; this is a soft, non-fatal check) -- "
              f"first mismatch: id={_name_mismatches[0]}, live={_feature_names[_name_mismatches[0]]!r}, "
              f"artifact={_artifact_names[str(_name_mismatches[0])]!r}")
    else:
        print("Class-name cross-check against the live dataset build: PASSED (200/200 match).")
except (AttributeError, KeyError, TypeError) as _exc:
    print(f"NOTE: could not cross-check class names against the live dataset build ({_exc}); "
          f"the ID-level check above (which is the one that actually matters for correctness) still passed.")

def _remap_cub200_label(batch):
    batch[LABEL_COL] = [_ORIGINAL_ID_TO_NEW_ID[int(v)] for v in batch[LABEL_COL]]
    return batch

dataset = dataset.map(_remap_cub200_label, batched=True)
print(f"CUB-200 label remap applied: {LABEL_COL!r} now holds the fixed, seed-{SEED} "
      f"benchmark-local IDs [0..199] recorded in {CUB200_CLASS_ORDER_ARTIFACT_PATH!r}, "
      f"not the dataset builder's own indices.")

# PROTOCOL-DEPTH-GENERIC CONSTRUCTION (unchanged mechanism, reused verbatim
# from the CIFAR-100 script): class_splits is still just a contiguous
# chunking of NUM_STEPS x CLASSES_PER_STEP over the remapped 0..199 label space
# it is given. For CUB-200 it is the FIXED,
# PERSISTED, seed-42-shuffled order recorded in the class-order artifact
# loaded above -- the remap step above is what turns "whichever original ID
# the HF dataset builder happened to assign" into that fixed order, before
# class_splits ever runs. class_splits itself does not know or care which
# dataset produced its input labels, and is NOT reshuffled here or anywhere
# else in this file -- the one shuffle happened once, when the artifact was
# generated, and is being reused, not repeated.
class_splits = [
...
# PROTOCOL LOCK -- hard assertion (task brief Section 7): exact class
# splits, cross-checked against the persisted artifact's own task list.
assert class_splits == [t["new_class_ids"] for t in CUB200_CLASS_ORDER["tasks"]], (
    "class_splits must match the persisted class-order artifact's own task assignment exactly"
)
...
seen_class_counts = [len([c for split in class_splits[:i + 1] for c in split]) for i in range(NUM_STEPS)]

# --- Sanity checks (task brief Section 12) ---------------------------------
assert len(set(all_classes)) == NUM_CLASSES, "exactly 200 benchmark classes required"
assert len(class_splits) == NUM_STEPS, "exactly 5 tasks required"
assert all(len(s) == 40 for s in class_splits), "exactly 40 classes per task required"
for _i in range(len(class_splits)):
    for _j in range(len(class_splits)):
        if _i != _j:
            assert not (set(class_splits[_i]) & set(class_splits[_j])), (
                f"class overlap detected between task {_i} and task {_j}"
            )
assert set(all_classes) == set(range(NUM_CLASSES)), "union of all task classes must equal all 200 benchmark classes"
assert all(0 <= c < NUM_CLASSES for c in all_classes), "all remapped labels must lie in [0,199]"
assert seen_class_counts == [40, 80, 120, 160, 200], (
    f"CUB seen-class progression must be [40,80,120,160,200], got {seen_class_counts}"
)
print("CUB-200 5x40 split summary:")
print(f"  dataset_identifier = {CUB_DATASET_ID!r}")
print(f"  class_order_seed   = {CUB200_CLASS_ORDER['seed']}")
print(f"  artifact_path      = {CUB200_CLASS_ORDER_ARTIFACT_PATH!r}")
for i, cls in enumerate(class_splits, start=1):
    print(f"  Task {i}: new IDs {cls[0]}-{cls[-1]} (40 classes)")
...
def _processor_size(value, default):
    if isinstance(value, dict):
        return value
    if isinstance(value, int):
        return {"height": int(value), "width": int(value)}
    return default


_clip_crop_size = _processor_size(
    getattr(image_processor, "crop_size", None),
    {"height": 224, "width": 224},
)
H = int(_clip_crop_size.get("height", 224))
W = int(_clip_crop_size.get("width", 224))

_clip_resize_size = _processor_size(
    getattr(image_processor, "size", None),
    {"shortest_edge": min(H, W)},
)
if "shortest_edge" in _clip_resize_size:
    _clip_shortest_edge = int(_clip_resize_size["shortest_edge"])
else:
    _clip_shortest_edge = int(min(
        _clip_resize_size.get("height", H),
        _clip_resize_size.get("width", W),
    ))

# CLIPImageProcessor exposes the processor's PIL resampling enum as an integer
# (3 == bicubic in PIL). Use the equivalent torchvision interpolation explicitly
# so arbitrary CUB aspect ratios are preserved before cropping.
_clip_resample = getattr(image_processor, "resample", 3)
_clip_interpolation = transforms.InterpolationMode.BICUBIC if int(_clip_resample) == 3 else transforms.InterpolationMode.BILINEAR

def _clip_resize_preserving_aspect():
    return transforms.Resize(_clip_shortest_edge, interpolation=_clip_interpolation)
...
train_transform = transforms.Compose([
    _clip_resize_preserving_aspect(),
...
def build_classwise_train_val_splits(train_ds, val_per_class):
    ...
    # CUB has 200 classes.  Calling Dataset.filter once per class makes the
    # readiness path needlessly rescan/decode the complete Arrow table 200
    # times.  Build the same per-class deterministic split from the label
    # column with indexed selects instead.  This is a dataset plumbing
    # optimization only; the classwise shuffle, validation count, and
    # train-only provenance are unchanged.
    class_indices = {int(cls): [] for cls in all_classes}
    for row_idx, label in enumerate(train_ds[LABEL_COL]):
        class_indices[int(label)].append(row_idx)

    for cls in all_classes:
        cls_ds = train_ds.select(class_indices[int(cls)]).shuffle(seed=SEED + int(cls))
        if len(cls_ds) <= 1:
...
assert len(train_source) + len(val_source) == dataset["train"].num_rows
assert len(train_source) == 4994, f"CUB train-after-validation count must be 4994 with 5 val/class, got {len(train_source)}"
assert len(val_source) == 1000, f"CUB validation count must be 1000 with 5 val/class, got {len(val_source)}"
print(f"CUB selected split counts: train={len(train_source)}, validation={len(val_source)}, test={len(dataset[FINAL_EVAL_SPLIT_NAME])}")
for _task_id, _task_classes in enumerate(class_splits, start=1):
    _task_train = len(filter_by_classes(train_source, _task_classes))
    _task_val = len(filter_by_classes(val_source, _task_classes))
    _task_test = len(filter_by_classes(dataset[FINAL_EVAL_SPLIT_NAME], _task_classes))
    print(f"CUB Task {_task_id}: train={_task_train}, validation={_task_val}, test={_task_test}, classes={len(_task_classes)}")
...
def make_eval_dataset(class_ids):
    # IMAGENET-100 ADAPTATION: this mirror's final evaluation split is named
    # "validation" (FINAL_EVAL_SPLIT_NAME), not "test" -- see that constant's
    # own comment near the dataset-loading cell.
    ds = filter_by_classes(dataset[FINAL_EVAL_SPLIT_NAME], class_ids)
    if FAST_RUN:
...
def _extract_classification_logits(predictions):
    if isinstance(predictions, (tuple, list)):
        candidates = [
            value for value in predictions
            if hasattr(value, "shape") and len(value.shape) == 2 and value.shape[-1] == NUM_CLASSES
        ]
        if len(candidates) != 1:
            raise ValueError(f"Expected one ({NUM_CLASSES})-way logits tensor, got {[getattr(v, 'shape', None) for v in predictions]}")
        return np.asarray(candidates[0])
    return np.asarray(predictions)


def _task_ids_from_labels(labels):
    return np.asarray([CLASS_TO_TASK[int(label)] for label in np.asarray(labels)], dtype=np.int64)


def _classifier_state_for_artifact(model):
    classifier = model.classifier
    if hasattr(classifier, "modules_to_save") and "default" in classifier.modules_to_save:
        classifier = classifier.modules_to_save["default"]
    return {
        "weight": classifier.weight.detach().cpu().clone(),
        "bias": classifier.bias.detach().cpu().clone(),
        "row_norm": classifier.weight.detach().norm(dim=1).cpu(),
    }


def _save_final_split_artifact(method_name, split_name, logits, labels, sample_ids):
    logits = np.asarray(logits, dtype=np.float32)
    labels = np.asarray(labels, dtype=np.int64)
    sample_ids = np.asarray(sample_ids, dtype=np.int64)
    assert logits.ndim == 2 and logits.shape[1] == NUM_CLASSES
    assert len(logits) == len(labels) == len(sample_ids)
    payload = {
        "logits": torch.from_numpy(logits),
        "labels": torch.from_numpy(labels),
        "task_ids": torch.from_numpy(_task_ids_from_labels(labels)),
        "sample_ids": torch.from_numpy(sample_ids),
        "method_name": method_name,
        "split": split_name,
    }
    method_dir = os.path.join(ARTIFACTS_DIR, method_name)
    os.makedirs(method_dir, exist_ok=True)
    torch.save(payload, os.path.join(method_dir, f"{split_name}.pt"))


def _export_validation_and_classifier_artifacts(model, trainer, method_name):
    raw_val_ds = val_source
    val_ds = raw_val_ds.with_transform(preprocess_val)
    val_pred = trainer.predict(val_ds)
    val_logits = _extract_classification_logits(val_pred.predictions)
    val_labels = np.asarray(val_pred.label_ids, dtype=np.int64)
    raw_val_labels = np.asarray(raw_val_ds[LABEL_COL], dtype=np.int64)
    assert np.array_equal(val_labels, raw_val_labels)
    _save_final_split_artifact(
        method_name, "validation", val_logits, val_labels,
        np.asarray(raw_val_ds["__sample_id__"], dtype=np.int64),
    )
    method_dir = os.path.join(ARTIFACTS_DIR, method_name)
    torch.save(_classifier_state_for_artifact(model), os.path.join(method_dir, "classifier.pt"))
...
def evaluate_per_step_accuracy(model, method_name):
    ...
    test_logits_parts = []
    test_labels_parts = []
    test_sample_id_parts = []
    for step_idx in range(NUM_STEPS):
        current_classes = classes_for_step(step_idx)
        raw_eval_ds = filter_by_classes(dataset[FINAL_EVAL_SPLIT_NAME], current_classes)
        eval_ds = raw_eval_ds.with_transform(preprocess_val)
        pred_out = trainer.predict(eval_ds)
        logits = _extract_classification_logits(pred_out.predictions)
        labels = np.asarray(pred_out.label_ids)
        test_logits_parts.append(logits)
        test_labels_parts.append(labels)
        test_sample_id_parts.append(np.asarray(raw_eval_ds["__sample_id__"], dtype=np.int64))
    ...
    _save_final_split_artifact(
        method_name, "test", np.concatenate(test_logits_parts, axis=0),
        np.concatenate(test_labels_parts, axis=0), np.concatenate(test_sample_id_parts, axis=0),
    )
    _export_validation_and_classifier_artifacts(model, trainer, method_name)
    return per_step_map
...
def compute_normalized_rankext_factor_orth_components(model, step_idx, eps=1e-12):
    """Task/rank-normalized FactorOrth for the two improved RankExt arms.

    The live RankExt tensors store all previous LoRA blocks concatenated in
    ``A_frozen``/``B_frozen``.  Split that tensor back into the exact previous
    task blocks from the active cumulative rank schedule, normalize each
    old/new pair independently by ``r_old * r_new``, average the pair
    penalties, then average across active q/v modules.  This preserves the
    production row/column factor normalization while removing the raw-energy
    growth caused solely by increasing cumulative old rank.
    """
    device = next(model.parameters()).device
    dtype = next(model.parameters()).dtype
    layer_a, layer_b, layer_total = [], [], []
    schedule = [int(v) for v in active_rankext_rank_schedule()]
    step_idx = int(step_idx)
    if step_idx <= 0:
        zero = torch.tensor(0.0, device=device, dtype=dtype)
        return {"num_layers": 0, "factor_A_mean": zero, "factor_B_mean": zero,
                "factor_total_mean": zero, "mean_A_overlap": zero, "mean_B_overlap": zero}

    for _, module in model.named_modules():
        if not isinstance(module, GrowingRankLoRALinear) or module.new_rank <= 0:
            continue
        if module.A_frozen is None or module.B_frozen is None:
            continue
        A_new, B_new = module.A_new, module.B_new
        new_rank = int(A_new.shape[0])
        assert new_rank == schedule[step_idx] - schedule[step_idx - 1], (
            f"RankExt new-rank mismatch at step {step_idx + 1}: "
            f"tensor={new_rank}, schedule={schedule}"
        )
        assert int(module.A_frozen.shape[0]) == schedule[step_idx - 1]
        assert int(module.B_frozen.shape[1]) == schedule[step_idx - 1]
        pair_a, pair_b = [], []
        for old_task_idx in range(step_idx):
            start = 0 if old_task_idx == 0 else schedule[old_task_idx - 1]
            end = schedule[old_task_idx]
            A_old = module.A_frozen[start:end, :]
            B_old = module.B_frozen[:, start:end]
            old_rank = int(end - start)
            denom = float(max(1, old_rank * new_rank))
            A_old_hat = A_old / A_old.norm(dim=1, keepdim=True).clamp_min(eps)
            A_new_hat = A_new / A_new.norm(dim=1, keepdim=True).clamp_min(eps)
            B_old_hat = B_old / B_old.norm(dim=0, keepdim=True).clamp_min(eps)
            B_new_hat = B_new / B_new.norm(dim=0, keepdim=True).clamp_min(eps)
            overlap_a = (A_old_hat @ A_new_hat.T).pow(2).sum() / denom
            overlap_b = (B_old_hat.T @ B_new_hat).pow(2).sum() / denom
            pair_a.append(overlap_a)
            pair_b.append(overlap_b)
        if pair_a:
            a_mean = torch.stack(pair_a).mean()
            b_mean = torch.stack(pair_b).mean()
            layer_a.append(a_mean)
            layer_b.append(b_mean)
            layer_total.append(0.5 * (a_mean + b_mean))
    ...
    if not layer_total:
        ...
        return {"num_layers": 0, "factor_A_mean": zero, "factor_B_mean": zero,
                "factor_total_mean": zero, "mean_A_overlap": zero, "mean_B_overlap": zero}
    return {
        "num_layers": len(layer_total),
        "factor_A_mean": torch.stack(layer_a).mean(),
        "factor_B_mean": torch.stack(layer_b).mean(),
        "factor_total_mean": torch.stack(layer_total).mean(),
        "mean_A_overlap": torch.stack(layer_a).mean(),
        "mean_B_overlap": torch.stack(layer_b).mean(),
    }
...
class DeltaOrthRankExtensionTrainer(RankExtensionTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.method_name = str(method_name)
        self.step_idx = int(step_idx)
        self.normalized_factor_orth = self.method_name in {
            "rank_extension_factor_orth_normalized_lam50",
            "rank_extension_factor_orth_normalized_lam50_fullkd_T2_protect30",
        }
        self.teacher_model = teacher_model
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        normalized_factor_comps = {"num_layers": 0, "factor_A_mean": zero,
                                   "factor_B_mean": zero, "factor_total_mean": zero,
                                   "mean_A_overlap": zero, "mean_B_overlap": zero}
        ...
        if self.orth_mode != "none" and self.orth_mode != "dense_orth":
            ...
            if self.orth_mode == "factor_orth" and self.normalized_factor_orth:
                normalized_factor_comps = compute_normalized_rankext_factor_orth_components(
        ...
        if self.orth_mode == "none":
            ...
            orth_loss_used = (
                normalized_factor_comps["factor_total_mean"]
                if self.normalized_factor_orth else comps["factor_total_mean"]
            )
        elif self.orth_mode == "dense_orth":
            # JOINT PACK ADDITION (RankExt Arms RE-2/RE-3).
            orth_loss_used = dense_orth_comps["dense_orth_mean"]
        ...
        normalized_factor_a_v = float(normalized_factor_comps["factor_A_mean"].detach().cpu().item())
        normalized_factor_b_v = float(normalized_factor_comps["factor_B_mean"].detach().cpu().item())
        normalized_factor_total_v = float(normalized_factor_comps["factor_total_mean"].detach().cpu().item())
        ...
        weighted_factor_used_v = float((effective_lambda_orth * orth_loss_used).detach().cpu().item())
        weighted_factor_ratio_v = abs(weighted_factor_used_v) / (ce_v + float(self.orth_eps))
        active_signal_v = ce_v + weighted_kd_v + weighted_protect_v
        row = {
            "method": self.method_name,
            "step": int(self.step_idx + 1),
            ...
            "weighted_factor_orth_mean": weighted_factor_used_v,
            "weighted_factor_orth_over_CE": weighted_factor_ratio_v,
            "factor_orth_normalized": bool(self.normalized_factor_orth),
            "normalized_factor_A_penalty_mean": normalized_factor_a_v,
            "normalized_factor_B_penalty_mean": normalized_factor_b_v,
            "normalized_factor_total_penalty_mean": normalized_factor_total_v,
            "weighted_normalized_factor_orth_mean": weighted_factor_used_v if self.normalized_factor_orth else np.nan,
            "normalized_factor_orth_over_CE": weighted_factor_ratio_v if self.normalized_factor_orth else np.nan,
            "weighted_factor_orth_over_CE_KD_Protect": abs(weighted_factor_used_v) / (active_signal_v + float(self.orth_eps)),
            "normalized_factor_orth_over_CE_KD_Protect": (
                abs(weighted_factor_used_v) / (active_signal_v + float(self.orth_eps))
                if self.normalized_factor_orth else np.nan
            ),
...
def run_cub200_lightweight_preflight():
    """Run readiness checks without entering a training epoch."""
    assert set(ACTIVE_METHOD_NAMES) == EXPECTED_METHODS
    assert len(ACTIVE_METHOD_NAMES) == 8
    assert all(int(v) == 5 for v in [
        FULL_FT_EPOCHS, FULL_LORA_EPOCHS, FULL_JOINT_EPOCHS,
        FULL_ORTH_EPOCHS, FULL_RANKEXT_EPOCHS, SCRATCH_EPOCHS,
        FT_EPOCHS, LORA_EPOCHS, JOINT_EPOCHS, ORTH_EPOCHS, RANKEXT_EPOCHS,
    ])
    assert active_rankext_rank_schedule() == [16, 32, 48, 64, 80]
    assert dataset["train"].num_rows == 5994
    assert dataset[FINAL_EVAL_SPLIT_NAME].num_rows == 5794
    assert len(_observed_original_ids) == NUM_CLASSES
    assert len(class_splits) == 5 and all(len(task) == 40 for task in class_splits)
    assert seen_class_counts == [40, 80, 120, 160, 200]
    assert sorted(c for task in class_splits for c in task) == list(range(NUM_CLASSES))
    assert isinstance(image_processor, CLIPImageProcessor)

    sample = dataset["train"][0]
    sample_image = to_pil(sample[IMAGE_COL])
    assert isinstance(sample_image, Image.Image)
    pixel_values = val_transform(sample_image).unsqueeze(0)
    labels = torch.tensor([int(sample[LABEL_COL])], dtype=torch.long)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    pixel_values = pixel_values.to(device)
    labels = labels.to(device)

    print(f"[preflight] device={device} | sample label={int(labels.item())} | image={sample_image.size}")
    print("[preflight] loading CLIP ViT-B/16 from configured cache/checkpoint")

    simple_model = add_lora(
        fresh_pretrained_model().to(device),
        target_modules=family_target_modules("simple_avg"),
    )
    simple_model.train()
    simple_out = simple_model(pixel_values=pixel_values, labels=labels)
    assert simple_out.logits.shape == (1, NUM_CLASSES)
    simple_out.loss.backward()
    print("[preflight] SimpleAvg LoRA forward/backward: PASS")
    del simple_model, simple_out
    cleanup()

    rank_model = build_rank_extension_model(previous_rank_state=None, step_idx=0, old_active_in_forward=True).to(device)
    rank_model.train()
    rank_out = rank_model(pixel_values=pixel_values, labels=labels)
    assert rank_out.logits.shape == (1, NUM_CLASSES)
    rank_out.loss.backward()
    print("[preflight] RankExt rank-16 forward/backward: PASS")
    del rank_model, rank_out
    cleanup()
    print("[preflight] all 8 active methods, epoch=5 values, dataset, processor, model, and protocol checks: PASS")


if CUB200_PREFLIGHT_ONLY:
    run_cub200_lightweight_preflight()
    print("CUB200_PREFLIGHT_ONLY=1: no full epoch or training driver started.")
    raise SystemExit(0)


# ============================================================================== 
# FINAL 8-METHOD JOB4971615-COMPATIBLE CUB-200 5x40 EXPERIMENT -- REPORTING SECTION. Everything
...
print("=" * 80)
print("FINAL 8-METHOD JOB4971615-COMPATIBLE CUB-200 5x40 EXPERIMENT -- REPORTING SECTION START")
print("=" * 80)
...
# ------------------------------------------------------------------------
# 1) Granular per-batch loss log for ALL 8 arms.
# ------------------------------------------------------------------------
final8_batch_log_df = pd.DataFrame(orth_kd_train_rows)
final8_batch_log_path = os.path.join(LOGS_DIR, "final_8method_training_loss_history_by_batch.csv")
final8_batch_log_df.to_csv(final8_batch_log_path, index=False)
print(f"[reporting] wrote {len(final8_batch_log_df)} per-batch loss rows -> {final8_batch_log_path}")
_methods_in_batch_log = set(final8_batch_log_df["method"].unique()) if "method" in final8_batch_log_df.columns else set()
...
if len(final8_batch_log_df) > 0 and {"method", "step", "epoch"}.issubset(final8_batch_log_df.columns):
    _agg_cols = [
        c for c in [
        ...
        ] if c in final8_batch_log_df.columns
    ]
    final8_epoch_agg_df = (
        final8_batch_log_df.groupby(["method", "step", "epoch"], as_index=False)[_agg_cols].mean()
    )
    final8_epoch_agg_path = os.path.join(TABLES_DIR, "final_8method_loss_by_method_step_epoch.csv")
    final8_epoch_agg_df.to_csv(final8_epoch_agg_path, index=False)
    print(f"[reporting] wrote {len(final8_epoch_agg_df)} (method, step, epoch) aggregated loss rows -> {final8_epoch_agg_path}")
else:
    final8_epoch_agg_df = pd.DataFrame()
    print("[reporting][WARNING] per-batch log is empty or missing method/step/epoch columns -- no epoch aggregation written")

# ------------------------------------------------------------------------
# 2) Full per-epoch train/val CE history for all 8 arms (raw accumulator,
#    already populated per-arm/per-step/per-epoch, unconditionally, by
#    train_with_trainer() for every call site in both families).
# ------------------------------------------------------------------------
final8_epoch_loss_df = pd.DataFrame(epoch_loss_rows)
final8_epoch_loss_path = os.path.join(TABLES_DIR, "final_8method_training_loss_history_by_epoch.csv")
final8_epoch_loss_df.to_csv(final8_epoch_loss_path, index=False)
print(f"[reporting] wrote {len(final8_epoch_loss_df)} raw per-epoch rows -> {final8_epoch_loss_path}")

final8_best_epoch_df = pd.DataFrame(best_epoch_selection_rows)
final8_best_epoch_path = os.path.join(TABLES_DIR, "final_8method_best_epoch_selection_by_method_step.csv")
final8_best_epoch_df.to_csv(final8_best_epoch_path, index=False)
print(f"[reporting] wrote {len(final8_best_epoch_df)} best-epoch-selection rows -> {final8_best_epoch_path}")

final8_new_block_warmup_df = pd.DataFrame(rankext_new_block_warmup_diagnostic_rows)
final8_new_block_warmup_path = os.path.join(TABLES_DIR, "final_8method_rankext_new_block_warmup_diagnostics.csv")
final8_new_block_warmup_df.to_csv(final8_new_block_warmup_path, index=False)
print(f"[reporting] wrote {len(final8_new_block_warmup_df)} new-block-warmup diagnostic rows -> {final8_new_block_warmup_path}")

# ------------------------------------------------------------------------
# 3) Per-step accuracy (open + restricted), all 8 arms.
# ------------------------------------------------------------------------
final8_per_step_open_df = pd.DataFrame(per_step_accuracy_rows)
final8_per_step_open_path = os.path.join(TABLES_DIR, "final_8method_per_step_accuracy_open.csv")
final8_per_step_open_df.to_csv(final8_per_step_open_path, index=False)

final8_per_step_restricted_df = pd.DataFrame(per_step_accuracy_restricted_rows)
final8_per_step_restricted_path = os.path.join(TABLES_DIR, "final_8method_per_step_accuracy_open_vs_restricted.csv")
final8_per_step_restricted_df.to_csv(final8_per_step_restricted_path, index=False)
print(
    f"[reporting] wrote {len(final8_per_step_open_df)} open per-step rows and "
    f"{len(final8_per_step_restricted_df)} open-vs-restricted per-step rows"
)

# ------------------------------------------------------------------------
# 4) Classifier protected-row restoration diagnostics.
# ------------------------------------------------------------------------
final8_classifier_restoration_df = pd.DataFrame(classifier_restoration_rows)
final8_classifier_restoration_path = os.path.join(TABLES_DIR, "final_8method_classifier_restoration_diagnostics.csv")
final8_classifier_restoration_df.to_csv(final8_classifier_restoration_path, index=False)
print(f"[reporting] wrote {len(final8_classifier_restoration_df)} classifier-restoration rows -> {final8_classifier_restoration_path}")
if len(final8_classifier_restoration_df) > 0:
    _max_w = float(final8_classifier_restoration_df["protected_weight_max_abs_diff"].max())
    _max_b = float(final8_classifier_restoration_df["protected_bias_max_abs_diff"].max())
...
# ------------------------------------------------------------------------
final8_cl_trajectory_df = pd.DataFrame(cl_trajectory_rows)
final8_cl_trajectory_path = os.path.join(TABLES_DIR, "final_8method_rankext_cl_retention_plasticity_trajectory.csv")
final8_cl_trajectory_df.to_csv(final8_cl_trajectory_path, index=False)
print(f"[reporting] wrote {len(final8_cl_trajectory_df)} RankExt CL trajectory rows -> {final8_cl_trajectory_path}")
print(
    ...
    "final_8method_training_loss_history_by_epoch.csv (each specialist's own train/val CE at "
...
# ------------------------------------------------------------------------
# 6) FINAL SUMMARY TABLE -- all 8 arms.
#
...
# ------------------------------------------------------------------------
final8_training_merge_df = pd.DataFrame(training_merge_summary_rows)
assert set(final8_training_merge_df["training_method"].unique()) == set(EXPECTED_8_METHODS), (
    "STOP CONDITION: the result table is missing at least one of the 8 expected arms -- "
    f"got {sorted(final8_training_merge_df['training_method'].unique())}, expected {EXPECTED_8_METHODS}"
)
...
if len(final8_best_epoch_df) > 0 and "method_name" in final8_best_epoch_df.columns:
    _val_ce_by_method = final8_best_epoch_df.groupby("method_name")["selected_val_ce"].mean()
else:
...
for _method in EXPECTED_8_METHODS:
    ...
    _row = final8_training_merge_df[final8_training_merge_df["training_method"] == _method].iloc[0]
    _orth_type = (
...
final8_summary_path = os.path.join(TABLES_DIR, "final_8method_summary_table.csv")
final8_summary_df.to_csv(final8_summary_path, index=False)
print(f"[reporting] wrote final 8-method summary table (units: all values 0-100 percentage) -> {final8_summary_path}")
print(final8_summary_df.to_string(index=False))
...
def _all_seen(method_name):
    return float(final8_summary_df.loc[final8_summary_df["method"] == method_name, "all_seen"].iloc[0])

_sa_plain = "simple_avg"
_sa_kd_t2 = "simple_avg_kd_oldseen_T2_warmup"
# JOB4971615-COMPATIBLE 8-METHOD SCRIPT: "_sa_kd_t4" removed -- the T4 arm
# never trains in this script (METHODS_TO_RUN), so no "simple_avg_kd_oldseen_
# T4_warmup" row exists in final8_summary_df to look up.
...
_re_orth = "rank_extension_factor_orth_normalized_lam50"
_re_combined = "rank_extension_factor_orth_normalized_lam50_fullkd_T2_protect30"

_within_run_deltas = [
    {"comparison": "SA KD-T2 - SA", "delta_all_seen_pp": _all_seen(_sa_kd_t2) - _all_seen(_sa_plain)},
    # JOB4971615-COMPATIBLE 8-METHOD SCRIPT: the two "SA KD-T4 - ..." rows are
    # removed -- T4 never trains in this script, so no such comparison exists.
...
final8_within_run_df = pd.DataFrame(_within_run_deltas)
final8_within_run_path = os.path.join(TABLES_DIR, "final_8method_automatic_within_run_comparisons.csv")
final8_within_run_df.to_csv(final8_within_run_path, index=False)
print("\n[reporting] AUTOMATIC WITHIN-RUN COMPARISONS (all-seen accuracy, percentage points):")
print(final8_within_run_df.to_string(index=False))
...
#    silently merged into the newly-trained final8_summary_df above. All
...
final8_reference_df = pd.DataFrame(_reference_rows)
final8_reference_path = os.path.join(TABLES_DIR, "final_8method_reference_values_not_retrained.csv")
final8_reference_df.to_csv(final8_reference_path, index=False)
print(f"\n[reporting] wrote {len(final8_reference_df)} REFERENCE_ONLY_NOT_RETRAINED rows -> {final8_reference_path}")
...
_cross_run_rows = [
    ...
    # JOB4971615-COMPATIBLE 8-METHOD SCRIPT: the "_sa_kd_t4" cross-run row is
    # removed -- T4 never trains in this script.
    {
...
final8_cross_run_df = pd.DataFrame(_cross_run_rows)
final8_cross_run_df["delta_vs_job4970580_pp"] = final8_cross_run_df["this_run_all_seen"] - final8_cross_run_df["job4970580_all_seen"]
final8_cross_run_df["delta_vs_r7_exp1_pp"] = final8_cross_run_df["this_run_all_seen"] - final8_cross_run_df["r7_exp1_all_seen"]
final8_cross_run_path = os.path.join(TABLES_DIR, "final_8method_cross_run_comparison_CONFOUNDED.csv")
final8_cross_run_df.to_csv(final8_cross_run_path, index=False)
print("\n[reporting] CROSS-RUN COMPARISON (CONFOUNDED -- see confound_note per row, NOT causal):")
print(final8_cross_run_df.to_string(index=False))

# ------------------------------------------------------------------------
# 10) Output checklist.
# ------------------------------------------------------------------------
_overall_pass = (
    set(final8_training_merge_df["training_method"].unique()) == set(EXPECTED_8_METHODS)
    and len(_methods_in_batch_log) >= 7  # at least the 7 non-plain arms must have granular logging
    and len(final8_classifier_restoration_df) > 0
    and len(final8_cl_trajectory_df) > 0
)
_checklist_lines = [
    "OVERALL PASS" if _overall_pass else "OVERALL FAIL -- see details below",
    f"8 expected methods present in results table: {set(final8_training_merge_df['training_method'].unique()) == set(EXPECTED_8_METHODS)}",
    f"Methods with granular per-batch logging: {sorted(_methods_in_batch_log)} ({len(_methods_in_batch_log)}/8)",
    f"Classifier restoration diagnostics rows: {len(final8_classifier_restoration_df)}",
    f"CL retention/plasticity trajectory rows (RankExt): {len(final8_cl_trajectory_df)}",
    f"Summary table path: {final8_summary_path}",
]
final8_checklist_path = os.path.join(REPORTS_DIR, "final_8method_output_checklist.txt")
with open(final8_checklist_path, "w", encoding="utf-8") as _f:
    _f.write("\n".join(_checklist_lines) + "\n")
print(f"\n[reporting] wrote output checklist -> {final8_checklist_path}")
for _line in _checklist_lines:
    print(f"  {_line}")

print("=" * 80)
print("FINAL 8-METHOD JOB4971615-COMPATIBLE CUB-200 5x40 EXPERIMENT -- REPORTING SECTION COMPLETE")
print("=" * 80)
