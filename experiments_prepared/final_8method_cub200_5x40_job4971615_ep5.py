...
#
# ==============================================================================
# JOB4971615-COMPATIBLE 8-METHOD CUB-200-2011 EXPERIMENT (2026-09-22).
#
...
# long).
RUN_NAME_BASE = f"cub200_5x40_final_8method_job4971615_compatible_seed{SEED}_ep5"
RUN_NAME = f"{RUN_NAME_BASE}_{'PREFLIGHT_ONLY' if CUB200_PREFLIGHT_ONLY else 'EPOCH5_MAIN'}"
...
assert LORA_EPOCHS == 5, f"LORA_EPOCHS must be 5 for this script, got {LORA_EPOCHS}"
assert RANKEXT_EPOCHS == 5, f"RANKEXT_EPOCHS must be 5 for this script, got {RANKEXT_EPOCHS}"
...
# alpha=160, while RankExt grows by +16 per task to final rank=160. These are
...
assert LORA_EPOCHS == 5 and RANKEXT_EPOCHS == 5, (
    f"Both families must use 5 epochs, got LORA_EPOCHS={LORA_EPOCHS}, RANKEXT_EPOCHS={RANKEXT_EPOCHS}"
)
...
print(f"[reporting] wrote final 8-method summary table (units: all values 0-100 percentage) -> {final9_summary_path}")
...
# T4_warmup" row exists in final9_summary_df to look up.
...
_checklist_lines = [
    ...
    f"8 expected methods present in results table: {set(final9_training_merge_df['training_method'].unique()) == set(EXPECTED_8_METHODS)}",
