...
#
# ==============================================================================
# JOB4971615-COMPATIBLE 8-METHOD IMAGENET-100 EXPERIMENT (2026-09-21).
#
...
#
# See R7/imagenet100_job4971615_compatibility_readiness.md for the full
...
#   2. New constants IMAGENET100_DATASET_ID / IMAGENET100_ROOT /
#      IMAGENET100_CLASS_ORDER_ARTIFACT_PATH / FINAL_EVAL_SPLIT_NAME, added
#      immediately before the dataset-loading cell.
#   3. The dataset-loading cell is replaced by a local prepared-folder reader.
...
#      (`"test"` for this prepared dataset; calibration is separate), since the
...
if not IMAGENET100_ROOT:
    raise SystemExit("FATAL: IMAGENET100_ROOT is required; production training never downloads ImageNet")
if not os.path.isdir(IMAGENET100_ROOT):
    raise SystemExit(f"FATAL: IMAGENET100_ROOT is not a directory: {IMAGENET100_ROOT}")
...
DEBUG_MODE = FAST_RUN
if FAST_RUN:
    raise SystemExit("FATAL: FAST_RUN_DEBUG is disabled for the production benchmark; use the separate local smoke tools.")
...
# long).
RUN_NAME_BASE = f"imagenet100_5x20_final_8method_canonical_job4971615_seed{SEED}_ep9"
RUN_NAME = f"{RUN_NAME_BASE}_{'FAST_RUN_DEBUG' if FAST_RUN else 'EPOCH9_MAIN'}"
...
METHOD_DISPLAY_NAME_MAP = {
    ...
    "simple_avg_dense_orth_lam20": "SimpleAvg + DenseOrth",
    "simple_avg_dense_orth_lam20_kd_oldseen_T2_warmup": "SimpleAvg + DenseOrth + KD",
    ...
    "rank_extension_factor_orth_lam50_new": "RankExt FactorOrth",
    "rank_extension_factor_orth_lam50_fullkd_T2_protect30": "RankExt KD+FactorOrth",
...
assert NUM_CLASSES == 100, f"This experiment requires 100 total classes, got {NUM_CLASSES}"
assert "imagenet100" in RUN_NAME_BASE.lower(), f"RUN_NAME_BASE must mention imagenet100: {RUN_NAME_BASE!r}"
...
print("=" * 80)
print(f"  dataset               = imagenet100 (prepared PODNet/DER/DyTox first-100 WNIDs; root={IMAGENET100_ROOT})")
...
print("=" * 80)
print("FINAL 8-METHOD JOB4971615-COMPATIBLE IMAGENET-100 EXPERIMENT -- READINESS VERIFICATION")
print("=" * 80)
print(f"SCRIPT UNDER TEST: experiments_prepared/final_8method_imagenet100_job4971615_compatible.py")
...
assert "train" in dataset and FINAL_EVAL_SPLIT_NAME in dataset, (
    f"Expected 'train' and {FINAL_EVAL_SPLIT_NAME!r} splits from {IMAGENET100_DATASET_ID!r}, "
...
print(f"IMAGENET-100 dataset identity check PASSED: {IMAGENET100_DATASET_ID}, "
...
# --- Fixed class order / 5x20 task split (task brief Sections 5-8) --------
# Load the persistent class-order artifact. It must already exist -- this
# script never generates or overwrites it; see
# IMAGENET100_CLASS_ORDER_ARTIFACT_PATH above and the readiness report for
# how it was produced.
assert os.path.isfile(IMAGENET100_CLASS_ORDER_ARTIFACT_PATH), (
    f"Missing persistent class-order artifact: {IMAGENET100_CLASS_ORDER_ARTIFACT_PATH!r}. "
    "This must be generated once (see R7/imagenet100_5x20_static_readiness.md) and "
    "committed to the repository -- it is never (re)generated automatically at "
    "training time, so that every method arm trains against the exact same task sequence."
)
...
# persistent 0..99 ID. This new ID -- NOT the original ImageNet/WNID label
...
_ORIGINAL_ID_TO_NEW_ID = {int(k): int(v) for k, v in IMAGENET100_CLASS_ORDER["original_id_to_new_id"].items()}
assert set(_ORIGINAL_ID_TO_NEW_ID.keys()) == set(range(100)), "class-order artifact must cover all 100 original IDs"
assert set(_ORIGINAL_ID_TO_NEW_ID.values()) == set(range(100)), "class-order artifact's new IDs must be exactly 0..99"

# Verify the LOADED dataset's own label set matches what the artifact was
# built from (100 distinct original IDs, 0..99) before remapping -- fails
# loudly, not silently, on a mismatch (e.g. a different dataset revision).
_observed_original_ids = set(int(v) for v in dataset["train"].unique(LABEL_COL))
assert _observed_original_ids == set(range(100)), (
    f"Loaded dataset's {LABEL_COL!r} column does not contain exactly the 100 original IDs "
    f"[0..99] the class-order artifact expects -- got {sorted(_observed_original_ids)}"
)
try:
    _feature_names = dataset["train"].features[LABEL_COL].names
    _artifact_names = IMAGENET100_CLASS_ORDER["original_class_names_by_original_id"]
    _name_mismatches = [i for i in range(100) if _feature_names[i] != _artifact_names[str(i)]]
    if _name_mismatches:
        ...
        print("Class-name cross-check against the live dataset build: PASSED (100/100 match).")
...
def _remap_imagenet100_label(batch):
    batch[LABEL_COL] = [_ORIGINAL_ID_TO_NEW_ID[int(v)] for v in batch[LABEL_COL]]
    return batch

dataset = {split_name: split_ds.map(_remap_imagenet100_label, batched=True) for split_name, split_ds in dataset.items()}
print(f"IMAGENET-100 label remap applied: {LABEL_COL!r} now holds the fixed, seed-{SEED} "
      f"benchmark-local IDs [0..99] recorded in {IMAGENET100_CLASS_ORDER_ARTIFACT_PATH!r}, "
...
# chunking of NUM_STEPS x CLASSES_PER_STEP over whatever 0..99 label space
# it is given. For CIFAR-100 that space is the dataset's own native
# (unshuffled) label order; for ImageNet-100 it is instead the FIXED,
...
assert class_splits == [
    ...
    list(range(60, 80)),
    list(range(80, 100)),
], f"class_splits must be the exact 5x20 partition, got {class_splits}"
assert class_splits == [t["new_class_ids"] for t in IMAGENET100_CLASS_ORDER["tasks"]], (
...
assert all(len(s) == 20 for s in class_splits), "exactly 20 classes per task required"
...
assert set(all_classes) == set(range(100)), "union of all task classes must equal all 100 benchmark classes"
assert all(0 <= c <= 99 for c in all_classes), "all remapped labels must lie in [0,99]"
print("IMAGENET-100 5x20 split summary:")
print(f"  dataset_identifier = {IMAGENET100_DATASET_ID!r}")
print(f"  class_order_seed   = {IMAGENET100_CLASS_ORDER['seed']}")
...
image_processor = CLIPImageProcessor.from_pretrained(MODEL_CHECKPOINT, local_files_only=True)
...
if hasattr(image_processor, "crop_size") and image_processor.crop_size is not None:
    ...
    H = W = 224
_clip_shortest_edge = int(getattr(image_processor, "size", {}).get("shortest_edge", max(H, W)))

train_transform = transforms.Compose([
    transforms.Resize(_clip_shortest_edge, interpolation=transforms.InterpolationMode.BICUBIC),
...
class CLIPVisionForCIFAR100(nn.Module):
    ...
    def __init__(self, checkpoint, num_labels):
        ...
        self.vision_model = CLIPVisionModel.from_pretrained(checkpoint, use_safetensors=True, local_files_only=True)
...
val_source = dataset["calibration"]
...
CPU_CALIBRATION_DIR = os.path.join(BASE_OUTPUT_DIR, "cpu_calibration_artifacts")
os.makedirs(CPU_CALIBRATION_DIR, exist_ok=True)


def save_cpu_calibration_artifacts(model, method_name):
    """Export raw validation/test logits and classifier state for CPU analysis."""
    safe_name = str(method_name).replace("/", "_")
    method_dir = os.path.join(CPU_CALIBRATION_DIR, safe_name)
    os.makedirs(method_dir, exist_ok=True)
    model.eval()
    device = next(model.parameters()).device

    def artifact_collate(examples):
    ...
    task_by_class = {int(cls): int(task_idx) for task_idx, classes in enumerate(class_splits) for cls in classes}
    with torch.no_grad():
        for source_split, artifact_name in (("calibration", "validation"), ("test", "test")):
            raw = dataset[source_split].with_transform(preprocess_val)
            loader = torch.utils.data.DataLoader(raw, batch_size=64, shuffle=False, collate_fn=artifact_collate)
            logits_parts, labels_parts, task_parts, sample_parts = [], [], [], []
            for batch in loader:
                ...
                task_parts.append(np.asarray([task_by_class[int(label)] for label in labels], dtype=np.int64))
                sample_parts.extend(batch["sample_ids"])
            np.savez_compressed(
                os.path.join(method_dir, f"{artifact_name}_logits_labels_tasks_samples.npz"),
                logits=np.concatenate(logits_parts, axis=0),
                labels=np.concatenate(labels_parts, axis=0),
                task_ids=np.concatenate(task_parts, axis=0),
                sample_ids=np.asarray(sample_parts, dtype="U"),
            )
    classifier = model.classifier
    np.savez_compressed(
        os.path.join(method_dir, "classifier_weights_bias.npz"),
        weights=classifier.weight.detach().cpu().numpy(),
        biases=classifier.bias.detach().cpu().numpy(),
    )
    with open(os.path.join(method_dir, "artifact_metadata.json"), "w", encoding="utf-8") as handle:
        json.dump({"method": method_name, "validation_source_split": "calibration", "test_source_split": "test", "num_classes": NUM_CLASSES, "seed": SEED}, handle, indent=2)
    print(f"[cpu artifacts] saved validation/test logits and classifier state -> {method_dir}")
...
def run_simple_avg_variant(method_name):
    ...
    save_cpu_calibration_artifacts(merged_model, method_name)
...
def run_rank_extension_variant(
    ...
    save_cpu_calibration_artifacts(final_rank_model, method_name)
...
# ==============================================================================
# FINAL 8-METHOD JOB4971615-COMPATIBLE IMAGENET-100 EXPERIMENT -- REPORTING SECTION. Everything
...
print("=" * 80)
print("FINAL 8-METHOD JOB4971615-COMPATIBLE IMAGENET-100 EXPERIMENT -- REPORTING SECTION COMPLETE")
print("=" * 80)
