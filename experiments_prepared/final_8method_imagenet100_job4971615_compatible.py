...
# long).
RUN_NAME_BASE = f"imagenet100_5x20_final_8method_job4971615_compatible_seed{SEED}"
...
print("=" * 80)
print(f"  dataset               = imagenet100 (clane9/imagenet-100) (label_col diagnostic deferred -- 'dataset'/'LABEL_COL'")
...
IMAGENET100_DATASET_ID = "clane9/imagenet-100"

# Configurable Hugging Face `datasets` download-cache root. Overridable via
# the IMAGENET100_CACHE_ROOT environment variable; defaults to a
...
# re-download).
IMAGENET100_CACHE_ROOT = os.environ.get(
    "IMAGENET100_CACHE_ROOT",
...
IMAGENET100_CLASS_ORDER_ARTIFACT_PATH = os.path.join(
    "experiments_prepared", "splits", "imagenet100_class_order_seed42.json"
)

# This mirror's HF splits are {"train": ..., "validation": ...} -- there is
# no split literally named "test". Per the task brief Section 9, the
# official validation split is treated as the final evaluation split
# (playing the same role CIFAR-100's "test" plays) and is never touched by
# model-selection / best-epoch logic; only `make_eval_dataset()` below reads
# this constant.
FINAL_EVAL_SPLIT_NAME = "validation"

dataset = load_dataset(IMAGENET100_DATASET_ID, cache_dir=IMAGENET100_CACHE_ROOT)
...
dataset = dataset.map(_remap_imagenet100_label, batched=True)
