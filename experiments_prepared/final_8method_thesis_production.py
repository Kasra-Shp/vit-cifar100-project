...
#   SA-4: simple_avg_dense_orth_lam1                            -- DenseOrth, lambda=1 (FINAL 8-METHOD THESIS PRODUCTION: lowered from historical lambda=20)
#   SA-5: simple_avg_dense_orth_lam1_kd_oldseen_T2_warmup       -- SA-2 + SA-4 combined
...
# RENAMED for the FINAL 8-METHOD THESIS PRODUCTION script -- deliberately
# isolated from job4971615's own RUN_NAME_BASE (final_9method_performance_
# recovery) and from every other prior run's, so this script can never write
# into, resume from, or be confused with job4971615's (or any other run's)
# results/checkpoints directory. Kept short for the same Windows MAX_PATH
# reason documented in the comment above (RankExt per-step diagnostic
# filenames are long).
RUN_NAME_BASE = f"cifar100_5x20_final_8method_thesis_production_seed{SEED}"
...
# FINAL 8-METHOD THESIS PRODUCTION (2026-09-21): rank_extension's head-LR
# multiplier raised 1.0 -> 2.0, the ONE scientific change to this family in
# this run. Evidence: a local laptop decision probe (2 tasks, 2 epochs/task,
# 10/5/5 images/class, seeds 42 and 123, historical pre-task-scale-
# calibration RankExt pipeline, no new calibration) found LR=2.0 beat LR=1.0
# beat LR=0.5 on BOTH seeds, on validation CE, final-open accuracy, mean-
# restricted accuracy, and Task2-restricted accuracy, with the improvement
# rate ACCELERATING (not saturating) from 0.5->1.0->2.0 and no NaN/Inf/
# divergence/classifier-restoration failure at any condition -- see
# R7/rankext_headlr_final_compare_report.md for the full analysis. This is a
# production hypothesis informed by local decision probes at a reduced
# scale, NOT proof of a globally optimal LR at full 5x20/9-epoch scale --
# that local-probe accuracy is explicitly NOT itself a thesis benchmark
# result (see R7/final_8method_thesis_production_readiness.md Section on
# local-probe-vs-benchmark scope). simple_avg's head-LR multiplier (x10,
# proven-safe, never implicated in any instability) is UNTOUCHED.
...
# purposes). DENSE_ORTH_WARMUP_EPOCHS matches R8's own audited convention
...
#
# FINAL 8-METHOD THESIS PRODUCTION (2026-09-21): lambda lowered 20.0 -> 1.0,
# the PRIMARY scientific change to the SimpleAvg orth arms in this run. A
# mechanistic gradient diagnostic (weighted orth-gradient / CE-LoRA-gradient
# ratio) found lambda=20 reached approximately 7.9x -- i.e. the DenseOrth
# term dominated the CE training signal by nearly an order of magnitude,
# consistent with this run's motivating hypothesis that lambda=20 was overly
# aggressive. The DenseOrth loss GEOMETRY itself (per-module Frobenius-
# normalized inner product between the current, differentiable delta and
# each detached previous specialist's delta, squared and averaged) is
# UNCHANGED -- only this coefficient and the two DenseOrth method
# identifiers (lam20 -> lam1, renamed everywhere below so no misleading
# "lam20" name survives on an arm that now trains at lambda=1) changed.
DENSE_ORTH_LAMBDA = 1.0
DENSE_ORTH_WARMUP_EPOCHS = 1.0

# ==============================================================================
# FINAL 8-METHOD THESIS PRODUCTION -- TASK-SCALE CALIBRATION INTEGRITY GUARD
# (Section 10 of the task brief): the job4972616 refinement introduced
# PerTaskScaleCalibration / RankExtCalibratedModel / a validation-fitted
# 4-scalar per-task logit rescaling, applied AFTER the historical row-norm
# calibration. That mechanism must be completely ABSENT from this production
# script. This file's own lineage (final_9method_5x20_performance_recovery.py,
# the source this was copied from) never contained that code at all -- so
# this guard is safe-by-construction, not a real risk -- but it is added
# anyway as an explicit, standing, fail-loud check rather than relying on
# that historical fact alone. RankExt's evaluation sequence in this script is,
# and must remain: train -> classifier restoration -> historical row-norm
# calibration (confidence_weighted_regime_grouped) -> final evaluation. NOT:
# train -> row-norm calibration -> learned task-scale calibration.
# ==============================================================================
...
for _forbidden_calib_name in (
    ...
    assert _forbidden_calib_name not in dir(), (
        f"PRODUCTION INTEGRITY FAILURE: new task-scale calibration symbol "
        f"{_forbidden_calib_name!r} must not exist in this production script"
    )
assert RANKEXT_TASK_SCALE_CALIBRATION_ENABLED is False, (
    "PRODUCTION INTEGRITY FAILURE: new task-scale calibration must stay disabled"
)
...
METHODS_TO_RUN = {
    ...
    # SA-3 / T4 ablation: DISABLED for FINAL 8-METHOD THESIS PRODUCTION --
    # this run is exactly the 8 canonical arms (Section 0 of the task brief:
    # "No T4 arm"), not the 9-method exploratory set. See the explicit
    # `assert "simple_avg_kd_oldseen_T4_warmup" not in ACTIVE_METHOD_MAP`
    # guard below (temperature-assertions block) for a loud, standing check
...
def build_active_method_configs():
    ...
    add_method(
        "simple_avg_dense_orth_lam1", "simple_avg", "simple_avg_dense_orth",
        uses_dense_orth=True,
    )
    # SA-5 (task brief Section 12): DenseOrth lambda=20 + old-seen KD, T=2,
    # weight=1.0, 1-epoch warmup -- SA-2 + SA-4 combined.
    add_method(
        "simple_avg_dense_orth_lam1_kd_oldseen_T2_warmup", "simple_avg", "simple_avg_dense_orth_kd_oldseen_warmup",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
...
# FINAL 8-METHOD THESIS PRODUCTION: updated to this script's actual 8
# enabled base_method keys (4 SimpleAvg + 4 RankExt) -- T4 removed -- see
# METHODS_TO_RUN above for the full True/False dict this is checked against.
...
# FINAL 8-METHOD THESIS PRODUCTION: T4 arm removed (Section 0: "No T4 arm").
# Loud, standing guard against accidental reintroduction rather than a
# silent absence -- see METHODS_TO_RUN above (flipped to False).
assert "simple_avg_kd_oldseen_T4_warmup" not in ACTIVE_METHOD_MAP, (
    "T4 arm must not be present in this 8-method production run"
)
assert float(ACTIVE_METHOD_MAP["simple_avg_dense_orth_lam1_kd_oldseen_T2_warmup"]["kd_temperature"]) == 2.0
...
# --- lambda assertions: DenseOrth1 (FINAL 8-METHOD THESIS PRODUCTION,
# lowered from historical lambda=20) for SimpleAvg, FactorOrth50 for
# RankExt -- never mixed. --------------------------------------------------
assert LAMBDA_ORTH == 50.0, f"FactorOrth lambda must stay canonical (50), got {LAMBDA_ORTH}"
assert DENSE_ORTH_LAMBDA == 1.0, f"DenseOrth lambda must be the production value (1), got {DENSE_ORTH_LAMBDA}"
_SA_DENSE_ORTH_ARMS = ("simple_avg_dense_orth_lam1", "simple_avg_dense_orth_lam1_kd_oldseen_T2_warmup")
...
# FINAL 8-METHOD THESIS PRODUCTION: exactly the 8 canonical arms, T4 absent.
EXPECTED_METHODS = {
    "simple_avg",
    "simple_avg_kd_oldseen_T2_warmup",
...
assert sum(1 for m in ACTIVE_METHOD_NAMES if ACTIVE_METHOD_MAP[m]["family"] == "simple_avg") == 4, (
    "Must be exactly 4 SimpleAvg training arms (FINAL 8-METHOD THESIS PRODUCTION: T4 removed)"
)
...
print("=" * 80)
print("FINAL 8-METHOD THESIS PRODUCTION EXPERIMENT -- READINESS VERIFICATION")
print("=" * 80)
print(f"SCRIPT UNDER TEST: experiments_prepared/final_8method_thesis_production.py")
...
# This check verifies every KD base_method swept the SAME global
# KD_TEMPERATURES=[2.0] list (a leftover multi-temperature-sweep convention
# from Exp1). FINAL 8-METHOD THESIS PRODUCTION: the T4 one-off ablation
# (simple_avg_kd_oldseen_T4_warmup) that used to need excluding from this
# check no longer exists (T4 arm removed, Section 0) -- no special-case
# exclusion set is needed any more; every active KD method is checked
# uniformly.
...
# FINAL 8-METHOD THESIS PRODUCTION (Section 19 of the task brief): isolated,
# evaluation-only diagnostic accumulator for SimpleAvg's pre/post-historical-
# calibration all-seen accuracy, for all 4 SimpleAvg arms. Populated only
# from evaluate_arm_with_merge()'s merge_method=="arithmetic" path (the one
# that becomes the main per-method result) via a side-effect-free evaluation
# helper (evaluate_all_seen_accuracy_only) that does NOT touch all_results/
# method_summary_rows -- this list is NEVER read by, or merged into, the main
# 8-row summary-table construction below; it exists purely to separate
# TRAINING effect from CALIBRATION effect for post-hoc analysis, written to
# its own dedicated CSV in the reporting section.
simpleavg_calibration_diagnostic_rows = []
epoch_loss_rows = []
...
def evaluate_all_seen_accuracy_only(model, label):
    """FINAL 8-METHOD THESIS PRODUCTION (Section 19): side-effect-free
    evaluation helper -- returns the model's all-seen accuracy (%) WITHOUT
    appending to all_results/method_summary_rows or any other global
    accumulator the main reporting pipeline reads from. Used exclusively for
    the SimpleAvg pre/post-historical-calibration diagnostic capture, which
    must never contaminate the main 8-row summary table. `label` is only for
    the printed progress line, not stored/used as a method key anywhere."""
    args = get_training_args(
        output_dir=os.path.join(MODELS_DIR, "tmp_calibration_diagnostic_eval"),
        epochs=1,
        lr=LR_LORA,
        batch_size=BATCH_LORA,
        accum_steps=ACCUM_LORA,
    ...
    eval_all_out = trainer.evaluate(eval_dataset=eval_all_seen)
    # NOTE: eval_accuracy from compute_metrics() is a 0..1 FRACTION (same raw
    # convention as all_results/method_summary_rows/training_merge_summary_
    # rows elsewhere in this file) -- converted to a 0-100 percentage here so
    # this diagnostic's units match the main summary table's own units
    # exactly (that table applies the same *100.0 conversion at construction
    # time, never earlier).
    acc_pct = float(eval_all_out["eval_accuracy"]) * 100.0
    print(f"[simpleavg calibration diagnostic] {label}: all_seen={acc_pct:.4f}%")
    return acc_pct
...
def evaluate_arm_with_merge(training_method, merge_method, step_states, specialist_diagonal_accuracy, specialist_fingerprint):
    ...
    # FINAL 8-METHOD THESIS PRODUCTION (Section 19 of the task brief):
    # SimpleAvg pre/post-historical-calibration diagnostic capture.
    # Evaluation-only (evaluate_all_seen_accuracy_only never touches
    # all_results/method_summary_rows), gated to merge_method=="arithmetic"
    # (the merge that becomes the main per-method result row) so this never
    # double-evaluates the do_merge path. Written to the ISOLATED
    # simpleavg_calibration_diagnostic_rows accumulator, never read by the
    # main summary-table construction below -- purpose is separating
    # TRAINING effect from CALIBRATION effect, not creating a new benchmark
    # row.
    _capture_sa_calibration_diagnostic = (
        method_cfg["family"] == "simple_avg" and merge_method == "arithmetic"
    )
    if _capture_sa_calibration_diagnostic:
        _sa_calib_pre = evaluate_all_seen_accuracy_only(merged_model, f"{tagged_name} PRE-calibration")
    ...
    if _capture_sa_calibration_diagnostic:
        _sa_calib_post = evaluate_all_seen_accuracy_only(merged_model, f"{tagged_name} POST-calibration")
        simpleavg_calibration_diagnostic_rows.append({
            "method": training_method,
            "apply_calibration": bool(method_cfg["apply_calibration"]),
            "calibration_mode": method_cfg.get("calibration_mode", "global") if method_cfg["apply_calibration"] else "none",
            "all_seen_pre_calibration_pct": _sa_calib_pre,
            "all_seen_post_calibration_pct": _sa_calib_post,
            "calibration_gain_pp": _sa_calib_post - _sa_calib_pre,
        })
...
print("=" * 80)
print("FINAL 8-METHOD THESIS PRODUCTION SCRIPT -- REPORTING SECTION START")
print("=" * 80)
...
final9_batch_log_path = os.path.join(LOGS_DIR, "final_8method_thesis_production_training_loss_history_by_batch.csv")
...
if len(final9_batch_log_df) > 0 and {"method", "step", "epoch"}.issubset(final9_batch_log_df.columns):
    ...
    final9_epoch_agg_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_loss_by_method_step_epoch.csv")
...
final9_epoch_loss_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_training_loss_history_by_epoch.csv")
...
final9_best_epoch_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_best_epoch_selection_by_method_step.csv")
...
final9_new_block_warmup_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_rankext_new_block_warmup_diagnostics.csv")
...
final9_per_step_open_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_per_step_accuracy_open.csv")
...
final9_per_step_restricted_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_per_step_accuracy_open_vs_restricted.csv")
...
final9_classifier_restoration_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_classifier_restoration_diagnostics.csv")
...
final9_cl_trajectory_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_rankext_cl_retention_plasticity_trajectory.csv")
...
print(
    ...
    "final_8method_thesis_production_training_loss_history_by_epoch.csv (each specialist's own train/val CE at "
...
final9_summary_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_summary_table.csv")
...
_sa_orth = "simple_avg_dense_orth_lam1"
_sa_combined = "simple_avg_dense_orth_lam1_kd_oldseen_T2_warmup"
...
_within_run_deltas = [
    ...
    {"comparison": "SA DenseOrth1 - SA", "delta_all_seen_pp": _all_seen(_sa_orth) - _all_seen(_sa_plain)},
    {"comparison": "SA DenseOrth1+KD-T2 - SA", "delta_all_seen_pp": _all_seen(_sa_combined) - _all_seen(_sa_plain)},
    {"comparison": "SA DenseOrth1+KD-T2 - SA KD-T2", "delta_all_seen_pp": _all_seen(_sa_combined) - _all_seen(_sa_kd_t2)},
...
final9_within_run_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_automatic_within_run_comparisons.csv")
...
final9_reference_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_reference_values_not_retrained.csv")
...
# ------------------------------------------------------------------------
# 8b) SIMPLEAVG PRE/POST-HISTORICAL-CALIBRATION DIAGNOSTICS (Section 19 of
#    the task brief) -- ISOLATED from the main summary table above; never
#    read by it, never creates a new benchmark method row. Purpose:
#    separate TRAINING effect from CALIBRATION effect for the 4 SimpleAvg
#    arms. Populated only from evaluate_arm_with_merge()'s arithmetic-merge
#    path (see that function's own comment for the exact capture points).
# ------------------------------------------------------------------------
final9_simpleavg_calibration_diagnostic_df = pd.DataFrame(simpleavg_calibration_diagnostic_rows)
final9_simpleavg_calibration_diagnostic_path = os.path.join(
    TABLES_DIR, "final_8method_thesis_production_simpleavg_calibration_diagnostics.csv"
)
final9_simpleavg_calibration_diagnostic_df.to_csv(final9_simpleavg_calibration_diagnostic_path, index=False)
print(
    f"\n[reporting] wrote {len(final9_simpleavg_calibration_diagnostic_df)} SimpleAvg pre/post-calibration "
    f"diagnostic rows (ISOLATED from main summary table) -> {final9_simpleavg_calibration_diagnostic_path}"
)
print(final9_simpleavg_calibration_diagnostic_df.to_string(index=False))
...
final9_cross_run_path = os.path.join(TABLES_DIR, "final_8method_thesis_production_cross_run_comparison_CONFOUNDED.csv")
...
# ------------------------------------------------------------------------
# 9b) COMPARISON AGAINST JOB4971615 (Section 23 of the task brief) -- the
#    correct, intended historical baseline for THIS run's controlled
#    changes. IMPORTANT (do not conflate, per Section 23): for the two
#    SimpleAvg DenseOrth arms, this comparison tests lambda 20 -> 1; for all
#    four RankExt arms, it tests head LR 1 -> 2, under the SAME historical
#    row-norm calibration family in both jobs (no new task-scale
#    calibration in either). Both comparisons hold every other scientific
#    setting fixed relative to job4971615 EXCEPT the one named change --
#    this is NOT the same confounded, multi-factor comparison as the
#    job4970580/R7-Exp1 table above.
# ------------------------------------------------------------------------
_job4971615_reference = {
    "simple_avg": {"all_seen": 75.16, "change": "none (unchanged baseline)"},
    "simple_avg_kd_oldseen_T2_warmup": {"all_seen": 75.21, "change": "none (unchanged baseline)"},
    "simple_avg_dense_orth_lam1": {"all_seen": 74.02, "change": "lambda 20 -> 1"},
    "simple_avg_dense_orth_lam1_kd_oldseen_T2_warmup": {"all_seen": 73.22, "change": "lambda 20 -> 1"},
    "rank_extension": {"all_seen": 34.99, "change": "head LR 1 -> 2"},
    "rank_extension_fullkd_T2_protect30": {"all_seen": 65.01, "change": "head LR 1 -> 2"},
    "rank_extension_factor_orth_lam50": {"all_seen": 41.00, "change": "head LR 1 -> 2"},
    "rank_extension_factor_orth_lam50_fullkd_T2_protect30": {"all_seen": 70.76, "change": "head LR 1 -> 2"},
}
_job4971615_rows = []
for _method, _ref in _job4971615_reference.items():
    _this_run_val = _all_seen(_method)
    _job4971615_rows.append({
        "method": _method,
        "job4971615_all_seen": _ref["all_seen"],
        "this_run_all_seen": _this_run_val,
        "delta_pp": _this_run_val - _ref["all_seen"],
        "controlled_change_tested": _ref["change"],
    })
final9_job4971615_comparison_df = pd.DataFrame(_job4971615_rows)
final9_job4971615_comparison_path = os.path.join(
    TABLES_DIR, "final_8method_thesis_production_vs_job4971615.csv"
)
final9_job4971615_comparison_df.to_csv(final9_job4971615_comparison_path, index=False)
print("\n[reporting] COMPARISON AGAINST JOB4971615 (controlled-difference, NOT the same as the CONFOUNDED table above):")
print(final9_job4971615_comparison_df.to_string(index=False))

# ------------------------------------------------------------------------
# 9c) MANDATORY THESIS RESULTS-TABLE PNG/CSV RENDERER (Section 22 of the
#    task brief). Isolated module, imported here only -- does not modify
#    this script or import from it, mirroring render_final_8method_
#    results.py's own established isolation pattern (that module cannot be
#    reused as-is: its DISPLAY_ROWS hardcodes the OLD lam20 method names,
#    which no longer exist in this script's active method set).
# ------------------------------------------------------------------------
import sys as _sys
...
from render_final_8method_thesis_production_results import (
    render_final_8method_thesis_production_results as _render_final_8method_thesis_production_results,
)

final9_render_csv_path, final9_render_table_png_path, final9_render_bar_png_path = (
    _render_final_8method_thesis_production_results(final9_summary_df, output_dir="R7")
)
print(f"\n[reporting] MANDATORY thesis results-table renderer wrote:")
print(f"  CSV:        {final9_render_csv_path}")
print(f"  Table PNG:  {final9_render_table_png_path}")
print(f"  Bar PNG:    {final9_render_bar_png_path}")
...
_overall_pass = (
    ...
    and len(_methods_in_batch_log) >= 6  # at least the 6 non-plain arms must have granular logging (8 methods - 2 plain: simple_avg, rank_extension)
...
final9_checklist_path = os.path.join(REPORTS_DIR, "final_8method_thesis_production_output_checklist.txt")
...
print("=" * 80)
print("FINAL 8-METHOD THESIS PRODUCTION SCRIPT -- REPORTING SECTION COMPLETE")
print("=" * 80)
