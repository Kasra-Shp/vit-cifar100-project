...
# long).
RUN_NAME_BASE = f"cifar100_5x20_final_9method_performance_recovery_seed{SEED}"
...
METHODS_TO_RUN = {
    ...
    "simple_avg_kd_oldseen_T4_warmup": True,
...
assert float(ACTIVE_METHOD_MAP["simple_avg_kd_oldseen_T4_warmup"]["kd_temperature"]) == 4.0
...
assert len(active_methods) == 9, f"Must be exactly 9 active methods, got {len(active_methods)}"
...
assert _expected_final_configs == 9, f"Expected 9 final configurations, computed {_expected_final_configs}"
