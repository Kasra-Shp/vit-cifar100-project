"""
Final 8-method results table/PNG renderer for the refined 8-method experiment.

Standalone, isolated module -- does not import from or modify
final_8method_5x20_refined.py or final_9method_5x20_performance_recovery.py.
The refined experiment script imports and calls
`render_final_8method_results()` from its own reporting section, once, only
after verifying all 8 expected methods are present (see
...
# Exact 8 rows, in the required display order, mapping this codebase's
# internal method name -> the thesis-ready display label.
DISPLAY_ROWS = [
...
def validate_summary_before_render(summary_df, method_col="method"):
    ...
    once, T4 absent, no unexpected extras (e.g. a stray calibration row)."""
...
def render_final_8method_results(
    ...
    csv_path = os.path.join(output_dir, "final_8method_results_table.csv")
    csv_df.to_csv(csv_path, index=False, encoding="utf-8")

    table_png_path = os.path.join(output_dir, "final_8method_results_table.png")
    _render_table_png(csv_df, best_idx, table_png_path)

    bar_png_path = None
    if also_render_bar_chart:
        bar_png_path = os.path.join(output_dir, "final_8method_allseen_accuracy.png")
...
def render_from_summary_csv(summary_csv_path, output_dir):
    """Convenience wrapper for the end-of-run hook: reads the real
    final_8method_summary_table.csv this experiment writes and renders from
    it -- the values always flow from that file, never typed by hand."""
    summary_df = pd.read_csv(summary_csv_path)
    return render_final_8method_results(summary_df, output_dir)
