...
#
# ==============================================================================
# SIMPLEAVG + RANKEXT JOINT SINGLE-JOB IMPROVEMENT PACK (5x20) -- derived from
# supervisor_exp1_cifar100_5x20_fixed_rankext.py (the VERIFIED R7/Exp1
# harness), via experiments_prepared/simpleavg_singlejob_improvement_pack_5x20.py
# (the earlier SimpleAvg-ONLY preparation, preserved unchanged at its own path
# -- THIS script is the final, authoritative target, not that one). This is a
# NEW, separate script -- Exp1/R7 and the SimpleAvg-only pack are untouched.
#
# PURPOSE: use exactly ONE cluster job (UniPD cluster capacity is scarce) to
# test, on the canonical 5x20/seed42 protocol, for BOTH families:
#   (A) SimpleAvg -- whether architecture-matched regularization (old-seen KD,
#       KD warmup, DenseOrth, combined) and/or a better post-hoc merge
#       (DO-Merging) can improve the already-strong SimpleAvg family beyond
#       its ~75-76% all-seen baseline.
#   (B) RankExt -- whether the SAME new R8-style mechanisms (old-seen KD,
#       DenseOrth, combined), ported into the CORRECT, verified R7/Exp1
#       RankExt harness (never R8's drifted manual rewrite -- R8's own plain-
#       RankExt result, 16.56, is invalid and is NEVER treated as a baseline
#       here), can do the same for RankExt. This is NOT a capacity ablation
#       (Exp2's [32,64,96,128,160] schedule is never used here).
#
# INTENTIONAL PROTOCOL CHANGE: NUM_EPOCHS=6 for ALL 9 training arms (was 9 in
# Exp1) -- see LORA_EPOCHS/RANKEXT_EPOCHS's own comment below for the full
# rationale and the historical best-epoch-selection audit this is based on.
# Every same-run comparison in this script is 6-epoch vs. 6-epoch; historical
# Exp1/Exp2 9-epoch numbers are included only as clearly-labeled secondary
# references, never as the primary comparison baseline.
#
# EVERYTHING not explicitly listed below as a deliberate addition is preserved
# BYTE-IDENTICAL in MECHANISM to supervisor_exp1_cifar100_5x20_fixed_rankext.py:
# dataset, class split/ordering, seed=42, optimizer, LR, weight decay,
# scheduler, LR warmup, gradient clipping, mixed precision, validation split,
# best-epoch selection (pure validation-CE argmin), classifier training,
# classifier stitching (SimpleAvg) / classifier-row gradient masking + hard
# restoration (RankExt), calibration, open/restricted evaluation, metrics,
# per-method seed reset, RankExt's persistent-growing-model architecture
# (GrowingRankLoRALinear, frozen prior blocks, new-block warmup). The ONLY
# numeric protocol change anywhere in this file is the epoch count.
#
# TRAINING ARMS -- 5 SimpleAvg (family="simple_avg", rank=80/alpha=160/
# scaling=2, q_proj/v_proj) + 4 RankExt (family="rank_extension", schedule
# [16,32,48,64,80]/scaling=2, q_proj/v_proj) = 9 total, all seed 42, all 6
# epochs:
#   SA-0: simple_avg                                              -- canonical plain, unchanged mechanism
#   SA-1: simple_avg_kd_oldseen_T2                                -- old-seen-only KD, NO KD-weight warmup
#   SA-2: simple_avg_kd_oldseen_T2_warmup                         -- old-seen-only KD, WITH 1-epoch KD-weight warmup (R8 convention)
#   SA-3: simple_avg_dense_orth_lam20                             -- DenseOrth (dense-delta cosine^2), lambda=20, 1-epoch lambda warmup (R8 convention)
#   SA-4: simple_avg_dense_orth_lam20_kd_oldseen_T2_warmup        -- SA-2 + SA-3 combined
#   RE-0: rank_extension                                          -- canonical plain, unchanged mechanism
#   RE-1: rank_extension_kd_oldseen_T2_warmup                     -- old-seen-only KD, WITH 1-epoch KD-weight warmup
#   RE-2: rank_extension_dense_orth_lam20                         -- DenseOrth (per-block dense-delta cosine^2), lambda=20, 1-epoch lambda warmup
#   RE-3: rank_extension_dense_orth_lam20_kd_oldseen_T2_warmup    -- RE-1 + RE-2 combined
#
# The historical controls (simple_avg_kd_T2, simple_avg_factor_orth,
# simple_avg_factor_orth_kd_T2, rank_extension_kd_only_T2,
# rank_extension_orth_factor_lam_50, rank_extension_orth_factor_lam_50_kd_T2)
# are DELIBERATELY NOT retrained -- their canonical seed-42 R7 9-epoch values
# are reused as clearly-labeled HISTORICAL 9-EPOCH REFERENCE only, never as
# same-run comparisons. See thesis_agent/reports/
# simpleavg_rankext_singlejob_improvement_pack_final_readiness.md for the
# full design audit, including the DO-Merging safety audit and the RankExt
# training-machinery map performed BEFORE this script was written.
#
# MERGE OPERATORS:
#   SimpleAvg (2 per arm, 5x2=10 evaluated configurations):
#     Merge A: arithmetic -- simple_average_deltas() (unchanged, Exp1's own function)
#     Merge B: do_merge   -- do_merge_deltas() (unchanged, Exp1's own dormant
#                            "DO-Merging" function) -- wired through a
#                            correct evaluation pipeline (calibration +
#                            restricted + BWT identical to the arithmetic
#                            path); see the SimpleAvg-only pack's own audit
#                            report for the dormant-integration-bug finding
#                            this fixes. DO-Merging is SimpleAvg-ONLY --
#                            never applied to RankExt (Section 7 of the
#                            user's task; RankExt has no independent
#                            specialists to arithmetically/DO-merge -- its
#                            "final model" IS its own persistent, incrementally
#                            -grown state).
#   RankExt (1 per arm, "persistent", 4x1=4 evaluated configurations):
#     merge_method="persistent" -- RankExt's own final accumulated model
#     state after all 5 CL steps, exactly as Exp1 always evaluated it; no
#     merge operator is applied or meaningful here.
#   TOTAL: 10 + 4 = 14 evaluated configurations.
#
# PERSISTENCE: every arm's trained state (SimpleAvg: 5 independent specialist
# deltas; RankExt: the final persistent model's accumulated block structure)
# + classifier state + per-step diagnostics are saved to disk via atomic
# writes with a completion marker immediately after that arm finishes
# training; on any restart, already-completed arms are loaded from disk and
# never retrained. See `save_arm_checkpoint_atomic` /
# `load_arm_checkpoint_if_present` / `ensure_arm_trained` (SimpleAvg) and
# their RankExt analogues below.
# ==============================================================================
#
...
RUN_NAME_BASE = f"clip_vit_lora_cifar100_5x20_simpleavg_rankext_singlejob_improvement_pack_seed{SEED}"
...
METHODS_TO_RUN = {
    ...
    # SINGLEJOB PACK ADDITION: SA-1..SA-4's own base_method flags (see
    # build_active_method_configs()'s add_method() calls for these).
    "simple_avg_kd_oldseen": True,
    ...
    # JOINT PACK ADDITION: RE-1..RE-3's own base_method flags (see
    # build_active_method_configs()'s add_method() calls for these, added
    # alongside the RankExt family's existing add_method() calls below).
    "rank_extension_kd_oldseen_warmup": True,
    "rank_extension_dense_orth": True,
    "rank_extension_dense_orth_kd_oldseen_warmup": True,
...
def build_active_method_configs():
    ...
    # ==========================================================================
    add_method(
        "simple_avg_kd_oldseen_T2", "simple_avg", "simple_avg_kd_oldseen",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
        kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM1,
    )
    add_method(
        "simple_avg_kd_oldseen_T2_warmup", "simple_avg", "simple_avg_kd_oldseen_warmup",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
        kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM2_ARM4,
    )
    add_method(
    ...
    add_method(
        ...
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
        kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM2_ARM4, uses_dense_orth=True,
    )
    # ==========================================================================
    ...
    # ==========================================================================
    add_method(
        "rank_extension_kd_oldseen_T2_warmup", "rank_extension", "rank_extension_kd_oldseen_warmup",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
        kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM2_ARM4,
    )
    add_method(
        "rank_extension_dense_orth_lam20", "rank_extension", "rank_extension_dense_orth",
        uses_dense_orth=True,
    )
    add_method(
        "rank_extension_dense_orth_lam20_kd_oldseen_T2_warmup", "rank_extension", "rank_extension_dense_orth_kd_oldseen_warmup",
        uses_kd=True, kd_temperature=2.0, kd_class_scope="old_seen",
        kd_warmup_epochs=KD_OLDSEEN_WARMUP_EPOCHS_ARM2_ARM4, uses_dense_orth=True,
    )
    # ==========================================================================
...
# SINGLEJOB PACK FIX (infrastructure/validation-only): Exp1's historical
# simple_avg_kd_T2/rank_extension_kd_only_T2 are DISABLED in this script (not
# retrained -- see METHODS_TO_RUN), so `ACTIVE_METHOD_MAP["simple_avg_kd_T2"]`
# would KeyError here. Replaced with the equivalent invariant check on THIS
# script's own 3 KD-using arms (1/2/4) -- same purpose (catch config drift
# before training starts), adapted to what's actually active, and additionally
# verifying the new kd_class_scope/kd_warmup_epochs axes match the
# pre-registered design (Arm 1: old_seen, no warmup; Arms 2/4: old_seen, 1-epoch
# warmup) -- a strictly stronger check than the one it replaces, not a weaker one.
for _kd_arm in (
    "simple_avg_kd_oldseen_T2", "simple_avg_kd_oldseen_T2_warmup", "simple_avg_dense_orth_lam20_kd_oldseen_T2_warmup",
    "rank_extension_kd_oldseen_T2_warmup", "rank_extension_dense_orth_lam20_kd_oldseen_T2_warmup",
):
...
assert float(ACTIVE_METHOD_MAP["simple_avg_kd_oldseen_T2"]["kd_warmup_epochs"]) == 0.0, (
    "SA-1 (simple_avg_kd_oldseen_T2) must have NO KD-weight warmup"
)
# JOINT PACK: RankExt has no SA-1-equivalent (no-warmup) KD arm -- both of
# its KD-using arms (RE-1, RE-3) use the 1-epoch warmup by design (see the
# header comment / user's task Section 10).
for _kd_warmup_arm in (
    "simple_avg_kd_oldseen_T2_warmup", "simple_avg_dense_orth_lam20_kd_oldseen_T2_warmup",
    "rank_extension_kd_oldseen_T2_warmup", "rank_extension_dense_orth_lam20_kd_oldseen_T2_warmup",
):
    assert float(ACTIVE_METHOD_MAP[_kd_warmup_arm]["kd_warmup_epochs"]) == 1.0, (
        f"{_kd_warmup_arm} must have a 1-epoch KD-weight warmup"
    )
...
# SINGLEJOB PACK FIX (infrastructure/validation-only): rank_extension_orth_
# factor_lam_50_kd_T2 is disabled (RankExt out of scope for this job) -- would
# KeyError here. Replaced with the equivalent invariant check on THIS script's
# own 2 DenseOrth-using arms (3/4), verifying uses_dense_orth and the
# pre-registered lambda=20 (independent of LAMBDA_ORTH=50, which stays
# reserved for the untouched FactorOrth path -- see DENSE_ORTH_LAMBDA's own
# comment above).
for _dense_orth_arm in (
...
# JOINT PACK FIX (infrastructure/validation-only): extended from the
# SimpleAvg-only pack's 5-method set to this joint script's actual 9-method
# set (5 SimpleAvg + 4 RankExt).
_EXPECTED_SINGLEJOB_METHOD_NAMES = {
    ...
    "rank_extension",
    "rank_extension_kd_oldseen_T2_warmup",
    "rank_extension_dense_orth_lam20",
    "rank_extension_dense_orth_lam20_kd_oldseen_T2_warmup",
}
assert set(ACTIVE_METHOD_NAMES) == _EXPECTED_SINGLEJOB_METHOD_NAMES, (
    "This experiment must run exactly these 9 arms (5 SimpleAvg + 4 RankExt) -- mismatch: "
    f"{set(ACTIVE_METHOD_NAMES)} != {_EXPECTED_SINGLEJOB_METHOD_NAMES}"
)
assert sum(1 for m in ACTIVE_METHOD_NAMES if ACTIVE_METHOD_MAP[m]["family"] == "simple_avg") == 5, (
    "Must be exactly 5 SimpleAvg training arms"
)
...
assert len(ACTIVE_METHOD_NAMES) == 9, f"Expected 9 total training arms, got {len(ACTIVE_METHOD_NAMES)}"
...
_expected_final_configs = len(_sa_arms) * 2 + len(_re_arms) * 1
print("=" * 80)
print("JOINT SIMPLEAVG+RANKEXT SINGLE-JOB PACK -- READINESS VERIFICATION")
print("=" * 80)
print(f"SCRIPT UNDER TEST: experiments_prepared/simpleavg_rankext_singlejob_improvement_pack_5x20.py")
...
_expected_merge_methods = ["arithmetic", "do_merge"]  # cross-checked against the real MERGE_METHODS_TO_EVALUATE constant at its own definition site, later in this file
...
print("=" * 80)
assert FAST_RUN or (LORA_EPOCHS == 6 and RANKEXT_EPOCHS == 6)
assert len(_sa_arms) == 5 and len(_re_arms) == 4 and len(ACTIVE_METHOD_NAMES) == 9
assert _expected_final_configs == 14, f"Expected 14 final configurations, computed {_expected_final_configs}"
...
# Merge B (do_merge) is included per the pre-registered design; Merge C
# (validation-fit scalar weights) was deliberately OMITTED from this single
# job -- see the preparation audit report's justification. It remains a
# zero-retraining follow-up against these same persisted checkpoints.
MERGE_METHODS_TO_EVALUATE = ["arithmetic", "do_merge"]
...
historical_r7_reference_rows = [
    {"training_method": "simple_avg", "merge_method": "arithmetic", "source": "R7/Exp1 seed42 (not retrained)", "first_step": 77.75, "later_steps": 74.5125, "all_seen": 75.16, "restricted_mean": 92.28, "backward_transfer": -0.212},
    {"training_method": "simple_avg_kd_T2", "merge_method": "arithmetic", "source": "R7/Exp1 seed42 (not retrained)", "first_step": 92.35, "later_steps": 55.6375, "all_seen": 62.98, "restricted_mean": 91.91, "backward_transfer": -0.252},
    {"training_method": "simple_avg_factor_orth", "merge_method": "arithmetic", "source": "R7/Exp1 seed42 (not retrained)", "first_step": 76.45, "later_steps": 73.75, "all_seen": 74.29, "restricted_mean": 93.24, "backward_transfer": -0.227},
    {"training_method": "simple_avg_factor_orth_kd_T2", "merge_method": "arithmetic", "source": "R7/Exp1 seed42 (not retrained)", "first_step": 69.10, "later_steps": 69.475, "all_seen": 69.40, "restricted_mean": 92.85, "backward_transfer": -0.219},
]
