...
# =============================================================================
# SUPERVISOR EXPERIMENT 2 (prepared 2026-09-15): CIFAR-100 5x20, SimpleAvg
# rank 32 vs. RankExt cumulative schedule [32,64,96,128,160] (+32/step).
#
# This is a direct, scientific-parent derivative of the already-COMPLETED
# Supervisor Experiment 1 (experiments_prepared/supervisor_exp1_cifar100_
# 5x20_fixed_rankext.py, untouched by this file). Every setting below is
# byte-identical to Experiment 1 EXCEPT the three intentional capacity/rank
# changes documented at "EXPERIMENT 2 OVERRIDE" comments throughout this file:
#   1. SimpleAvg LORA_R:            80 -> 32   (LORA_ALPHA = 2*LORA_R -> 64)
#   2. RankExt RANKEXT_RANK_SCHEDULE: [16,32,48,64,80] -> [32,64,96,128,160]
#      (Experiment 1's schedule appends a fixed +16 new rank per step;
#      Experiment 2's appends a fixed +32 new rank per step instead.
#      RANKEXT_ALPHA_PER_RANK stays 2.0, unchanged, so per-step scaling
#      remains exactly 2.0 at every step of both schedules -- see
#      active_rankext_lora_alpha() / GrowingRankLoRALinear.scaling below,
#      neither of which is edited by this file).
#   3. (Consequence of 1+2, not an independent lever) The Experiment-1-only
#      "final-rank-matched" invariant asserts (LORA_R == RANKEXT_RANK_
#      SCHEDULE[-1], etc.) are REMOVED here: Experiment 2 deliberately runs
#      SimpleAvg and RankExt at DIFFERENT final capacities (32 vs. 160) per
#      the supervisor's request, so that invariant no longer holds by design.
#
# This file does NOT import or reuse anything from
# supervisor_regularizer_repair_both_families_r8.py ("R8"): no R8-corrected
# KD (old-seen-only masking), no R8 DenseOrth formulation. Every KD/
# FactorOrth/calibration/merge/best-epoch/metric algorithm below is reused
# UNCHANGED from Experiment 1 (which itself reuses it unchanged from the
# canonical n5.py source -- see Experiment 1's own PROVENANCE NOTE below,
# still accurate for this file). See thesis_agent/reports/
# supervisor_exp2_preparation_audit.md for the full static difference audit.
# =============================================================================
...
# Method notes:
# 
# - `simple_avg` trains one independent LoRA per step with LoRA rank `32` and alpha `64` (EXPERIMENT 2 OVERRIDE; Experiment 1 used rank `80`/alpha `160`), then merges step deltas by simple averaging.
...
# - `rank_extension` is a true growing-rank LoRA with rank schedule `32 -> 64 -> 96 -> 128 -> 160` (EXPERIMENT 2 OVERRIDE; Experiment 1 used `16 -> 32 -> 48 -> 64 -> 80`): previous `A/B` slices are copied forward, frozen, and only the newest `A/B` slice is trainable.
...
# - EXPERIMENT 2 OVERRIDE: unlike Experiment 1 (which compared `simple_avg` and `rank_extension` at the same final rank `80`), Experiment 2 deliberately runs them at DIFFERENT final capacities -- `simple_avg` final rank `32`/`LORA_ALPHA = 64`, `rank_extension` final cumulative rank `160` -- per the supervisor's capacity-configuration request. Both keep scaling (`alpha/rank`) exactly `2.0`.
...
# EXPERIMENT 2 OVERRIDE: distinct run name so this can never collide with or
# overwrite Experiment 1's output directory (Exp1's RUN_NAME_BASE ends in
# "_fixed_rankext_hist_schedule_seed{SEED}"), R7, R8, or any canonical result.
RUN_NAME_BASE = f"clip_vit_lora_cifar100_5x20_supervisor_exp2_sa32_re160_seed{SEED}"
...
# EXPERIMENT 2 OVERRIDE (supervisor-requested capacity-configuration change):
# SimpleAvg LORA_R 80 -> 32. LORA_ALPHA stays derived as 2*LORA_R (-> 64), so
# scaling (alpha/rank) stays exactly 2.0, matching Experiment 1's SimpleAvg
# scaling and RankExt's RANKEXT_ALPHA_PER_RANK. This is one of only two
# intentional scientific changes from Experiment 1 (the other is
# RANKEXT_RANK_SCHEDULE below) -- everything else in this file is
# byte-identical to Experiment 1's source.
LORA_R = 32
LORA_ALPHA = 2 * LORA_R
...
# EXPERIMENT 2 OVERRIDE: Experiment 1's historical schedule [16,32,48,64,80]
# (+16 new rank/step) -> [32,64,96,128,160] (+32 new rank/step), per the
# supervisor's second requested 5x20 experiment. RANKEXT_ALPHA_PER_RANK stays
# 2.0 -- GrowingRankLoRALinear.scaling = RANKEXT_ALPHA_PER_RANK is constant at
# every step regardless of total_rank (see active_rankext_lora_alpha()'s own
# docstring below, unchanged), so scaling remains exactly 2.0 at every step of
# this wider schedule with zero other code changes required.
RANKEXT_RANK_SCHEDULE = [32, 64, 96, 128, 160]
...
# EXPERIMENT 2 OVERRIDE: resized/rescaled to keep this file's own "2x the
# active default schedule, elementwise" convention (compare RANKEXT_RANK_
# SCHEDULE=[32,64,96,128,160] above to [64,128,192,256,320] here -- exactly 2x
# per entry, same relationship Experiment 1 had between its [16,32,48,64,80]
# and this list's old [32,64,96,128,160]). Still DEAD CODE for training
# purposes: USE_RANKEXT_RANK_SCHEDULE_WIDE stays False below (Experiment 2 is
# a fixed-schedule capacity-configuration run, not a wide-schedule ablation)
# -- this list only exists to satisfy the unconditional length/monotonicity
# asserts a few lines down.
RANKEXT_RANK_SCHEDULE_WIDE = [64, 128, 192, 256, 320]
...
# EXPERIMENT 2 OVERRIDE: SimpleAvg rank/alpha are 32/64 here, not Experiment
# 1's 80/160 -- see LORA_R's own "EXPERIMENT 2 OVERRIDE" comment above.
assert LORA_R == 32
assert LORA_ALPHA == 64
# EXPERIMENT 2 OVERRIDE: Experiment 1's "final-rank-matched invariant"
# (LORA_R == RANKEXT_RANK_SCHEDULE[-1], and the matching alpha-equality
# check) is DELIBERATELY REMOVED here. Experiment 2 intentionally runs
# SimpleAvg (final rank 32) and RankExt (final cumulative rank 160) at
# DIFFERENT final capacities -- per the supervisor's capacity-configuration
# request -- so that invariant no longer holds by design, not by oversight.
# This is a necessary structural consequence of the two intentional changes
# (SimpleAvg rank 80->32, RankExt schedule ...->[32,64,96,128,160]), not a
# third independent scientific lever. See supervisor_exp2_preparation_audit.md.
assert LORA_R != RANKEXT_RANK_SCHEDULE[-1], (
    "Experiment 2 intentionally decouples SimpleAvg's final rank from "
    f"RankExt's final cumulative rank -- got LORA_R={LORA_R}, "
    f"RANKEXT_RANK_SCHEDULE[-1]={RANKEXT_RANK_SCHEDULE[-1]} (unexpectedly equal)"
)
...
# =============================================================================
# EXPERIMENT 2 HARD ASSERTIONS + STARTUP DIAGNOSTICS (prepared 2026-09-15,
# direct derivative of Experiment 1's own hard-assertion block above -- see
# this file's own top-of-file header and
# thesis_agent/reports/supervisor_exp2_preparation_audit.md). Fails fast,
# before epoch 1, if this file's own required overrides above did not take
# effect (e.g. an accidental merge/edit reverted one back to Experiment 1's
# canonical values, or a stray env var interfered). Every check below is
# against LIVE state (the actual resolved constants/objects at this point in
# module execution), not against this file's own source text.
# =============================================================================
...
assert NUM_CLASSES == 100, f"Experiment 2 requires 100 total classes, got {NUM_CLASSES}"
...
# --- no R8 corrected-KD / R8 DenseOrth contamination ------------------------
# Defense-in-depth sentinel (same pattern as the ImageNet leftover-name check
# above): these names are unique to supervisor_regularizer_repair_both_
# families_r8.py's R8-corrected KD (old-seen-only masking) and R8 DenseOrth
# formulation, which this file must NOT import, port, or reimplement (see
# this file's own top-of-file header). Their presence in globals() would mean
# R8 code was accidentally pasted into this file.
for _leftover_r8_name in (
    "uses_dense_orth", "DenseOrth", "KD_WARMUP_ENABLED", "KD_WARMUP_EPOCHS",
    "KD_BASE_WEIGHT", "ORTH_WARMUP_ENABLED", "ORTH_WARMUP_EPOCHS",
    "old_seen_class_ids", "LORA_SCALING", "RANKEXT_SCALING",
):
    assert _leftover_r8_name not in globals(), (
        f"No R8 corrected-KD/DenseOrth code must be present: found leftover name {_leftover_r8_name!r}"
    )
for _forbidden_substring in ("dense_orth", "old_seen", "oldseen", "r8"):
    assert _forbidden_substring not in RUN_NAME_BASE.lower(), (
        f"RUN_NAME_BASE must not mention R8/DenseOrth/old-seen wording: {RUN_NAME_BASE!r}"
    )

# --- protocol / seed ---------------------------------------------------------
assert SEED == 42, f"Experiment 2 requires seed=42, got {SEED}"
assert NUM_STEPS == 5 and CLASSES_PER_STEP == 20, (
    f"Experiment 2 is the 5x20 protocol; got NUM_STEPS={NUM_STEPS}, "
...
assert not USE_RANKEXT_RANK_SCHEDULE_WIDE, "Wide RankExt schedule (4x25 capacity ablation) must be OFF for Experiment 2"
# EXPERIMENT 2 OVERRIDE: cumulative schedule [32,64,96,128,160], i.e. +32 new
# rank appended per step (Experiment 1 used [16,32,48,64,80], +16/step).
assert active_rankext_rank_schedule() == [32, 64, 96, 128, 160], (
    f"Experiment 2 requires the RankExt cumulative schedule [32,64,96,128,160] "
    f"(+32 rank per step), got {active_rankext_rank_schedule()}"
)
_rankext_schedule_active = active_rankext_rank_schedule()
_rankext_increments = [
    _rankext_schedule_active[0],
    *[_rankext_schedule_active[i] - _rankext_schedule_active[i - 1] for i in range(1, len(_rankext_schedule_active))],
]
assert all(inc == 32 for inc in _rankext_increments), (
    f"Experiment 2 requires a uniform +32 new-rank increment at every step, got increments={_rankext_increments}"
)
...
# --- SimpleAvg (EXPERIMENT 2 capacity config, NOT Experiment 1's 80/160) --
assert LORA_R == 32 and LORA_ALPHA == 64, (
    f"Experiment 2's SimpleAvg config must be the supervisor-requested "
    f"rank=32/alpha=64, got LORA_R={LORA_R}, LORA_ALPHA={LORA_ALPHA}"
)
...
# Same 8 historical/canonical method identifiers as Experiment 1 -- NOT
# renamed into R8 IDs (see this file's own top-of-file header).
_EXPECTED_EXP2_METHOD_NAMES = {
...
assert set(ACTIVE_METHOD_NAMES) == _EXPECTED_EXP2_METHOD_NAMES, (
    "Experiment 2 must run exactly these 8 methods -- mismatch: "
    f"{set(ACTIVE_METHOD_NAMES)} != {_EXPECTED_EXP2_METHOD_NAMES}"
)
assert len(ACTIVE_METHOD_NAMES) == 8
assert set(ENABLED_METHOD_FAMILIES) == EXPECTED_ENABLED_METHOD_FAMILIES

print("=" * 80)
print("EXPERIMENT 2 STARTUP DIAGNOSTICS -- 5x20 capacity-configuration comparison,")
print("SimpleAvg rank 32 vs. RankExt cumulative schedule [32,64,96,128,160] (+32/step)")
print("=" * 80)
...
print(f"                          'EXPERIMENT 2 dataset identity check' printed later, right")
...
print("EXPERIMENT 2 hard assertions PASSED.")
print("=" * 80)
...
# DATASET IDENTITY CHECK (deferred from the EXPERIMENT 2 HARD ASSERTIONS
...
print(f"EXPERIMENT 2 dataset identity check PASSED: CIFAR-100, "
