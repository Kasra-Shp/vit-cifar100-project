...
Classifier-row / feature protection & --- & Protect30 (weight 30, KD-bearing arms only) \\
Epochs per task & \multicolumn{2}{l}{9} \\
Optimizer / scheduler & \multicolumn{2}{l}{AdamW, cosine schedule, 10\% warmup ratio} \\
Replay & \multicolumn{2}{l}{None (rehearsal-free)} \\
Calibration & \multicolumn{2}{l}{Confidence-weighted, regime-grouped row-norm calibration only} \\
Seed & \multicolumn{2}{l}{42} \\
\bottomrule
\end{tabularx}
\caption[Hyperparameter reference]{Consolidated hyperparameter and protocol
reference for the primary benchmark.}
...
combined RankExt configuration (FactorOrth + full KD + Protect30) at
2 tasks, 2 epochs per task, and 10--15 images per class, across two seeds
...
decision probe on the combined RankExt configuration (2 seeds; 2 tasks, 2
epochs/task, 10--15 images/class). Non-benchmark diagnostic evidence.}
...
open (all-seen) accuracy values underlying the four $R_{i,j}$ heatmaps of
Figure~\ref{fig:ch5-rij-comparison}, for readers who want precise numbers
rather than a colour scale. $R_{i,j}$ is the accuracy on task $j$ after
training has completed through task $i$ ($j \le i$); empty cells are
structurally undefined (a task cannot be evaluated before it is trained).
