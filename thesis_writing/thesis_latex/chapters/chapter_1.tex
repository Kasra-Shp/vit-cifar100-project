...
adaptations, while RankExt tests how closely a persistent, growing-rank
adapter with incrementally introduced capacity can approach that
...
thesis examines the retention--plasticity behaviour each produces within a
shared rehearsal-free experimental framework. Figure~\ref{fig:ch1-integration-strategies}
...
per-step adaptations, while RankExt tests how closely a persistent,
growing-rank adapter with incrementally introduced capacity can approach
...
closely a persistent, growing-rank adapter with incrementally introduced
capacity can approach the reference provided by independently trained
...
comparison, while RankExt tests how closely a persistent, growing-rank
adapter with incrementally introduced capacity can approach that reference
...
\item \textbf{A controlled comparison of two LoRA integration strategies
   ...
   active, within the primary controlled CIFAR-100 protocol, using the same backbone, class
...
concrete formulation is family-specific, as stabilisation mechanisms, the classifier construction and classifier rescaling applied to both
...
experimentation: the datasets and class-incremental protocols under which
it is studied --- CIFAR-100 as the primary controlled benchmark and
CUB-200-2011 as a second-dataset supporting evaluation --- the backbone and adaptation configuration, the
...
within RankExt, the paired open/restricted diagnostic evaluation, the
supporting CUB-200-2011 evaluation, and supporting capacity analyses where
relevant.
