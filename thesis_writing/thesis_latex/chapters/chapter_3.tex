...
fixes the concrete instantiation used to obtain the results reported in
this thesis; a future dataset or protocol would be added here, as an
additional row of Table~\ref{tab:ch3-protocol-config}, without requiring
any change to the general methodology in the rest of this chapter.

The dataset used throughout this thesis is CIFAR-100, with $C=100$
classes. The \emph{primary} protocol partitions these into five incremental
steps of 20 classes each ($T=5$, $|C_t|=20$, $5\times20$); this is the
canonical protocol against which every method in this chapter is defined
and compared, and the one used for the main results reported in
Chapter~\ref{ch:results}. Unless a passage explicitly notes otherwise,
every concrete number stated elsewhere in this chapter --- a rank
schedule, the classifier's dimensionality, and so on --- refers to this
primary protocol.

Holding $C=100$ fixed, the same class set could also be partitioned more
coarsely or more finely. No such alternative protocol depth is evaluated in
this thesis; protocol depth is discussed only as literature background
(Section~\ref{sec:protocol-depth}) and as a direction for future work
...
\toprule
\textbf{Dataset} & \textbf{Total classes ($C$)} & \textbf{Main protocol} \\
\midrule
CIFAR-100 & 100 & 5 incremental steps $\times$ 20 classes \\
\bottomrule
\end{tabularx}
\caption[Dataset and protocol configuration]{Dataset and primary protocol
configuration used in this thesis. A future dataset would extend this
table with one additional row; no other part of this chapter's methodology
...
with calibration, is developed in Section~\ref{sec:ch3-classifier-calibration}.
...
Rather than training a fresh adapter at every step, it maintains one
persistent model across the entire sequence. At each step, every low-rank
block learned at a previous step is retained and kept frozen, and exactly
one new block is appended and trained; the pretrained base weight $W_0$
remains frozen throughout, as it does for SimpleAvg.
...
general formulation above. A different partition of the same class set would require a
correspondingly adjusted schedule; no such alternative is evaluated in this
thesis. A previously learned block's parameters have zero gradient from
...
and calibration mechanism is again deferred to Section~\ref{sec:ch3-classifier-calibration}. Growing-rank,
...
it permanently, appends one new trainable block per step, and persists a
single evolving model throughout the sequence. Both strategies are, in
...
\textbf{full} output distribution over every class seen so far, including
the classes newly introduced at the current step. For SimpleAvg, it is
...
performed once after the complete sequence, described in Section~\ref{sec:ch3-simpleavg}. RankExt, by contrast, already maintains one persistent,
complete model at every point in the sequence, so its teacher is simply
that model as it stood at the end of the previous step, before the current
step's new block is added: no separate merge-and-stitch operation is
needed to construct it, because RankExt's own architecture already
supplies a directly usable checkpoint at every step boundary.
...
\section{Classifier Construction and Calibration}
...
touches the classifier, and, symmetrically, the post-hoc calibration
...
covering every class the model will ever see ($C=100$ in the CIFAR-100
protocol of Section~\ref{sec:ch3-protocol-instantiation}). This is precisely the classifier component of the three-way
...
For RankExt, one classifier instance persists across the entire sequence,
as already noted in Section~\ref{sec:ch3-rankext}, mirroring the persistence of the
low-rank blocks themselves: neither the classifier nor the adapter is ever
rebuilt from scratch after the first step. Rows belonging to classes
...
Calibration is not part of gradient-based training, does not touch the
...
mean norm. The calibration used in this thesis is not a direct
...
evaluation mode will later be applied, the calibrated classifier is what
...
diagnostic reported for the completed model uses the final calibrated model
...
This diagnostic role connects directly to the calibration mechanism of
Section~\ref{sec:ch3-calibration}. Because the same positive factor is
applied to all weight rows within a step, calibration does not alter their
relative weight-vector scaling. However, classifier biases are left
unchanged, so restricted accuracy is not mathematically guaranteed to
remain invariant under the rescaling. Open accuracy is more directly
exposed to calibration, since its argmax compares logits across groups that
calibration may have scaled differently.
...
construction and calibration, and the open/restricted evaluation
...
calibration procedure, and the evaluation protocols --- fixed across all
...
target modules, the same classifier mechanism, the same calibration
procedure, and the same evaluation protocols (Sections~\ref{sec:ch3-lora-framework}, \ref{sec:ch3-classifier-calibration}, \ref{sec:ch3-open-restricted}).
...
\item Configure the current adaptation state: continue the single persistent
   model carried over from step $t-1$ --- the base weight remains frozen,
   every previously learned rank block remains frozen, and one new
   trainable block is appended (Section~\ref{sec:ch3-rankext}).
...
\item At the end of the step, the persistent model itself --- now including the
   newly trained block and the newly trained classifier rows --- is carried
   forward as the retained state for step $t+1$; no separate extraction or
   reconstruction is needed, and no raw example of $D_t$ is kept.
\end{enumerate}

Because the persistent model produced by this loop already is the model at
...
assembled: the configured post-hoc classifier calibration is applied
...
RankExt, $a_{j,j}$ is a genuine checkpoint: the persistent model exactly as
it existed immediately after step $j$, evaluated on $C_j$. For SimpleAvg,
