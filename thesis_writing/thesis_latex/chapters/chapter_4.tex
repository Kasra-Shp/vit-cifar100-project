...
objectives structure the chapter: to fix, once, the dataset, protocol,
backbone, and training configuration shared by every experiment reported in
...
At the time of writing, \textbf{CIFAR-100} \parencite{krizhevsky2009learning}
is the dataset used for all completed experiments reported in this thesis:
100 fine-grained object classes, 50,000 training images and 10,000 test
images, loaded in its native (unshuffled) label order. Every
dataset-specific setting in the remainder of this chapter is stated for
CIFAR-100.
...
No alternative class-incremental partition is reported in this thesis:
every result in Chapter~\ref{ch:results} uses the primary $5\times20$
protocol, and the effect of protocol depth is left to future work
(Section~\ref{sec:ch6-future-work}). No dataset-level accuracy number
appears in this section; results under the primary protocol are reported in
...
$\mathbb{R}^{768}\to\mathbb{R}^{100}$ (Section~\ref{sec:ch3-classifier-construction}).
...
it compares the full output distribution over every class seen so far, and
...
\textbf{Compute environment.} Training was performed on the University of
Padova DEI computing cluster using one NVIDIA A40 GPU per job, with jobs
...
have already trained earlier in the same process. The default seed is 42,
used for every primary $5\times20$ result reported in
Chapter~\ref{ch:results}. Every primary $5\times20$ result is single-seed; no primary result is
