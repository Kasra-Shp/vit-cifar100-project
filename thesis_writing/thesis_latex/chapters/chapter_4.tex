...
Every main result in Chapter~\ref{ch:results} uses the primary
$5\times20$ protocol. The only alternative partition reported is a
secondary $4\times25$ diagnostic for the RankExt family
(Section~\ref{sec:ch5-ablation}); a systematic study of protocol depth is
left to future work (Section~\ref{sec:ch6-future-work}). No dataset-level accuracy number
...
it compares the full 100-way output distribution, which covers every class
seen so far, and
