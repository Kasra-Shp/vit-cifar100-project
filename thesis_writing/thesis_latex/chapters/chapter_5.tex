...
bound on within-step discrimination, not a replacement headline metric.
...
(within-step discriminability) from the cross-step competition effects that
...
\parencite{lopezpaz2017gradient}: the mean, over steps 1 through 4, of the
difference between a step's accuracy at the end of the full sequence and its
accuracy immediately after it was first learned. In the reported values,
the end-of-sequence term is the step's open accuracy in the final model
\emph{after} classifier rescaling, while the ``immediately after learning''
term is evaluated during training, at the end of that step's own training
...
rather than repeated at every table: \textbf{RankExt carries a single, persistent
adapter state}, trained sequentially, so its BWT has the standard
interpretation -- a genuine measurement of how later training affects
earlier-learned steps in the same evolving adapter state. \textbf{SimpleAvg is not a single evolving model};
each step is trained as an independent specialist and the five specialists
...
RankExt, where it is well-defined (the mean, over steps 1--4, of each step's
...
evaluations made after each step, before classifier rescaling --- the same
...
redistribution as a factual pattern --- accuracy moves from the first step
toward later steps whenever KD is present, roughly in proportion to how it
...
RankExt, and presents the per-step retention trajectory underlying
...
persistent adapter state, such as RankExt, this chapter additionally reports the full retention
matrix $R_{i,j}$: the open (all-seen) accuracy on step $j$, evaluated after
...
pattern. Restricted evaluation removes cross-step class competitors from the
decision rule and therefore provides a diagnostic view of within-step
...
At every step, RankExt trains fewer new parameters than SimpleAvg (589{,}824
vs.\ 2{,}949{,}120, both constant across steps,
...
the final step, exactly matching its own final active rank (80), because
...
mixed directions at $\times2$ (first-step accuracy rises from 60.70\% to
67.75\%; BWT improves from $-$0.1584 to $-$0.0532; forgetting improves from
0.2474 to 0.1289; later-steps accuracy falls from 73.28\% to 67.96\%),
