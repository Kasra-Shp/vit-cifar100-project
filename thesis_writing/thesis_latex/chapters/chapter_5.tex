...
This chapter reports results on \textbf{CIFAR-100} under a \textbf{5-task,
20-class-per-task} class-incremental protocol ($5\times20$), evaluated at
\textbf{seed 42}. The backbone, LoRA configuration, method definitions, and
...
against only the current task's own 20-class subset --- a diagnostic upper
...
merged into this benchmark; the refinement and protocol-diagnostic
experiments are reported separately in Section~\ref{sec:ch5-ablation}.
...
training has completed sequentially through task $i$ ($j \le i$). The four
matrices are shown together in Figure~\ref{fig:ch5-rij-comparison}, which
uses one shared 0--100\% colour scale across all four panels so the visual
difference between configurations is directly comparable. Each row $i$ of
$R_{i,j}$ is evaluated during training, immediately after task $i$ has been
trained (from its best-validation checkpoint), before classifier rescaling,
with predictions taken over the full 100-way classifier output; at $i=5$
this coincides with evaluation over all seen classes. The final row of
$R_{i,j}$ therefore describes the model before classifier rescaling, whereas
the final accuracies reported in Table~\ref{tab:ch5-main-extended}
describe the same weights after it. The two are not interchangeable: for plain
RankExt, for example, task 1 reaches 6.40\% in the final row of $R_{i,j}$
and 11.70\% after classifier rescaling.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/chapter5/cifar100_rankext_Rij_comparison.pdf}
\caption[RankExt $R_{i,j}$ retention matrices]{$R_{i,j}$ retention matrices
for all four RankExt configurations (open accuracy, shared 0--100\% colour
scale): plain, FactorOrth, KD+Protect, and combined. $R_{i,j}$ = accuracy on
task $j$ after training has completed through task $i$, evaluated before
classifier rescaling.}
...
both deltas as observed, single-seed results; a mechanistic reading
(drawing on independently-derived loss-magnitude evidence) is given in
Chapter~\ref{ch:discussion-conclusion}.
...
SimpleAvg's two FactorOrth variants.
...
result. The reported benchmark and refinement results are single-seed
