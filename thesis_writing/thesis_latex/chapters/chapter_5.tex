...
This chapter reports the empirical results of the experiments defined in
Chapter~\ref{ch:experimental-setup}, beginning with the primary CIFAR-100
benchmark and the supporting CUB-200-2011 evaluation, followed by
per-family analyses and the targeted refinement and diagnostic
experiments. Chapter~\ref{ch:discussion-conclusion} interprets the
mechanisms and broader implications of these results.
...
under a \textbf{5-step, 20-class-per-step} class-incremental protocol
...
Section~\ref{sec:ch5-cub}. The method definitions and training
configuration are those of Chapters~\ref{ch:methodology}
and~\ref{ch:experimental-setup}; this section recaps only the metric
definitions, which apply to both datasets.

\textbf{All-seen (open) accuracy} is the primary continual-learning metric
throughout this chapter: accuracy on a step's own test images, evaluated
against the full, task-agnostic output space of every class introduced up to
and including the current step. Unless stated otherwise, ``accuracy'' in
this chapter means all-seen accuracy.

\textbf{Restricted (task-oracle) accuracy} evaluates the same predictions
against only the current step's own class subset (20 classes on
CIFAR-100, 40 on CUB-200-2011). Restricted accuracy is used as a
diagnostic of within-step discrimination rather than as the headline
metric; the two measures are reported side by side because they can
diverge substantially (Section~\ref{sec:ch5-stability}).

\textbf{Classifier rescaling.} After training is complete, the classifier
weight rows associated with each step are rescaled by a positive
step-specific factor to reduce scale imbalance across steps
...
\textbf{First-step accuracy} is the open accuracy of the final model on
the classes introduced at step 1. \textbf{Later-steps accuracy} is the open
accuracy of the final model over the pooled union of evaluation examples
from steps 2--5; it is a single accuracy over that pooled set, not an
arithmetic mean of separately computed step-level accuracies. Both are
measured after the full five-step sequence has completed, and together
they indicate whether a method's aggregate accuracy is limited by
early-step retention, later-step acquisition, or both.

\textbf{Backward transfer (BWT)} \parencite{lopezpaz2017gradient} is the
mean, over steps 1 through 4, of the difference between a step's accuracy at
the end of the full sequence and its accuracy immediately after it was first
learned. The end-of-sequence term is the step's open accuracy in the final
model \emph{after} classifier rescaling; the reference term is evaluated at
the end of that step's own training (from its best-validation checkpoint),
\emph{before} classifier rescaling. For RankExt, which carries a single,
persistent adapter state, this reference is a genuine sequential
checkpoint. For SimpleAvg, whose specialists are trained independently and
merged only once at the end (Chapter~\ref{ch:methodology}), it is each
specialist's own accuracy before the merge. The BWT values of the two
families are therefore \textbf{not structurally equivalent} and should not
be compared directly.

\textbf{Forgetting} \parencite{chaudhry2018riemannian} is the mean, over
steps 1--4, of each step's best-ever observed accuracy minus its accuracy at
the end of the sequence. Unlike BWT, it is computed entirely from the
in-training evaluations made after each step, before classifier rescaling
--- the values that form the $R_{i,j}$ matrices of
Section~\ref{sec:ch5-rankext}. It is reported for RankExt only: SimpleAvg has
no sequential merged-model trajectory, so the metric is not defined for it
(``---'' in the tables).

The primary CIFAR-100 benchmark and the CUB-200-2011 supporting evaluation
are single-seed results (seed 42); no statistical significance testing is
reported.

\section{CIFAR-100 Main Results}
\label{sec:ch5-main-results}

The primary benchmark compares \textbf{eight} methods under the CIFAR-100
$5\times20$ protocol at seed 42. All KD-bearing configurations use a
distillation temperature of $T=2$, as specified in
Chapter~\ref{ch:experimental-setup}. The CUB-200-2011 evaluation is reported
...
reporting first-step accuracy, later-steps accuracy, BWT, and
forgetting for all eight methods, is given as
Table~\ref{tab:ch5-main-extended} in Section~\ref{sec:ch5-summary}.

Two patterns are immediately visible. The four SimpleAvg variants occupy a narrow band, from 73.22\% to
...
highest observed SimpleAvg result (SimpleAvg + KD, 75.21\%) and the highest
observed RankExt result (RankExt + FactorOrth + KD + Protect, 70.76\%)
differ by \textbf{4.45 percentage points}, with SimpleAvg ahead
(Figure~\ref{fig:ch5-best-per-family}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{figures/chapter5/cifar100_best_per_family.pdf}
\caption[Highest observed result per family]{Highest observed all-seen
accuracy per family in the primary run:
SimpleAvg + KD (75.21\%) versus RankExt + FactorOrth + KD + Protect
(70.76\%).}
\label{fig:ch5-best-per-family}
\end{figure}

SimpleAvg establishes a strong baseline near 75\% without requiring any
auxiliary objective, and its three auxiliary-loss variants do not move
materially away from that baseline in either direction. RankExt's plain
configuration is far weaker, and the configurations with auxiliary
objectives reach substantially higher all-seen accuracy, up to 70.76\%.
Sections~\ref{sec:ch5-simpleavg} and~\ref{sec:ch5-rankext} examine each
family in turn, and Chapter~\ref{ch:discussion-conclusion} discusses the
...
\textbf{SimpleAvg.} In the CUB experiment, none of the
...
6.04-percentage-point difference in the CUB experiment.
...
plain SimpleAvg (75.16\%), alongside first-step and later-steps
accuracy.
...
\toprule
\textbf{Method} & \shortstack[r]{\textbf{All-seen}\\\textbf{(\%)}} & \shortstack[r]{\textbf{$\Delta$ vs.\ plain}\\\textbf{(pp)}} & \shortstack[r]{\textbf{First-step}\\\textbf{(\%)}} & \shortstack[r]{\textbf{Later-steps}\\\textbf{(\%)}} & \shortstack[r]{\textbf{Restricted}\\\textbf{(\%)}} \\
\midrule
...
delta from plain, first-step, later-steps, and restricted accuracy.}
...
The near-tied all-seen numbers mask a larger redistribution across steps.
SimpleAvg + KD's first-step accuracy (68.25\%) is 9.50 percentage points
below plain SimpleAvg's (77.75\%), while its later-steps accuracy (76.95\%) is
...
same direction at a larger magnitude --- first-step accuracy 57.55\% (20.20
points below plain), later-steps accuracy 77.14\% (the highest later-steps value
...
this redistribution: its first-step accuracy (76.60\%) and later-steps accuracy
(73.38\%) both stay close to plain's. Both KD-bearing variants therefore
shift accuracy from the first step toward later steps, with the larger
redistribution occurring when KD is combined with FactorOrth; the mechanism
behind this pattern is discussed in Chapter~\ref{ch:discussion-conclusion}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/chapter5/cifar100_sa_per_task_final_open.pdf}
\caption[SimpleAvg per-step final open accuracy]{Final (post-sequence) open
accuracy for each of the five steps, per SimpleAvg variant.}
\label{fig:ch5-sa-per-step-open}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/chapter5/cifar100_sa_per_task_restricted.pdf}
\caption[SimpleAvg per-step restricted accuracy]{Restricted (task-oracle)
accuracy for each of the five steps, per SimpleAvg variant.}
\label{fig:ch5-sa-per-step-restricted}
\end{figure}

Per-step detail (Figures~\ref{fig:ch5-sa-per-step-open}
and~\ref{fig:ch5-sa-per-step-restricted}) shows this redistribution is not
uniform across the four non-final steps: for both KD-bearing variants, step
1's open accuracy is the most affected, while steps 2--4 move by smaller and
...
91.97\% to 92.96\%. The per-step restricted results
(Figure~\ref{fig:ch5-sa-per-step-restricted}) provide the corresponding
step-level detail.

\section{RankExt Analysis}
\label{sec:ch5-rankext}

This section compares RankExt's four variants with plain RankExt and
presents the per-step retention trajectory underlying the aggregate
numbers.
...
Every auxiliary configuration improves on plain RankExt, but by very
different amounts. The FactorOrth-only configuration recovers 6.01
percentage points. The KD+Protect configuration, which combines knowledge
distillation with the projected feature-protection term, recovers 30.02
percentage points. The combined configuration (FactorOrth together with
KD+Protect) reaches the highest observed RankExt result, 70.76\%, 35.77
percentage points above plain and 5.75 points above the KD+Protect
configuration alone. BWT and forgetting improve monotonically in the same
order, consistent with the all-seen ranking. The largest observed
improvement within RankExt is therefore associated with the
KD+Protect-bearing configurations; because knowledge distillation and
projected feature protection are always active together in this benchmark,
the improvement is not attributed to either component in isolation.

\textbf{$R_{i,j}$ trajectories.} For a sequentially trained method with a
persistent adapter state, such as RankExt, the full retention matrix
$R_{i,j}$ is also reported: the open (all-seen) accuracy on step $j$, evaluated after
training has completed sequentially through step $i$ ($j \le i$). Each row
$i$ of $R_{i,j}$ is evaluated during training, immediately after step $i$
...
plain RankExt, for example, step 1 reaches 6.40\% in the final row of
...
each RankExt matrix, a separate ``Final'' row gives the per-step open
...
\caption[Retention matrices and final per-step accuracy]{Open (all-seen)
accuracy per step group, shared 0--100\% colour scale. Top row: for
RankExt, the panels show the sequential $R_{i,j}$ trajectory (accuracy on
step $j$ after training through step $i$, evaluated before classifier
...
only final merged-model per-step accuracies, after classifier rescaling.}
\label{fig:ch5-rij-comparison}
\end{figure}

Plain RankExt's matrix shows every step's own diagonal accuracy above 93\%
at the step it was learned (95.40\%, 95.70\%, 94.05\%, 93.90\%, 97.05\% for
steps 1--5 respectively), while its off-diagonal, older-step cells decline
steeply as later steps are trained: step 1's open accuracy, for example,
falls from 95.40\% (its own diagonal) to 43.50\% after step 2 is trained,
20.75\% after step 3, 12.10\% after step 4, and 6.40\% after step 5. The
combined configuration's matrix shows the same qualitative decline in
direction but at a markedly smaller magnitude: step 1's open accuracy after
step 5 is 66.40\%, compared with plain RankExt's 6.40\% at the same matrix
position. The KD+Protect configuration's matrix (step 1 at 60.10\% after
step 5) and the FactorOrth-only configuration's matrix (step 1 at 1.95\%
after step 5, the lowest value observed at that matrix position among the
four configurations) sit, respectively, close to and below the combined
configuration's retention level. The mechanistic interpretation of these
differences is discussed in Chapter~\ref{ch:discussion-conclusion}.

\section{Stability, Plasticity, and Open-vs-Restricted Performance}
\label{sec:ch5-stability}

This section compares open and restricted accuracy across all eight
methods.
...
method --- a diagnostic indicator of cross-step competition, not a direct
...
first-step redistribution described in Section~\ref{sec:ch5-simpleavg}) and
...
The gap indicates that within-step discrimination is substantially stronger
than task-agnostic discrimination across all seen classes for every RankExt
configuration, most severely for the two configurations without
KD+Protect. The open--restricted gap is used here as a diagnostic of
cross-step competition; it does not isolate a unique causal source of
error, and possible readings are examined in
...
\caption[First-step vs.\ later-steps accuracy]{First-step and later-steps
accuracy (pooled over steps 2--5) for all eight methods.}
\label{fig:ch5-first-vs-later}
\end{figure}

Figure~\ref{fig:ch5-first-vs-later} presents first-step and later-steps
...
and~\ref{sec:ch5-rankext}: SimpleAvg's KD-bearing variants trade first-step
accuracy for later-steps accuracy at roughly constant all-seen accuracy.
Within RankExt, the KD+Protect-bearing configurations substantially increase
both first-step and later-steps accuracy relative to plain RankExt, whereas
FactorOrth alone increases later-steps accuracy (48.79\% vs.\ 40.81\%) but
not first-step accuracy (9.85\% vs.\ 11.70\%)
(Table~\ref{tab:ch5-main-extended}).

\section{Parameter and Rank Efficiency}
\label{sec:ch5-efficiency}

This section complements the accuracy results with the structural
parameter cost of each family.
...
each of the five steps; RankExt trains a cumulative adapter that grows by a
fixed rank-16 increment at each step, reaching a cumulative rank of 80 only
at the fifth and final step (Figure~\ref{fig:ch5-rank-growth}). The two rank
trajectories reach the same final value by construction, but represent
structurally different histories: SimpleAvg's rank-80 at every step is a
\emph{new, independent} specialist, not an accumulation; RankExt's rank-80
at step 5 is the sum of five smaller, sequentially-added, never-retrained
...
\caption[LoRA rank growth]{LoRA rank used by each family across the 5-step
sequence. SimpleAvg's rank-80 is a new, independent specialist at every
step, not cumulative history in the same sense as RankExt's growing
adapter.}
...
\midrule
Rank behaviour & Fixed rank 80, fresh independent specialist per step & Growing cumulative rank: $16{\to}32{\to}48{\to}64{\to}80$ \\
New trainable LoRA params, per step & 2{,}949{,}120 (constant) & 589{,}824 (constant) \\
LoRA parameter volume touched across all 5 steps & 14{,}745{,}600 (5 independent adapters) & 2{,}949{,}120 (5 blocks, each trained once) \\
...
\caption[New trainable LoRA parameters per step]{New trainable LoRA
parameters introduced at each step, both families.}
\label{fig:ch5-trainable-params}
\end{figure}

RankExt introduces fewer new trainable parameters than SimpleAvg at every
step (Figure~\ref{fig:ch5-trainable-params}); because each of its blocks is
trained once and never retrained, its cumulative adapter matches its final
active rank of 80, whereas SimpleAvg's five independently trained
specialists are a training-time quantity that is not retained at
deployment. The deployment forms also differ. In the evaluated
implementation, SimpleAvg's averaged dense update is fused into the base
weights, so no separate low-rank adapter survives the merge; stored
separately, the dense update would occupy the larger figure in
Table~\ref{tab:ch5-param-efficiency}, because averaging operates on dense
rather than low-rank matrices. RankExt's cumulative adapter is instead
retained in low-rank, additive form and remains separable from the base
weights. These are structural parameter counts, not wall-clock or FLOP
measurements, and they describe different trade-offs between training-time
parameter volume and deployed-model structure rather than a single
efficiency ranking (Chapter~\ref{ch:discussion-conclusion}).
...
controlled two-run comparisons, not a replication of the primary benchmark
itself.
...
setting; the primary benchmark retains $\lambda=20$. Their interpretation is
discussed in Chapter~\ref{ch:discussion-conclusion}.
...
to 67.92\%, a change of $-$2.84 percentage points. This does not show that
the $\times2$ setting is worse in general: the combined configuration's
other metrics move in mixed directions at $\times2$ (first-step accuracy
rises from 60.70\% to 67.75\%; BWT improves from $-$0.1584 to $-$0.0532;
forgetting improves from 0.2474 to 0.1289; later-steps accuracy falls from
73.28\% to 67.96\%), a pattern interpreted in
Chapter~\ref{ch:discussion-conclusion}. Nor is $\times2$ globally better
...
of the combined RankExt configuration (2 steps, 2 epochs per step, 10--15
images per class) favoured it; this probe is not benchmark evidence. Its
preference did not fully transfer to the full five-step continual
...
\toprule
\textbf{Method} & \textbf{All-seen} & \textbf{First-step} & \textbf{Later-steps} & \textbf{Restricted} & \textbf{BWT} & \textbf{Forgetting} \\
\midrule
...
\textbf{Ranking.} SimpleAvg's four variants cluster
...
combined configuration reaches the highest observed RankExt result
(70.76\%), 4.45 percentage points below the highest observed SimpleAvg
result (75.21\%). RankExt's restricted accuracy remains high
(91.03\%--94.78\%) across all four configurations even where all-seen
accuracy is low, indicating that within-step discrimination is retained
more consistently than task-agnostic discrimination
(Section~\ref{sec:ch5-stability}). Parameter-efficiency accounting
(Section~\ref{sec:ch5-efficiency}) shows that RankExt's final adapter
matches one SimpleAvg specialist in parameter count, while the two families
follow structurally different training and deployment paths. A controlled refinement of two hyperparameters
...
\textbf{Limitations.} All results in this chapter --- the eight-method
benchmark and per-family analyses of
Sections~\ref{sec:ch5-main-results}--\ref{sec:ch5-efficiency}, the
refinement and protocol-diagnostic comparisons of
Section~\ref{sec:ch5-ablation}, and the CUB-200-2011 evaluation of
Section~\ref{sec:ch5-cub} --- are \textbf{single-seed (seed 42)}
observations; no confidence intervals or statistical-significance claims
are made. Differences such as the 4.45-percentage-point
SimpleAvg-vs-RankExt gap and the RankExt combined arm's $-$2.84-point change
under the head-learning-rate refinement are the observed outcomes of these
runs. The CUB-200-2011 results add a second dataset but do not replace
repeated-seed evidence, and because the CUB-200-2011 configuration also
changes the RankExt classifier-head learning rate, they support qualitative
rather than controlled cross-dataset conclusions.
Best-epoch selection (by validation cross-entropy, per method and per step)
